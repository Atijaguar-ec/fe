...
export class TextinputComponent implements OnInit {
  ...
  isNew = false;   // new object creation mode
  ...
  formatNumber(value, locale = 'sl-SI', useGrouping = false) {
    ...
    return new Intl.NumberFormat(locale, { useGrouping, minimumSignificantDigits: 1 }).format(value);
  }
  ...
  thousandsSeparator(locale = 'sl-SI') {
    const sep = this.decimalSeparator(locale);
    return sep === '.' ? ',' : '.';
  }
  ...
  parseNumber(value, locale = 'sl-SI') {
    ...
    const sep = this.decimalSeparator(locale);
    const sep1000 = this.thousandsSeparator(locale);
    if (value.indexOf(sep1000) >= 0) { return NaN; }
    const cleaned1000 = value.replace(sep1000, '');
  ...
  onChange(event) {
    ...
    // }
    const thsep = this.thousandsSeparator();
  ...
  isRequired() {
    ...
    if (!this.form) {
      ...
      const validator = this.languageFormArray.validator({} as AbstractControl);
  ...
  resubscribe() {
    if (this.languageFormArray) {
      ...
      this.langAndTextSubscription = combineLatest(this.currentLanguage$, this.languageFormControl.valueChanges,
        (language: string, text: string) => ({ language, text })
      ).pipe(
  ...
  setForLanguage(language: string, text: string) {
    if (!language) { return; }
    const res: Array<any> = this.languageFormArray.controls.filter(x => {
  ...
  clearEmptyLanguages() {
    let i = 0;
    for (const el of this.languageFormArray.controls) {
      const val = (el as FormControl).value.value;
      if (!val) {
