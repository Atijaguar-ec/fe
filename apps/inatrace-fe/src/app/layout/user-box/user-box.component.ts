...
export class UserBoxComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.showProductStakeholdersTabs = this.router.url.startsWith(
      `/product-labels/${this.productId}/stakeholders`,
    );
      ...
      this.authService.userProfile$
        ...
        .subscribe((companyProfile) => {
          if (user) {
            ...
            if (companyProfile) {
              this.isCompanyAdmin = user.companyIdsAdmin.includes(
                companyProfile.id,
              );
  ...
  goTo(type) {
    ...
    return this.router.navigate(
      ['/product-labels', this.productId].concat(arr),
    );
  }
