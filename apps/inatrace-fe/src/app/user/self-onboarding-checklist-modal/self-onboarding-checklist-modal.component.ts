...
export class SelfOnboardingChecklistModalComponent implements OnInit {
  selfOnboardingStatus = new UntypedFormGroup({
    hasProduct: new UntypedFormControl(true),
    hasFacility: new UntypedFormControl(false),
    hasProcessingAction: new UntypedFormControl(false),
    hasFarmers: new UntypedFormControl(false),
  });
      ...
      this.selUserCompanyService.selectedCompanyProfile$
        ...
        .subscribe(async (value) => {
          const companyOnboardingState = await this.companyControllerService
            .getCompanyOnboardingState(value.id)
            .toPromise();
  ...
  private updateSelfOnboardingStatus() {
    this.selfOnboardingStatus
      .get('hasProduct')
      .setValue(this.companyOnboardingState?.hasCreatedProduct);
    this.selfOnboardingStatus
      .get('hasFacility')
      .setValue(this.companyOnboardingState?.hasCreatedFacility);
    this.selfOnboardingStatus
      .get('hasProcessingAction')
      .setValue(this.companyOnboardingState?.hasCreatedProcessingAction);
    this.selfOnboardingStatus
      .get('hasFarmers')
      .setValue(this.companyOnboardingState?.hasAddedFarmers);
  }
}
