...
import { SelfOnboardingWelcomeModalComponent } from '../self-onboarding-welcome-modal/self-onboarding-welcome-modal.component';
import { SelfOnboardingAssistantModalComponent } from '../self-onboarding-assistant-modal/self-onboarding-assistant-modal.component';
import { SelfOnboardingService } from '../../shared-services/self-onboarding.service';
import { ChecklistAddProductSuccessModalComponent } from '../self-onboarding-checklist-modal/checklist-add-product-success-modal/checklist-add-product-success-modal.component';
import { ChecklistAddFacilitySuccessModalComponent } from '../self-onboarding-checklist-modal/checklist-add-facility-success-modal/checklist-add-facility-success-modal.component';
import { ChecklistAddProcessingActionSuccessModalComponent } from '../self-onboarding-checklist-modal/checklist-add-processing-action-success-modal/checklist-add-processing-action-success-modal.component';
import { CompanyControllerService } from '../../../api/api/companyController.service';
import { ChecklistAddFarmersSuccessModalComponent } from '../self-onboarding-checklist-modal/checklist-add-farmers-success-modal/checklist-add-farmers-success-modal.component';
import { NgbTooltip } from '@ng-bootstrap/ng-bootstrap';
import { Router } from '@angular/router';
import { GuidedTourSuccessModalComponent } from '../self-onboarding-checklist-modal/guided-tour-success-modal/guided-tour-success-modal.component';
...
export class UserHomeComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  products$ = combineLatest([this.paging$]).pipe(
    map(([page]) => {
      return {
    ...
    switchMap((reqParams) =>
      this.productControllerService.listProductsByMap(reqParams),
    ),
        ...
        if (this.user != null && company != null) {
          const addProductStep =
            await this.selfOnboardingService.addProductCurrentStep$
              .pipe(take(1))
              .toPromise();
          const addFacilityStep =
            await this.selfOnboardingService.addFacilityCurrentStep$
              .pipe(take(1))
              .toPromise();
          const addProcessingActionStep =
            await this.selfOnboardingService.addProcessingActionCurrentStep$
              .pipe(take(1))
              .toPromise();
          const addFarmersStep =
            await this.selfOnboardingService.addFarmersCurrentStep$
              .pipe(take(1))
              .toPromise();

          const guidedTourStep =
            await this.selfOnboardingService.guidedTourStep$
              .pipe(take(1))
              .toPromise();

          const companyOnboardingState = await this.companyControllerService
            .getCompanyOnboardingState(company.id)
            .toPromise();
          ...
          if (addProductStep === 'success') {
            ...
            const dismissedOnboardingWelcomeModal = sessionStorage.getItem(
              'dismissedOnboardingWelcomeModal',
            );

            if (
              companyOnboardingState.data?.hasCreatedProduct == false &&
              companyOnboardingState.data?.hasCreatedFacility == false &&
              companyOnboardingState.data?.hasCreatedProcessingAction ==
                false &&
