...
export class UserListComponent implements OnInit, OnDestroy {
  ...
  myUsers = new UntypedFormControl(true);
  ...
  searchParams$ = combineLatest(
    ...
    this.searchRole.valueChanges.pipe(startWith(null)),
    this.myUsers.valueChanges.pipe(startWith(true)),
    ...
    map((resp: ApiPaginatedResponseApiUserBase) => {
      if (resp) {
        if (
          ...
          this.showedUsers = resp.data.count;
        } else {
  ...
  async setAllUsers() {
    if (this.isRegionalAdmin) {
      const res = await this.userController
        .regionalAdminListUsers(null, null, null, null, null, 'COUNT')
        .pipe(take(1))
        .toPromise();
      ...
      const res = await this.userController
        .adminListUsers(null, null, null, null, null, 'COUNT')
        .pipe(take(1))
        .toPromise();
      if (res && res.status === 'OK' && res.data && res.data.count >= 0) {
        this.allUsers = res.data.count;
      ...
      const res = await this.userController
        .listUsers(null, null, null, null, null, 'COUNT')
        .pipe(take(1))
        .toPromise();
      if (res && res.status === 'OK' && res.data && res.data.count >= 0) {
        this.allUsers = res.data.count;
  ...
  get statusList() {
    ...
    obj['UNCONFIRMED'] =
      $localize`:@@userList.statusList.unconfirmed:Unconfirmed`;
    obj['CONFIRMED_EMAIL'] =
      $localize`:@@userList.statusList.comfirmedEmail:Comfirmed email`;
    obj['ACTIVE'] = $localize`:@@userList.statusList.active:Active`;
    obj['DEACTIVATED'] =
      $localize`:@@userList.statusList.deactiveted:Deactiveted`;
    return obj;
  }
  ...
  async deactivate(id) {
    try {
      this.globalEventsManager.showLoading(true);
      await this.userController
        .activateUser('DEACTIVATE_USER', { id })
        .pipe(take(1))
        .toPromise();
      this.reloadPage();
    } catch (e) {
  ...
  clearValue(form: UntypedFormControl, myUsers: boolean = false) {
    if (myUsers) {
      form.setValue(false);
    } else {
      form.setValue('');
  ...
  showStatus(value: string, userRole: boolean = false) {
    if (userRole) {
      this.searchRole.setValue(value);
    } else {
      this.searchStatus.setValue(value);
    }
  }
  ...
  showPagination() {
    return (
      (this.showedUsers - this.pageSize === 0 &&
        this.allUsers >= this.pageSize) ||
      this.page > 1
    );
  }
}
