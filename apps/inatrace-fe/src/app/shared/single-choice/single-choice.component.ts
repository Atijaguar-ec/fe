...
import {
  CodebookHelperService,
  PagedSearchResults,
} from 'src/interfaces/CodebookHelperService';
...
export class SingleChoiceComponent implements OnInit {
  ...
  private _formControl: UntypedFormControl = null;
  @Input() set formControlInput(value: UntypedFormControl) {
  ...
  get formControlInput(): UntypedFormControl {
  ...
  resubscribeCodebook() {
    if (this.formControlInput && this.value) {
      throw Error(
        'Pri uporabi codebookService lahko uporabiš le enega izmed inputov formControl ali value',
      );
    }

    if (this.formControlInput) {
      if (this.codebookService.isEnumFormControl()) {
        this.resubscribeSub = this.codebookService
          .enumOptions()
          .subscribe((options) => {
            if (options) {
              this.modelChoice = this.codebookService.enumValueToObject(
      ...
      this.modelChoiceSubscription =
        this.formControlInput.valueChanges.subscribe((val) => {
          if (this.codebookService.isEnumFormControl()) {
            let sub = this.codebookService
              .enumOptions()
              .subscribe((options) => {
                if (options) {
                  this.modelChoice = this.codebookService.enumValueToObject(
      ...
      if (this.codebookService.isEnumFormControl()) {
        this.selectedObjects$ = combineLatest(
          this.codebookService.enumOptions(),
        ...
        ).pipe(startWith(null), shareReplay());
      } else {
    ...
    this.autocompleteSelection$ = this.codebookService
      .autocompleteCandidates(
        this.typeahead$.pipe(debounceTime(200), distinctUntilChanged()),
      ...
      .pipe(
        ...
        map((lst) =>
          lst.filter(
            (x) => this.forbiddenElements && !this.forbiddenElements(x),
          ),
        ),
      );
    ...
    this.optionsSubscription = this.autocompleteSelection$.subscribe(
      (choices) => {
  ...
  resubscribeEnum() {
    this.allOptions = pripraviSifrant(
      this.enumChoices,
      this.forceAllInLowercase,
    );
    if (!this.formControlInput) {
    ...
    if (this.allowMultiple) {
      ...
      this.modelChoice = this.allOptions.find((x) => {
        if (this.forceAllInLowercase) {
          if (val == null) return false;
          return (
            x.value.toLocaleLowerCase() === (val as string).toLocaleLowerCase()
          );
        }
        return x.value === val;
      });
    ...
    this.modelChoiceSubscription = this.formControlInput.valueChanges.subscribe(
      (val) => {
        if (this.allowMultiple) {
          this.modelChoiceArr = this.allOptions.filter((x) =>
            val?.includes(x.value),
          );
        } else {
    ...
    this.optionsSubscription = this.typeahead$
      .pipe(debounceTime(200), distinctUntilChanged())
      .pipe(
        map((key) => {
          let lkey = key ? key.toLocaleLowerCase() : '';
          return this.allOptions.filter(
            (x) => String(x.value).toLocaleLowerCase().indexOf(lkey) >= 0,
          );
        }),
  ...
  resubscribeBoolean() {
    this.options = [this.booleanTrue, this.booleanFalse];
      ...
      if (this.enumChoices || this.isBooleanSelector()) {
        ...
        if (this.codebookService && this.codebookService.isEnumFormControl()) {
          this.formControlInput.setValue(
            this.codebookService.objectToEnumValue(event),
          );
        } else {
