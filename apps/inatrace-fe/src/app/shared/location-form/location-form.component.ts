...
export class LocationFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subs.push(
      this.globalEventsManager.loadedGoogleMapsEmitter.subscribe(
        (loaded) => {
          ...
          if (tmpVis != null) {
            this.form.get('publiclyVisible').setValue(tmpVis.toString());
          }
        },
        () => {},
      ),
    ...
    this.subs.push(
      this.form.get('address.country').valueChanges.subscribe(() => {
        // Honduras specifics
  ...
  enableValidationHonduras() {
    this.form
      .get('address.hondurasDepartment')
      .setValidators([Validators.required]);
    this.form.get('address.hondurasFarm').setValidators(null);
    this.form
      .get('address.hondurasMunicipality')
      .setValidators([Validators.required]);
    this.form
      .get('address.hondurasVillage')
      .setValidators([Validators.required]);
  }
