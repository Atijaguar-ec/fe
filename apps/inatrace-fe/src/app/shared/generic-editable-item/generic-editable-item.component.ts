...
export class GenericEditableItemComponent<T> implements OnInit {
  ...
  @Input()
  set formControlInput(value: UntypedFormControl) {
    if (!value || value.value == null) {
      throw Error(
        "It's possible that 'null' Editable component is not supported.",
      );
    }
  ...
  originalFormControl: UntypedFormControl = null;
  tmpFormGroup: UntypedFormGroup = null;
  ...
  save() {
    ...
    if (this.form.dirty) {
      // predpostavka: če generiramo prazen objekt, ga moramo vsaj malo dopolniti (ni defaultov)
  ...
  toggle(returnFocusElement) {
    if (this.listEditorManager && this.listEditorManagerPosition != null) {
      this.listEditorManager.toggle(
        this.listEditorManagerPosition,
        returnFocusElement,
      );
    }
  ...
  public updateCustomFormControl(key: string, value: any) {
    if (!this.tmpFormGroup) {
      throw Error(
        "Cannot update custom form control '" + key + "'. FormGroup null.",
      );
    }
  ...
  get contentObject() {
    if (this.formControlInput && this.rawObjectData) {
      throw Error(
        'FormControlInput and rawObjectData cannot be used together!',
      );
    }
