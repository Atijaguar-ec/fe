...
export class TagListComponent implements OnInit {
  ...
  onBlur() {
    setTimeout(() => {
      // če kliknemo kljukico pri dodajanju sinonimov, to poskrbi, da se textinput ne zapre. Če kliknemo ven se zapre
  ...
  resubscribe() {
    if (this.selectedObjectsSubscription)
      this.selectedObjectsSubscription.unsubscribe();
    this.selectedObjectsSubscription = this.formArray.valueChanges
      .pipe(startWith(this.rawFormArray()))
      .subscribe((val) => {
  ...
  typeaheadSearch = (text$: Observable<string>) => {
    ...
    this.noAutocompleteSelections$ = this.autocompleteSelection$.pipe(
      map(
        (choices: Array<any>) =>
          this.newTagInputField &&
          this.newTagInputField.nativeElement.value.length > 0 &&
          choices.length == 0,
      ),
    );
  ...
  newTagValid() {
    ...
    return this.codebookService.valid(
      this.modelChoice,
      this.newTagInputField.nativeElement.value,
    );
  }
  ...
  finishAddTag() {
    ...
    if (this.typeaheadEnabled()) {
      ...
      toAdd = this.codebookService.makeNewForInput(
        this.newTagInputField.nativeElement.value,
      );
    }
