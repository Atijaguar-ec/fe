...
export class CheckboxGroupMultiChoiceComponent implements OnInit, OnDestroy {
  _formArray: UntypedFormArray = null;
  ...
  resubscribe() {
    if (this.codebookService) {
      this.sub1 = this.choicesSub = this.codebookService
        .enumOptions()
        .subscribe((options) => {
              ...
              selected:
                this.useForm && this.form
                  ...
                  : !!this.formArray.value.find(
                      (y) =>
                        x.id ===
                        (this.codebookService.isEnumFormControl ? y : y.id),
                    ),
            };
          });
        });
    ...
    if (this.useForm && this.form) {
      this.sub2 = this.form.valueChanges.subscribe((newValue) => {
        ...
        this.checkboxes = this.checkboxes.map((el) => {
          el.selected = !!(this.codebookService.isEnumFormControl
            ? el.selection.id == newValue
            : el.selection.id == newValue.id);
          return el;
        });
      });
    } else {
  ...
  onKey(event, i, selection) {
    if (event.target.checked) {
      if (this.useForm && this.form) {
        this.form.setValue(
          this.codebookService.isEnumFormControl() ? selection.id : selection,
        );
      } else {
        let pos = this.formArray.value.findIndex(
          (x) =>
            (this.codebookService.isEnumFormControl() ? x : x.id) ===
            selection.id,
        );
        if (pos >= 0) return;
        let data = this.codebookService.isEnumFormControl()
          ? selection.id
          : selection;
        this.formArray.push(new UntypedFormControl(data));
      }
