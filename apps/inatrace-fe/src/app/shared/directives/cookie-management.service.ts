...
export class CookieManagementService {
  ...
  loadRocketChat() {
    ...
    (function (w: any, d, s, u) {
      ...
      j.async = true;
      j.src =
        environment.rocketChatServer +
        '/rocketchat-livechat.min.js?_=201903270000'; //+ Math.random(); //201903270000';
  ...
  hasDefinedConsentToAllCookies() {
    return Object.keys(this.allCookieKeys).every((key) =>
      this.isDefinedForKey(key),
    );
  }
