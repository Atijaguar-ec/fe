...
export class TinymceTextareaComponent implements OnInit {
  @Input()
  formControlInput: UntypedFormControl = null;
  ...
  init = {
    ...
    content_style:
      '@import url("https://fonts.googleapis.com/css?family=Signika");.mce-content-body {font-size:1rem;font-family:Signika,Helvetica Neue,sans-serif;color:#495057}',
  ...
  getValue() {
    return this.formControlInput ? this.formControlInput.value : '';
  }
  ...
  tooManyCharacters() {
    return (
      (this.getMaxLength() && this.getSteviloZnakov() > this.getMaxLength()) ||
      (this.getMinLength() && this.getSteviloZnakov() < this.getMinLength())
    );
  }
}
