...
export class ListEditorComponent implements OnInit {
  ...
  constructor(@Optional() @Host() public closable: ClosableComponent) {}
  ...
  ngOnInit() {
    if (this.closable && this.closable.mode === 'intelligent') {
      this.closable.openOnValueChange = (value: any[]) => {
  ...
  scrollToTarget(n = 10) {
    ...
    window.setTimeout(() => {
      if (this.targetElement) {
        // scrolanje novega podokna v vidno polje
  ...
  isDisabled() {
    return (
      this.disabled ||
      (this.listEditorManager && this.listEditorManager.isDisabled())
    );
  }
  addElement() {
    if (this.listEditorManager) {
  ...
  formattedMessageForHeader(i: number) {
    if (this.headerFormatter) {
      return this.headerFormatter(
        (this.formArray.controls[i] as UntypedFormGroup).getRawValue(),
      );
    }
  }
  ...
  contentObject(i: number): any {
    ...
    return (this.formArray.at(i) as UntypedFormGroup).getRawValue();
  }
