...
export class TextinputComponent implements OnInit {
  ...
  _form = null;
  @Input() set form(value: UntypedFormControl) {
    this._form = value;
    if (value) {
      this.resubscribe();
    }
  ...
  private _languageFormArray: UntypedFormArray;

  @Input() set languageFormArray(value: UntypedFormArray) {
  ...
  get languageFormArray(): UntypedFormArray {
  ...
  pushAutocompleteParameters() {
    if (this.autocompleteSearchAdditionalParams) {
      this.autocompleteSearchAdditionalParams$.next(
  ...
  ngOnInit() {
    ...
    if (this.typeaheadEnabled()) {
      this.pushAutocompleteParameters();
      if (this.pushToFormControlSubscription)
        this.pushToFormControlSubscription.unsubscribe();
  ...
  languageFormControl = new UntypedFormControl(null);
  lastLanguage = 'sl';
  ...
  resubscribe() {
    if (this.languageFormArray) {
      this.original = true;
      if (this.langAndTextSubscription)
        this.langAndTextSubscription.unsubscribe();
      this.langAndTextSubscription = combineLatest(
        ...
        .pipe(
          distinctUntilChanged(
            (prev, curr) =>
              prev.language === curr.language && prev.text === curr.text,
          ),
        )
        .subscribe((val) => {
          if (val.language) {
  ...
  setForLanguage(language: string, text: string) {
    ...
    let res: Array<any> = this.languageFormArray.controls.filter((x) => {
      return (x as UntypedFormControl).value.key == language;
    });
    if (res.length == 0) {
      if (text) {
      ...
      throw Error(
        'languageFormArray ne sme vsebovati večkratnih vnosov za jezik: ' +
          language,
      );
    } else {
      let fc = res[0] as UntypedFormControl;
      if (this.lastLanguage == language) {
        // zavedi spremembo samo, če ni bilo spremembe jezika
        fc.setValue({
          key: language,
          value: text,
        });
  ...
  clearEmptyLanguages() {
    ...
    for (let el of this.languageFormArray.controls) {
      let val = (el as UntypedFormControl).value.value;
      if (!val) {
  ...
  initializeLanguage(language: string) {
    ...
    if (res.length > 1) {
      throw Error(
        'večkratni prevodi v languageFormArray za jezik:  ' + this.language,
      );
    }
    if (res.length == 0) {
  ...
  typeaheadSearch = (text$: Observable<string>) => {
    ...
    this.autocompleteSelection$ = this.codebookService
      .autocompleteCandidates(
        merge(
          this.input$.pipe(debounceTime(200), distinctUntilChanged()),
          inputFocus$.pipe(
    ...
    this.noAutocompleteSelections$ = this.autocompleteSelection$.pipe(
      map(
        (choices: Array<any>) =>
          this.inputfield.first &&
          this.inputfield.first.nativeElement.value.length > 0 &&
          choices.length == 0,
      ),
    );
