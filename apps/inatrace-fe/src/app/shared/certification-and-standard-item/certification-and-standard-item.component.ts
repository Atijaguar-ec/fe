...
export class CertificationAndStandardItemComponent
  extends GenericEditableItemComponent<ApiCertification>
  implements OnInit
{
  constructor(
  ...
  static emptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      return new UntypedFormControl(
        CertificationAndStandardItemComponent.createEmptyObject(),
  ...
  onDownload() {
    ...
    if (apiDoc && apiDoc.storageKey) {
      const sub = this.commonController
        .getDocument(apiDoc.storageKey)
        .subscribe((res) => {
