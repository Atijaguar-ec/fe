...
export class DatepickerComponent implements OnInit, OnDestroy {
  ...
  resubscribe() {
    if (this.form) {
      ...
      this.valueSubscription = this.form.valueChanges.subscribe((v) =>
  ...
  isDisabled() {
    ...
    if (this.form && this.form.enabled === false) {
      return true;
    }
    return false;
  }
  ...
  updateDisplay(value, cleanToNull = true) {
    if (value === this.lastValue && cleanToNull === this.lastCleanToNull) {
      return;
    } // če ni tega guarda se na safariju zacikla
    ...
    if (value) {
      ...
      if (cleanToNull) {
        // glitch - če hočeš, da se zbriše še input, moraš najprej vsa polja nastaviti na null, potem pa še enkrat celoto na null (****)
  ...
  change(event) {
    ...
    if (event != null && typeof event !== 'string') {
      ...
      if (!this.isValidDate(datumOceneDate)) {
        // če datum ni pravi, kar zapri datepicker (samo tipkanje)
      ...
      // delno napisano -> zapri datepicker
      if (this.datePicker.isOpen) {
        this.datePicker.close();
      }
    }
