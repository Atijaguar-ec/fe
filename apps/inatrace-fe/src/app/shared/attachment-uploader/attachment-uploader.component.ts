...
class AttachmentFileUploader extends FileUploader {
  uploadResponse: EventEmitter<UploadResponse> =
    new EventEmitter<UploadResponse>();
  ...
  onCompleteItem(
    ...
    this.uploadResponse.emit(
      new UploadResponse(item, status, response, headers),
    );
  }
}
...
export class AttachmentUploaderComponent implements OnInit {
  ...
  _form: UntypedFormControl | UntypedFormGroup = null;

  @Input() set form(value: UntypedFormControl | UntypedFormGroup) {
    this._form = value;
  ...
  get form(): UntypedFormControl | UntypedFormGroup {
    return this._form;
  }
  ...
  @Input()
  allowedMimeTypeErrorMessage: string =
    $localize`:@@ui.attachmentUploader.allowedMimeTypeErrorMessageDefault:Upload only file types: PDF, PNG and JPG.`;
  ...
  resubscribe() {
    ...
    this.metadata$ = merge(this.metadataGET$, this.metadataPOST$).pipe(
      shareReplay(),
    );
    ...
    this.finalSubscription = this.metadata$.subscribe((val) => {
      // ta subscription je obvezen, če ne se nič ne sproži
  ...
  ngOnInit() {
    ...
    let config = {
      url: this.rootImageUrl
        ? this.rootImageUrl
        : this.type
          ? this.rootUrlManualType + this.type
          : this.rootUrl,
      autoUpload: true,
    ...
    this.uploaderSubscription = this.uploader.uploadResponse.subscribe(
      (ev: UploadResponse) => {
        if (ev.successful()) {
  ...
  isImage() {
    return (
      this.contentType === 'image/jpeg' || this.contentType === 'image/png'
    );
  }
  ...
  deleteCurrent() {
    ...
    this.httpClient
      .delete(this.deleteRouteGenerator(this.identifier()), {
        observe: 'response',
        responseType: 'text',
      })
      .subscribe((resp) => {
  ...
  async showContentModal() {
    ...
    if (this.isImage()) {
      ...
      if (this.mode === 'simpleAsTextField' && this.fileInfo) {
        let res = await this.fileService
          .getDocument(this.fileInfo.storageKey)
          .pipe(take(1))
          .toPromise();
        if (res) {
  ...
  onDownload() {
    ...
    if (this.rootImageUrl) {
      let subImg = this.fileService
        .getImage(this.fileInfo.storageKey)
        .subscribe((res) => {
          this.fileSaverService.save(res, this.fileInfo.name);
          subImg.unsubscribe();
        });
    } else {
      let sub = this.fileService
        .getDocument(this.fileInfo.storageKey)
        .subscribe((res) => {
          this.fileSaverService.save(res, this.fileInfo.name);
          sub.unsubscribe();
