...
export class NgbModalImproved {
  ...
  protected get ngbModal(): NgbModal {
    if (this._ngbModal == null) {
      ...
      this.location.onPopState((ev) => {
        this._ngbModal.dismissAll('back_button');
      });
      this.location.onHashChange((ev) => {
        this._ngbModal.dismissAll('location_change');
      });
    }
  ...
  public open(
    content: any,
    options?: NgbModalImprovedOptions,
    inputs?: any,
  ): NgbModalRef {
    let opts = options || {};
