...
export class SelectedUserCompanyService {
  private readonly selectedCompanyProfileSubject =
    new ReplaySubject<ApiCompanyGet>(1);
  ...
  constructor(
    ...
    this.authService.userProfile$.subscribe((userProfile) =>
      this.selectUserCompany(userProfile),
    );
  }
  ...
  private async selectUserCompany(user: ApiUserGet): Promise<void> {
    if (user) {
      ...
      let currentlySelectedCompany = localStorage.getItem(
        'selectedUserCompany',
      );
      if (
        user.companyIds == null ||
        user.companyIds.length === 0 ||
        !user.companyIds.includes(Number(currentlySelectedCompany))
      ) {
      ...
      if (currentlySelectedCompany == null) {
        if (user.companyIds && user.companyIds.length === 1) {
          const companyProfile = await this.fetchCompanyProfile(
            user.companyIds[0],
          );
          if (companyProfile) {
        ...
        const companyProfile = await this.fetchCompanyProfile(
          Number(currentlySelectedCompany),
        );
        if (companyProfile) {
          this.setSelectedCompany(companyProfile);
        }
  ...
  private async listMyCompanies(userProfile: ApiUserGet): Promise<void> {
    ...
    const codebookMyCompanies = new ActiveCompaniesService(
