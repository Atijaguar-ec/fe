...
export class RedirectClearCacheComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (
      Array.isArray(this.router.url.match(stockUrlTempl)) &&
      '/' + this.router.url.match(stockUrlTempl)[0] == this.router.url
    ) {
      ...
      Array.isArray(this.router.url.match(stakeholdersUrlTempl)) &&
      '/' + this.router.url.match(stakeholdersUrlTempl)[0] == this.router.url
    ) {
      localStorage.setItem('token', 'clear-cache');
      let urlArray = this.router.url.split('/');
      urlArray[0] = '/';
