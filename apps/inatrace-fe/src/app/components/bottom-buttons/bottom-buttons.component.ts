...
export class BottomButtonsComponent implements OnInit {
  ...
  @Input()
  floatingValidateTitle =
    $localize`:@@bottomButoms.button.floatingValidate:Validation check`;
  ...
  onSubmitMessage: string =
    $localize`:@@bottomButoms.message.onSubmit:Do you want to submit an application?`;
  ...
  isValidate() {
    return (
      this.onValidate.observers.length > 0 &&
      ((this.form && this.form.enabled === true) || !this.form)
    );
  }
  ...
  isSubmit() {
    return (
      this.onSubmit.observers.length > 0 &&
      ((this.form && this.form.enabled === true) || !this.form)
    );
  }
  ...
  submitEnabled() {
    return (
      !this.form ||
      (this.form &&
        (this.ignoreOnSubmitDirty || !this.form.dirty) &&
  ...
  async submit() {
    ...
    if (
      ((this.form && this.form.dirty) || (!this.form && this.changed)) &&
      !this.ignoreOnSubmitDirty
    ) {
      this.globalEventsManager.push({
        action: 'error',
        notificationType: 'warning',
      ...
      (this.form && this.form.invalid) ||
      (!this.form && this.invalid)
    ) {
      this.globalEventsManager.push({
        action: 'error',
        notificationType: 'error',
  ...
  isSave() {
    return (
      this.onSave.observers.length > 0 &&
      ((this.form && this.form.enabled === true) || !this.form)
    );
  }
  ...
  isDownload() {
    return (
      this.downloadButton &&
      ((this.form && this.form.enabled === false) || !this.form)
    );
  }
