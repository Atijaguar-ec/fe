...
export class BatchDetailPageComponent implements OnInit, OnDestroy {
  ...
  goToLink: string = this.router.url.substr(
    0,
  ...
  batchDetailForm: UntypedFormGroup;
  ...
  newBatch() {
    this.batchDetailForm = generateFormFromMetadata(
      ApiProductLabelBatch.formMetadata(),
      {},
      ApiProductLabelBatchValidationScheme,
    );
  }
  ...
  getBatch(): void {
    ...
    let sub = this.productController.getProductLabelBatch(id).subscribe(
      (batch) => {
  ...
  async saveBatch(goBack = true) {
    ...
    let res = await this.productController
      .updateProductLabelBatch({ ...params, ...this.batchDetailForm.value })
      .pipe(take(1))
      .toPromise();
  ...
  async createBatch(goBack = true) {
    ...
    this.prepareRequest();
    let res = await this.productController
      .createProductLabelBatch(this.batchDetailForm.value)
      .pipe(take(1))
      .toPromise();
    if (res && res.status == 'OK' && goBack) {
      this.batchDetailForm.markAsPristine();
  ...
  get origCheck() {
    return (
      this.batchDetailForm.get('traceOrigin').value == null ||
      !this.batchDetailForm.get('traceOrigin').value ||
      (this.batchDetailForm.get('traceOrigin').value &&
        this.batchDetailForm.get('locations').value.length > 0)
    );
  }
  ...
  prepareRequest() {
    ...
    if (pd != null)
      this.batchDetailForm.get('productionDate').setValue(dateISOString(pd));
    let ed = this.batchDetailForm.get('expiryDate').value;
    if (ed != null)
      this.batchDetailForm.get('expiryDate').setValue(dateISOString(ed));
    if (this.mode == 'create')
      this.batchDetailForm
        .get('labelId')
        .setValue(Number(this.route.snapshot.params.labelId));
  }
  ...
  get originLocations(): UntypedFormArray {
    return this.batchDetailForm.get('locations') as UntypedFormArray;
  }
  ...
  initializeOriginLocations(locs): void {
    let tmp = new UntypedFormArray([]);
    for (let loc of locs) {
      tmp.push(this.createOrFillOriginLocationsItem(loc, false));
    }
    (this.batchDetailForm as UntypedFormGroup).setControl('locations', tmp);
