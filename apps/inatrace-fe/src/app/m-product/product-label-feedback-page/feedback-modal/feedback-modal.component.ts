...
export class FeedbackModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.labelId) {
      this.feedbackForm = generateFormFromMetadata(
        ApiProductLabelFeedback.formMetadata(),
        defaultEmptyObject(ApiProductLabelFeedback.formMetadata()),
        ApiProductLabelFeedbackValidationScheme,
      );
      const questionnaireAnswersForm = generateFormFromMetadata(
        questionnaireAnswersFormMetadata(),
        {},
        questionnaireAnswersValidationScheme,
      );
      ...
      const questionnaireAnswersForm = generateFormFromMetadata(
        questionnaireAnswersFormMetadata(),
        this.feedback.questionnaireAnswers,
        questionnaireAnswersValidationScheme,
      );
      this.feedbackForm.setControl(
        'questionnaireAnswers',
  ...
  async submitFeedback() {
    ...
    try {
      ...
      const res = await this.publicController
        .addProductLabelFeedback(this.labelId, data)
        .pipe(take(1))
        .toPromise();
  ...
  get statusList() {
    const obj = {};
    obj['PRAISE'] =
      $localize`:@@productLabelFrontFeedback.statusList.registred:Praise`;
    obj['PROPOSAL'] =
      $localize`:@@productLabelFrontFeedback.statusList.active:Proposal`;
    obj['COMPLAINT'] =
      $localize`:@@productLabelFrontFeedback.statusList.deactivated:Complaint`;
    return obj;
  }
