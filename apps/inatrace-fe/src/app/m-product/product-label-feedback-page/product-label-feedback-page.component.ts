...
export class ProductLabelFeedbackPageComponent implements OnInit {
  ...
  async setAllFeedbacks() {
    const res = await this.publicController
      .listProductLabelFeedbacks(this.labelId, null, 'COUNT')
      .pipe(take(1))
      .toPromise();
  ...
  get statusList() {
    ...
    obj['PRAISE'] =
      $localize`:@@productLabelFeedback.statusList.registred:Praise`;
    obj['PROPOSAL'] =
      $localize`:@@productLabelFeedback.statusList.active:Proposal`;
    obj['COMPLAINT'] =
      $localize`:@@productLabelFeedback.statusList.deactivated:Complaint`;
    return obj;
  }
  ...
  async deleteFeedback(feedback) {
    ...
    let res = await this.productController
      .deleteProductLabelFeedback(feedback.id)
      .pipe(take(1))
      .toPromise();
  ...
  showPagination() {
    return (
      (this.showedFeedbacks - this.pageSize === 0 &&
        this.allFeedbacks >= this.pageSize) ||
      this.page > 1
    );
  }
  ...
  async downloadFeedbacks() {
    let res = await this.publicController
      .listProductLabelFeedbacks(this.labelId, null, null, null, null, 'DESC')
      .pipe(take(1))
      .toPromise();
