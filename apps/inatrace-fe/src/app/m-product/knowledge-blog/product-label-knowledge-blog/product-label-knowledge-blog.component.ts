...
export class ProductLabelKnowledgeBlogComponent implements OnInit {
  ...
  async setAllBlogs() {
    let res = await this.productController
      .getProductKnowledgeBlogs(this.productId, null, 'COUNT')
      .pipe(take(1))
      .toPromise();
  ...
  get statusList() {
    ...
    obj['FAIRNESS'] =
      $localize`:@@productLabelKnowledgeBlog.type.fairness:Fairness`;
    obj['QUALITY'] =
      $localize`:@@productLabelKnowledgeBlog.type.quality:Quality`;
    obj['PROVENANCE'] =
      $localize`:@@productLabelKnowledgeBlog.type.provenance:Provenance`;
    return obj;
  }
  ...
  blogs$ = combineLatest(
    ...
    switchMap((params) => {
      return this.productController.getProductKnowledgeBlogsByMap({
    ...
    map((resp: ApiPaginatedResponseApiKnowledgeBlogBase) => {
      if (resp) {
        if (resp.data && resp.data.count && this.pageSize - resp.data.count > 0)
          this.showedBlogs = resp.data.count;
        else {
  ...
  toc = [this.fairnessSection, this.qualitySection, this.provenanceSection];
  ...
  showPagination() {
    if (
      (this.showedBlogs - this.pageSize == 0 &&
        this.allBlogs >= this.pageSize) ||
      this.page > 1
    )
      return true;
    else return false;
