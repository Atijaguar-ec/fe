...
export class KnowledgeBlogPartComponent
  ...
  implements OnInit
{
  ...
  async initializeData() {
    ...
    if (this.type === 'fairness') {
      this.currentType = 'FAIRNESS';
    }
    if (this.type === 'quality') {
      this.currentType = 'QUALITY';
    }
    if (this.type === 'provenance') {
      this.currentType = 'PROVENANCE';
    }

    if (this.knowledgeBlogId) {
      this.showPage = true;
      let resp = await this.productController
        .getProductKnowledgeBlog(this.knowledgeBlogId)
        .pipe(take(1))
        .toPromise();
      let knowledgeBlogData = resp.data;
      this.form = generateFormFromMetadata(
        ApiKnowledgeBlogPart.formMetadata(),
        knowledgeBlogData,
      );
    } else {
      this.form = generateFormFromMetadata(
  ...
  titleName(type: string) {
    if (this.knowledgeBlogId)
      this.title += $localize`:@@knowledgeBlogPart.title.edit:Edit `;
    else this.title += $localize`:@@knowledgeBlogPart.title.edit:Add `;

    if (this.type === 'fairness')
      this.title += $localize`:@@knowledgeBlogPart.title.fairness:Fairness`;
    if (this.type === 'quality')
      this.title += $localize`:@@knowledgeBlogPart.title.quality:Quality`;
    if (this.type === 'provenance')
      this.title += $localize`:@@knowledgeBlogPart.title.provenance:Provenance`;
  }

  initializeListManagers() {
    this.listManager = new ListEditorManager<ApiDocument>(
      this.form.get('documents') as UntypedFormArray,
      KnowledgeBlogPartComponent.EmptyObjectFormFactory(),
      null,
    );
  }
  ...
  static EmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      let f = new UntypedFormControl(
        KnowledgeBlogPartComponent.CreateEmptyObject(),
      );
      return f;
    };
  }
  ...
  async save() {
    ...
    if (this.knowledgeBlogId) {
      try {
        ...
        data = {
          type: this.currentType as ApiKnowledgeBlog.TypeEnum,
          ...data,
          id: this.knowledgeBlogId,
        };
        let res = await this.productController
          .updateProductKnowledgeBlog(data)
          .pipe(take(1))
          .toPromise();
      ...
      try {
        ...
        let res = await this.productController
          .addProductKnowledgeBlog(this.productId, data)
          .pipe(take(1))
          .toPromise();
        if (res && res.status === 'OK') {
          this.form.markAsPristine();
