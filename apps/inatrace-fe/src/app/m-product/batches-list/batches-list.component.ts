...
export class BatchesListComponent implements OnInit {
  ...
  batches$ = combineLatest(
    ...
    switchMap((params) => {
      let labelId = this.route.snapshot.params.labelId;
      return this.productController.getProductLabelBatchesByMap({
        ...params,
        id: labelId,
      });
    }),
    map((resp: ApiPaginatedResponseApiProductLabelBatch) => {
      if (resp) {
        if (resp.data && resp.data.count && this.pageSize - resp.data.count > 0)
          this.showedBatches = resp.data.count;
        else {
  ...
  createBatch() {
    ...
    let labelId = this.route.snapshot.params.labelId;
    let r =
      'product-labels/' + productId + '/labels/' + labelId + '/batches/new';
  ...
  async deleteBatch(batch) {
    ...
    if (result != 'ok') return;
    let res = await this.productController
      .deleteProductLabelBatchByMap(batch)
      .pipe(take(1))
      .toPromise();
  ...
  showPagination() {
    if (
      (this.showedBatches - this.pageSize == 0 &&
        this.allBatches >= this.pageSize) ||
      this.page > 1
    )
      return true;
    else return false;
