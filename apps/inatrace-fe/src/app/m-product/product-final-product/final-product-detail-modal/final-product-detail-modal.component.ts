...
export class FinalProductDetailModalComponent implements OnInit {
  ...
  productLabelForm = new UntypedFormControl(null);
  ...
  ngOnInit(): void {
    this.productLabelsCodebook = new ProductLabelsService(
  ...
  async init() {
    ...
    if (this.isUpdate) {
      const resp = await this.productControllerService
        .getFinalProduct(this.productId, this.finalProductId)
        .pipe(take(1))
        .toPromise();
  ...
  async addSelectedProductLabel(productLabel: ApiProductLabelBase) {
    if (
      !productLabel ||
      this.productLabels.some((pl) => pl.id === productLabel.id)
    ) {
  ...
  async save() {
    ...
    const res = await this.productControllerService
      .createOrUpdateFinalProduct(this.productId, finalProduct)
      .pipe(take(1))
      .toPromise();
