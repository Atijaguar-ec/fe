...
export class ProductListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  myProducts = new UntypedFormControl(true);
  ...
  searchParams$ = combineLatest(
    ...
    this.myProducts.valueChanges.pipe(startWith(true)),
    ...
    map((resp: ApiPaginatedResponseApiProductListResponse) => {
      if (resp) {
        if (
          ...
          this.showedProducts = resp.data.count;
        } else {
  ...
  async createProductForCompany() {
    const currentStep = await this.selfOnboardingService.addProductCurrentStep$
      .pipe(take(1))
      .toPromise();
    if (currentStep === 2) {
    ...
    if (company && valueChain) {
      this.router
        .navigate(['product-labels', 'new', company.id, valueChain.id])
        .then();
    }
  }
  ...
  clearValue(form: UntypedFormControl, myProducts: boolean = false) {
    if (myProducts) {
      form.setValue(false);
    } else {
      form.setValue('');
  ...
  showPagination() {
    return (
      (this.showedProducts - this.pageSize === 0 &&
        this.allProducts >= this.pageSize) ||
      this.page > 1
    );
  }
}
