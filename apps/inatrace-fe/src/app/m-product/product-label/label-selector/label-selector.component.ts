...
export class LabelSelectorComponent implements OnInit {
  ...
  @Input()
  currentLabelForm: UntypedFormControl;
  ...
  isSelected(item) {
    ...
    return (
      ...
      this.selected.label &&
      item &&
      this.selected.label.id === item.id
    );
  }
  ...
  showInView(event) {
    ...
    if (
      card.x < container.x ||
      card.x + card.width > container.x + container.width
    ) {
      window.setTimeout(() => {
    ...
    if (this.toSelect) {
      this.selectSub = this.toSelect.subscribe((val: any) => {
        ...
        if (val.position != null) {
          // negative values mean -(index + 1) of deleted position
          let prev = val.position - 1;
          prev = prev < 0 ? 0 : prev;
          if (prev < this.labels.length) {
            // valid index
