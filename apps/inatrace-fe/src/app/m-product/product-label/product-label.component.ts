  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  get originLocations(): UntypedFormArray {
    return this.productForm.get('origin.locations') as UntypedFormArray;
  }

  get isGoogleMapsLoaded() {
    // fix of a Google Maps glitch
    return !!window.google;
  }

  get labelChanged() {
    return (
      (this.visibilityForm && this.visibilityForm.dirty) ||
      this.labelTitleForm.dirty
    );
  }
  ...
  get invalid() {
    return (
      ...
      (this.visibilityForm && this.visibilityForm.invalid) ||
      (this.mediaForm && this.mediaForm.invalid)
    );
  }
  get publishText() {
    if (!this.currentLabel) {
    ...
    if (this.currentLabel.status === ApiProductLabel.StatusEnum.PUBLISHED) {
      return this.unpublishString;
    }
    if (this.currentLabel.status === ApiProductLabel.StatusEnum.UNPUBLISHED) {
      return this.publishString;
    }
    return this.publishString;
  }
  ...
  product$ = combineLatest(
    ...
    switchMap((id) =>
      this.productController.getProduct(id).pipe(catchError(() => of(null))),
    ),
    filter((resp) => !!resp),
    map((resp) => {
      return resp.data;
    ...
    tap((data: ApiProduct) => {
      ...
      companyFormMediaLinks.setValue({
      ...
      const valueChainForm = generateFormFromMetadata(
        ApiValueChain.formMetadata(),
        product.valueChain,
        ApiValueChainValidationScheme,
      );
  ...
  visibilityForm: UntypedFormGroup = null;
  ...
  labelTitleForm = new UntypedFormControl(null);
  editTitleMode = false;
    ...
    this.unsubscribeList.add(
      this.authService.userProfile$
        ...
        .subscribe((cp) => {
          ...
          if (this.mode === 'update') {
            this.unsubscribeList.add(this.product$.subscribe(() => {}));
            this.unsubscribeList.add(
  ...
  ngAfterViewInit() {
    // ViewChildren for templates works only after ngAfterViewInit
    setTimeout(() => {
  ...
  loadLabel(label) {
    ...
    if (this.initialReload) {
      ...
      this.labelSelect$.next({
        id: this.redirectToCertainLabel,
        preventEmit: true,
      });
  ...
  async newProduct() {
    ...
    try {
      this.globalEventsManager.showLoading(true);
      let resp = await this.companyController
        .getCompany(Number(companyId))
        .pipe(take(1))
        .toPromise();
      const company = resp.data;
      const companyForm = generateFormFromMetadata(
        ApiCompany.formMetadata(),
        company,
        ApiCompanyValidationScheme,
      );
      this.productForm.setControl('company', companyForm);
      resp = await this.valueChainController
        .getValueChain(Number(valueChainId))
        .toPromise();
  ...
  async saveCurrentLabel(reload = false) {
    ...
    if (res && res.status === 'OK') {
      ...
      const labelDocuments: ApiProductLabelCompanyDocument[] = this.mediaForm
        .get('video')
        .value.concat(
          this.mediaForm.get('farmers').value,
          this.mediaForm.get('productionRecord').value,
        );
      const resLD = await this.productController
        .updateCompanyDocumentsForProductLabel(
  ...
  async create() {
    ...
    try {
      ...
      const data = this.productForm.value;
      delete data['id'];
      const res: ApiResponseApiBaseEntity = await this.productController
        .createProduct(data)
        .pipe(take(1))
        .toPromise();
      if (res && res.status === 'OK') {
        ...
        const currentStep =
          await this.selfOnboardingService.addProductCurrentStep$
            .pipe(take(1))
            .toPromise();
        if (currentStep == 4) {
  ...
  createOrFillOriginLocationsItem(loc: any, create: boolean): UntypedFormGroup {
    return new UntypedFormGroup({
      latitude: new UntypedFormControl(create ? loc.lat : loc.latitude),
      longitude: new UntypedFormControl(create ? loc.lng : loc.longitude),
      pinName: new UntypedFormControl(create ? null : loc.pinName),
      numberOfFarmers: new UntypedFormControl(
        create ? null : loc.numberOfFarmers,
      ),
    });
  }

  initializeOriginLocations(): void {
    const tmp = new UntypedFormArray([]);
    for (const loc of this.originLocations.value) {
      tmp.push(this.createOrFillOriginLocationsItem(loc, false));
    }
    (this.productForm.get('origin') as UntypedFormGroup).setControl(
      'locations',
      tmp,
    );
    this.productForm.updateValueAndValidity();
  ...
  addOriginLocations(loc) {
    ...
    this.markers.push(tmp);
    this.originLocations.push(
      this.createOrFillOriginLocationsItem(loc, true) as UntypedFormGroup,
    );
  ...
  generateProductElements() {
    this.productElements = [
      ...
      },
      {
        ...
        section: 'product',
        visible: new UntypedFormControl(true),
        template: this.productDescriptionTmpl,
        disableDrag: true,
      },
      {
        name: 'origin',
  ...
  generateSocialResponsibilityElements() {
    this.socialResponsibilityElements = [
      {
        name: 'responsibility.laborPolicies',
        section: 'responsibility',
        visible: new UntypedFormControl(false),
        template: this.laborPoliciesTmpl,
      },
    ];
  }
  ...
  generateEnvironmentalSustainabilityElements() {
    this.environmentalSustainabilityElements = [
      {
        name: 'sustainability.production',
        section: 'sustainability',
        visible: new UntypedFormControl(false),
        template: this.environmentalyFriendlyProductionTmpl,
      },
      {
        name: 'sustainability.packaging',
        section: 'sustainability',
        visible: new UntypedFormControl(false),
        template: this.sustainablePackagingTmpl,
      },
      {
        name: 'sustainability.co2Footprint',
        section: 'sustainability',
        visible: new UntypedFormControl(false),
        template: this.co2FootprintTmpl,
      },
    ];
  }
    ...
    this.companyElements = [
      {
        ...
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.companyNameTmpl,
      },
      {
        name: 'company.logo',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.companyLogoTmpl,
      },
      {
        name: 'company.headquarters',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.companyHeadquartersTmpl,
      },
      {
        name: 'company.about',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.aboutTheCompanyTmpl,
      },
      {
        name: 'company.manager',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.nameOfManagerCEOTmpl,
      },
      {
        name: 'company.email',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.contactEmailTmpl,
      },
      {
        name: 'company.phone',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.contactPhoneNumberTmpl,
      },
      {
        name: 'company.webPage',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.companyWebPageTmpl,
      },
      {
        name: 'company.mediaLinks.facebook',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.facebookTmpl,
      },
      {
        name: 'company.mediaLinks.instagram',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.instagramTmpl,
      },
      {
        name: 'company.mediaLinks.twitter',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.twitterTmpl,
      },
      {
        name: 'company.mediaLinks.youtube',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.youtubeTmpl,
      },
      {
        name: 'company.mediaLinks.other',
        section: 'company',
        visible: new UntypedFormControl(false),
        template: this.otherTmpl,
      },
    ];
  }
      ...
      this.settingsElements = [
        {
          ...
          section: 'settings',
          visible: new UntypedFormControl(false),
          template: this.languageTmpl,
        },
        {
          name: 'settings.gdprText',
          section: 'settings',
          visible: new UntypedFormControl(false),
          template: this.gdprTextTmpl,
        },
        {
          name: 'settings.privacyPolicyText',
          section: 'settings',
          visible: new UntypedFormControl(false),
          template: this.privacyPolicyTextTmpl,
        },
        {
          name: 'settings.termsOfUseText',
          section: 'settings',
          visible: new UntypedFormControl(false),
          template: this.termsOfUseTextTmpl,
        },
      ];
      return;
    }
  ...
  generateBusinessToCustomerSettingsElements() {
    this.b2cElements = [
      {
        name: 'businessToCustomerSettings.primaryColor',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cPrimaryColor,
      },
      {
        name: 'businessToCustomerSettings.secondaryColor',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cSecondaryColor,
      },
      {
        name: 'businessToCustomerSettings.tertiaryColor',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cTertiaryColor,
      },
      {
        name: 'businessToCustomerSettings.quaternaryColor',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cQuaternaryColor,
      },
      {
        name: 'businessToCustomerSettings.productTitleColor',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cProductTitleColor,
      },
      {
        name: 'businessToCustomerSettings.headingColor',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cHeadingColor,
      },
      {
        name: 'businessToCustomerSettings.textColor',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cTextColor,
      },
      {
        name: 'businessToCustomerSettings.tabFairPrices',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cTabFairPrices,
      },
      {
        name: 'businessToCustomerSettings.tabProducers',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cTabProducers,
      },
      {
        name: 'businessToCustomerSettings.tabQuality',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cTabQuality,
      },
      {
        name: 'businessToCustomerSettings.tabFeedback',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cTabFeedback,
      },
      {
        name: 'businessToCustomerSettings.productFont',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cProductFont,
      },
      {
        name: 'businessToCustomerSettings.textFont',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cTextFont,
      },
      {
        name: 'businessToCustomerSettings.landingPageImage',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cLandingPageImage,
      },
      {
        name: 'businessToCustomerSettings.landingPageBackgroundImage',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cLandingPageBackgroundImage,
      },
      {
        name: 'businessToCustomerSettings.headerBackgroundImage',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cHeaderBackgroundImage,
      },
      {
        name: 'businessToCustomerSettings.graphicPriceToProducer',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cPricePaidToProducer,
      },
      {
        name: 'businessToCustomerSettings.graphicFarmGatePrice',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cFarmGatePrice,
      },
      {
        name: 'businessToCustomerSettings.prices',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cPrices,
      },
      {
        name: 'businessToCustomerSettings.graphicQuality',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cGraphicQuality,
      },
    ];
    if (this.action === 'labels') {
      this.b2cElements.push({
        name: 'businessToCustomerSettings.media',
        section: 'businessToCustomerSettings',
        visible: new UntypedFormControl(false),
        template: this.b2cMedia,
      });
    }
  }
  ...
  generateJointVisibilityForm() {
    ...
    this.visibilityForm = new UntypedFormGroup(formObj);
  }
  ...
  generateLabelMaps() {
    ...
    this.sectionToNameToObj.set(
      'sustainability',
      environmentalSustainabilityMap,
    );
  ...
  currentLabelFields() {
    const labels = [];
    const allSocialResponsibilityElements =
      this.prepareSocialResponsibilityElements();
    const allProductElements = this.prepareProductElements();
    const allList = [
      allProductElements,
      this.processElements,
      allSocialResponsibilityElements,
      this.environmentalSustainabilityElements,
      this.settingsElements,
      this.companyElements,
    ];
  ...
  get videoList(): AbstractControl[] {
    return this.mediaForm && this.mediaForm.get('video')
      ? (this.mediaForm.get('video') as UntypedFormArray).controls
      : [];
  }

  get farmerList(): AbstractControl[] {
    return this.mediaForm && this.mediaForm.get('farmers')
      ? (this.mediaForm.get('farmers') as UntypedFormArray).controls
      : [];
  }

  get productionRecordList(): AbstractControl[] {
    return this.mediaForm && this.mediaForm.get('productionRecord')
      ? (this.mediaForm.get('productionRecord') as UntypedFormArray).controls
      : [];
  }
  ...
  async initializeByLabel(label: ApiProductLabel) {
    const media = await this.productController
      .getCompanyDocumentsForProductLabel(label.id)
      .pipe(take(1))
      .toPromise();
    if (media && media.status === 'OK' && media.data) {
      this.availableMedia = media.data;

      this.mediaForm.setControl(
        'video',
        new UntypedFormArray([], maxActiveArrayControls(1)),
      );
      this.mediaForm.setControl('farmers', new UntypedFormArray([]));
      this.mediaForm.setControl('productionRecord', new UntypedFormArray([]));

      this.availableMedia.map((value) => {
        switch (value.category) {
          case ApiProductLabelCompanyDocument.CategoryEnum.VIDEO:
            (this.mediaForm.get('video') as UntypedFormArray).push(
              generateFormFromMetadata(
                ApiProductLabelCompanyDocument.formMetadata(),
                value,
              ),
            );
            break;
          case ApiProductLabelCompanyDocument.CategoryEnum.MEETTHEFARMER:
            (this.mediaForm.get('farmers') as UntypedFormArray).push(
              generateFormFromMetadata(
                ApiProductLabelCompanyDocument.formMetadata(),
                value,
              ),
          ...
          case ApiProductLabelCompanyDocument.CategoryEnum.PRODUCTIONRECORD:
            (this.mediaForm.get('productionRecord') as UntypedFormArray).push(
              generateFormFromMetadata(
                ApiProductLabelCompanyDocument.formMetadata(),
                value,
              ),
    ...
    const res = await this.productController
      .getProductLabelContent(label.id)
      .pipe(take(1))
      .toPromise();
    ...
    label.fields.forEach((field) => {
      if (field.name === 'settings.incomeIncreaseDescription') {
        increaseVisible = field.visible;
      }
      newFieldMap.set(field.name, field);
      sortOrderMap.set(field.name, i);
      i++;
    });
    this.productElements.sort((a, b) =>
      sortOrderMap.get(a.name) < sortOrderMap.get(b.name) ? -1 : 1,
    );
    this.processElements.sort((a, b) =>
      sortOrderMap.get(a.name) < sortOrderMap.get(b.name) ? -1 : 1,
    );
    this.socialResponsibilityElements.sort((a, b) =>
      sortOrderMap.get(a.name) < sortOrderMap.get(b.name) ? -1 : 1,
    );
    this.environmentalSustainabilityElements.sort((a, b) =>
      sortOrderMap.get(a.name) < sortOrderMap.get(b.name) ? -1 : 1,
    );
    this.settingsElements.sort((a, b) =>
      sortOrderMap.get(a.name) < sortOrderMap.get(b.name) ? -1 : 1,
    );
    this.companyElements.sort((a, b) =>
      sortOrderMap.get(a.name) < sortOrderMap.get(b.name) ? -1 : 1,
    );
    this.visibilityMap = new Map();
    allList.forEach((el) => {
      // set visibility forms
      ...
      if (el.name === 'settings.increaseIncome') {
        el.visible.setValue(increaseVisible);
      }
  ...
  async togglePublish() {
    ...
    if (!this.currentLabel || !this.currentLabel.status) {
      return;
    }
    const toBePublished =
      this.currentLabel.status === ApiProductLabel.StatusEnum.UNPUBLISHED;
    const action = toBePublished ? 'PUBLISH_LABEL' : 'UNPUBLISH_LABEL';
    const res = await this.productController
      .executeProductAction(action as any, this.currentLabel)
      .pipe(take(1))
      .toPromise();
    if (res && res.status === 'OK') {
      this.reloadLabel();
      ...
      this.globalEventsManager.push({
        ...
        message: toBePublished
          ? successMessagePublished
          : successMessageWithdrawn,
      });
    } else {
      this.globalEventsManager.push({
        action: 'error',
  ...
  batches() {
    ...
    this.router
      .navigate(['/product-labels', productId, 'labels', labelId, 'batches'])
      .then();
  }
  ...
  statistics() {
    ...
    const labelId = this.currentLabel.id;
    this.router
      .navigate(['/product-labels', productId, 'labels', labelId, 'statistics'])
      .then();
  }
  ...
  viewLabel() {
    ...
    if (this.currentLabel) {
      ...
      if (
        ...
        this.productForm.value.settings &&
        this.productForm.value.settings.language
      ) {
        window.open(
          this.productForm.value.settings.language.toLowerCase() + url,
          '_blank',
        );
      } else {
  ...
  downloadInstructions() {
    ...
    if (this.currentLabel) {
      const sub = this.productController
        .getProductLabelInstructions(this.currentLabel.id)
        .subscribe(
          (blob) => {
  ...
  feedbacks() {
    ...
    const labelId = this.currentLabel.id;
    this.router
      .navigate(['/product-labels', productId, 'labels', labelId, 'feedback'], {
        state: { data: this.currentLabel.uuid },
      })
      .then();
  }
  ...
  prefillFields(fromSocialResp: boolean, data: any) {
    if (fromSocialResp) {
      if (data.responsibility.laborPolicies) {
        this.productForm
          .get('responsibility.laborPolicies')
          .setValue(data.responsibility.laborPolicies);
      }
    } else {
      if (data.sustainability.production) {
        this.productForm
          .get('sustainability.production')
          .setValue(data.sustainability.production);
      }
      if (data.sustainability.packaging) {
        this.productForm
          .get('sustainability.packaging')
          .setValue(data.sustainability.packaging);
      }
      if (data.sustainability.co2Footprint) {
        this.productForm
          .get('sustainability.co2Footprint')
          .setValue(data.sustainability.co2Footprint);
      }
    }
    this.productForm.markAsDirty();
  }
  ...
  async initializeLabelsHelperLink() {
    const resp = await this.commonController
      .getGlobalSettings(
        this.globalEventsManager.globalSettingsKeys(
          'PRODUCT_LABELS_HELPER_LINK',
        ),
      )
      .pipe(take(1))
      .toPromise();
    if (resp && resp.data && resp.data.value) {
      this.editInfoLabelLink = resp.data.value;
    }
  }

  productNameLabelAndPlaceholder(type, field) {
    if (this.currentLabel) {
      if (type === 'LABEL' && field === 'name') {
        return $localize`:@@productLabel.label.textinput.label.name:Label name`;
      }
      if (type === 'LABEL' && field === 'photo') {
        return $localize`:@@productLabel.label.textinput.label.photo:Label photo`;
      }
      if (type === 'LABEL' && field === 'description') {
        return $localize`:@@productLabel.label.textinput.label.description:Label description`;
      }
      if (type === 'PLACEHOLDER' && field === 'name') {
        return $localize`:@@productLabel.label.textinput.placeholder.name:Enter label name`;
      }
    } else {
      if (type === 'LABEL' && field === 'name') {
        return $localize`:@@productLabel.product.textinput.label.name:Product name`;
      }
      if (type === 'LABEL' && field === 'photo') {
        return $localize`:@@productLabel.product.textinput.label.photo:Product photo`;
      }
      if (type === 'LABEL' && field === 'description') {
        return $localize`:@@productLabel.product.textinput.label.description:Product description`;
      }
      if (type === 'PLACEHOLDER' && field === 'name') {
        return $localize`:@@productLabel.product.textinput.placeholder.name:Enter product name`;
      }
    }
  }

  async setLanguageForLabel() {
    const modalRef = this.modalService.open(LanguageForLabelModalComponent, {
      centered: true,
    });
    Object.assign(modalRef.componentInstance, {
    ...
    if (modalResult) {
      ...
      if (res && res.status === 'OK') {
        const resp = await this.productController
          .getProductLabelContent(res.data.id)
          .pipe(take(1))
          .toPromise();
  ...
  public get journeyMarkersCtrl(): UntypedFormArray {
    return this.productForm.get('journeyMarkers') as UntypedFormArray;
  }
}
