...
export class ProductionRecordItemComponent extends GenericEditableItemComponent<ApiProcessDocument> {
  ...
  static emptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      const f = new UntypedFormControl(
        ProductionRecordItemComponent.createEmptyObject(),
        ApiProcessDocumentValidationScheme.validators,
      );
      return f;
    };
  }
  ...
  public generateForm(value: any): UntypedFormGroup {
    return generateFormFromMetadata(
      ApiProcessDocument.formMetadata(),
      value,
