...
} from './stakeholders-value-chain/validation';
...
export class ProductLabelStakeholdersComponent
  implements OnInit, AfterViewInit
{
  ...
  tabs = [$localize`:@@productLabelStakeholders.tab0.title:Value chain`];

  tabNames = ['value-chain'];
  ...
  product$ = combineLatest(
    ...
    tap((data: ApiProduct) => {
      ...
      this.isOwner = data.associatedCompanies.some(
        (value) =>
      ...
      for (const ac of this.currentProduct.associatedCompanies) {
        switch (ac.type) {
          case ApiProductCompany.TypeEnum.BUYER:
            this.buyers.push(ac);
            break;
          case ApiProductCompany.TypeEnum.IMPORTER:
            this.importers.push(ac);
            break;
          case ApiProductCompany.TypeEnum.EXPORTER:
            this.exporters.push(ac);
            break;
          case ApiProductCompany.TypeEnum.OWNER:
            this.owners.push(ac);
            break;
          case ApiProductCompany.TypeEnum.PRODUCER:
            this.producers.push(ac);
            break;
          case ApiProductCompany.TypeEnum.ASSOCIATION:
            this.associations.push(ac);
            break;
          case ApiProductCompany.TypeEnum.PROCESSOR:
            this.processors.push(ac);
            break;
          case ApiProductCompany.TypeEnum.TRADER:
            this.traders.push(ac);
            break;
        }
      }
    }),
  ...
  static ApiProductDataSharingAgreementEmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      return new UntypedFormControl(
        ProductLabelStakeholdersComponent.ApiProductDataSharingAgreementCreateEmptyObject(),
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.unsubscribeList.add(
      this.authService.userProfile$.subscribe((userProfile) => {
        if (userProfile) {
          this.isSystemAdmin =
            userProfile.role === ApiUserGet.RoleEnum.SYSTEMADMIN;
  ...
  initListManagers() {
    this.dataSharingAgreementListManager =
      new ListEditorManager<ApiProductDataSharingAgreement>(
        this.productForm.get('dataSharingAgreements') as UntypedFormArray,
  ...
  async addStakeholderToProduct(cId, cType) {
    try {
      this.globalEventsManager.showLoading(true);
      this.currentProduct.associatedCompanies.push({
        company: { id: cId },
        type: cType,
      });
  ...
  async newBuyer() {
    ...
    const modalRef = this.modalService.open(CompanySelectModalComponent, {
      centered: true,
    });
    ...
    if (company) {
      this.addStakeholderToProduct(
        company.id,
        ApiProductCompany.TypeEnum.BUYER,
      ).then();
    }
  }
  ...
  async newImporter() {
    ...
    if (company) {
      this.addStakeholderToProduct(
        company.id,
        ApiProductCompany.TypeEnum.IMPORTER,
      ).then();
    }
  }
  ...
  async newExporter() {
    ...
    if (company) {
      this.addStakeholderToProduct(
        company.id,
        ApiProductCompany.TypeEnum.EXPORTER,
      ).then();
    }
  }
  ...
  async newProducer() {
    ...
    if (company) {
      this.addStakeholderToProduct(
        company.id,
        ApiProductCompany.TypeEnum.PRODUCER,
      ).then();
    }
  }
  ...
  async newOwner() {
    ...
    if (company) {
      this.addStakeholderToProduct(
        company.id,
        ApiProductCompany.TypeEnum.OWNER,
      ).then();
    }
  }
  ...
  async newAssociation() {
    ...
    if (company) {
      this.addStakeholderToProduct(
        company.id,
        ApiProductCompany.TypeEnum.ASSOCIATION,
      ).then();
    }
  }
  ...
  async newProcessor() {
    ...
    if (company) {
      this.addStakeholderToProduct(
        company.id,
        ApiProductCompany.TypeEnum.PROCESSOR,
      ).then();
    }
  }
  ...
  async newTrader() {
    ...
    if (company) {
      this.addStakeholderToProduct(
        company.id,
        ApiProductCompany.TypeEnum.TRADER,
      ).then();
    }
  }
  ...
  async remove(company) {
    ...
    if (
      company.type === ApiProductCompany.TypeEnum.OWNER &&
    ...
    try {
      this.globalEventsManager.showLoading(true);
      this.currentProduct.associatedCompanies =
        this.currentProduct.associatedCompanies.filter((c) => c !== company);
      const res = await this.productController
        .updateProduct(this.currentProduct)
        .pipe(take(1))
        .toPromise();
