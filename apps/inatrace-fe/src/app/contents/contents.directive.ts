...
export class ContentsDirective implements OnInit, OnChanges, OnDestroy {
  ...
  private scrollFun: EventListenerOrEventListenerObject = (event: Event) =>
  ...
  ngOnInit() {
    ...
    this._activeParentSection$ = new BehaviorSubject<string>(
  ...
  subscribeScrollEventListener() {
    (this.scrollingView || document).addEventListener(
      'scroll',
      this.scrollFun,
      false,
    );
  ...
  unsubscribeScrollEventListener() {
    (this.scrollingView || document).removeEventListener(
      'scroll',
      this.scrollFun,
      false,
    );
  ...
  activeSection(): Observable<string> {
    return this._activeSection$.pipe(filter((section) => !!section));
  }

  activeParentSection(): Observable<string> {
    return this._activeParentSection$.pipe(filter((section) => !!section));
  }
}
