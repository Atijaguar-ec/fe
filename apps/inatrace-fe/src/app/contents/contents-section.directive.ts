...
export class ContentsSectionDirective implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    @SkipSelf()
    @Optional()
    public parentContentsSection: ContentsSectionDirective,
  ...
  resubscribe() {
    if (this.forms || this.errors) {
      let err =
        (this.forms && this.forms.some((x: AbstractControl) => x.invalid)) ||
      ...
      this.errorReportSub = combineLatest(
        ...this.forms.map((x: AbstractControl) =>
          x.statusChanges.pipe(startWith(x.status)),
        ),
        function (...args) {
          return [...args];
        },
  ...
  detectActiveChanges() {
    if (this.isInRange()) {
      ...
      if (this.isParent) {
        if (
          this.contents._activeParentSection$.value ===
          this.contentsSection.anchor
        ) {
  ...
  isInRange(): boolean {
    const pageOffset: number = this.contents.scrollingView
      ? this.contents.scrollingView.scrollTop
      : documentOffset();
    ...
    return (
      pageOffset >= offset - this.scrollOffset &&
      pageOffset <= offset + height - this.scrollOffset
    );
  }
}
