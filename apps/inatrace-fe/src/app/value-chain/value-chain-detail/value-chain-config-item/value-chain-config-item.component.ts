...
export class ValueChainConfigItemComponent
  extends GenericEditableItemComponent<any>
  implements OnInit
{
  @Input()
  ...
  configItemForm = new UntypedFormControl(null, Validators.required);
  ...
  ngOnInit(): void {
    this.procEvidenceTypesCodebook = new ProcessingEvidenceTypeService(
      this.procEvidenceTypeController,
      this.codebookTranslations,
      null,
    );
    this.procEvidenceFieldsCodebook = new ProcessingEvidenceFieldsService(
      this.procEvidenceFieldController,
      this.codebookTranslations,
    );
  }
  ...
  setSelectedValue($event) {
    if ($event) {
      ...
      if (this.configItemType === 'measure-unit-types') {
        this.form.setValue({
          ...$event,
          underlyingMeasurementUnitType: null,
          weight: null,
        });
      }
  ...
  private existingValue(selectedValue: any): boolean {
    ...
    const formControls: UntypedFormControl[] = listEditorManager.formArray
