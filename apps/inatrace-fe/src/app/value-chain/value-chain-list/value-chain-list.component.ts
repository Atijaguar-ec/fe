...
import {
  GetValueChainList,
  ValueChainControllerService,
} from '../../../api/api/valueChainController.service';
...
import {
  ...
  SortOption,
  SortOrder,
} from '../../shared/result-sorter/result-sorter-types';
...
export class ValueChainListComponent implements OnInit {
  ...
  filterStatus = new UntypedFormControl('ENABLED');
  ...
  private sortingParams$ = new BehaviorSubject<{
    sortBy: string;
    sort: SortOrder;
  ...
  private searchParams$ = combineLatest([
    ...
    this.filterStatus.valueChanges.pipe(startWith('ENABLED')),
  ...
  valueChains$ = combineLatest([
    ...
    switchMap((requestParams) =>
      this.valueChainService.getValueChainListByMap(requestParams),
    ),
  ...
  showPagination() {
    return (
      (this.showedValueChains - this.pageSize === 0 &&
        this.allValueChains >= this.pageSize) ||
      this.page > 1
    );
  }
