...
export class CompanyQuoteProcActionsForFacilityService extends GeneralSifrantService<ApiProcessingAction> {
  ...
  public makeQuery(
    key: string,
    params?: any,
  ): Observable<PagedSearchResults<ApiProcessingAction>> {
      ...
      map((allChoices: PagedSearchResults<ApiProcessingAction>) => {
        return {
          results: allChoices.results.filter(
            (x: ApiProcessingAction) =>
              queryString == null ||
              x.name.toLocaleLowerCase().indexOf(queryString) >= 0,
          ),
  ...
  public initializeCodebook() {
    this.sifrant$ =
      ...
      this.processingActionController
        .listProcessingActionsByCompanyByMap({ ...this.requestParams })
        .pipe(
          map((x) => {
            ...
            actions = actions.filter((procAction) => {
              if (
                procAction.supportedFacilities?.length > 0 &&
                procAction.supportedFacilities.findIndex(
                  (sf) => sf.id === this.facility.id,
                ) === -1
              ) {
                return false;
              }

              return !!this.facility.facilitySemiProductList.find(
                (fsp) => fsp.id === procAction.inputSemiProduct?.id,
              );
            });
