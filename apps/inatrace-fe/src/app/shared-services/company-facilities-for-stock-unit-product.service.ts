...
export class CompanyFacilitiesForStockUnitProductService extends GeneralSifrantService<ApiFacility> {
  ...
  makeQuery(
    key: string,
    params?: any,
  ): Observable<PagedSearchResults<ApiFacility>> {
    ...
    return this.facilityControllerService
      .listFacilitiesByCompanyByMap(reqParams)
      .pipe(
        map((res: ApiPaginatedResponseApiFacility) => {
          if (this.facilityIds && this.facilityIds.length > 0) {
            const filteredItems: ApiFacility[] = res.data.items.filter(
              (facility) =>
                this.facilityIds.findIndex((sfId) => sfId === facility.id) >= 0,
            );
            return {
