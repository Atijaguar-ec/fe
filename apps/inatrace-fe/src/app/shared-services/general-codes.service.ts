...
export class GeneralCodesService<T> implements CodebookHelperService<T> {
  ...
  public formGenerator(): (
    ...
    //: (x: T, validators: Array<ValidatorFn>) => FormGroupTyped<T> | FormControl {
  ...
  public makeQuery(
    key: string,
    params?: any,
  ): Observable<PagedSearchResults<T>> {
    throw new Error('Method not implemented.');
  }

  lastKey = null;

  public onTheFlyCodebook(
  ...
  public autocompleteCandidates(
    ...
    let acomplete: Observable<PagedSearchResults<T>> = combineLatest(
      ...
      (
        key: string,
        sifrant: PagedSearchResults<T>,
        selectedObjects: Array<T>,
      ) => {
        let choices = sifrant.results.filter(
          (x: T) =>
            this.allowDuplicateSelection() ||
            !selectedObjects.find(
              (el) => this.identifier(el) === this.identifier(x),
            ),
  ...
  public addElement(arr: UntypedFormArray, el: T): void {
    throw new Error('Method not implemented.');
  }
  public removeElement(arr: UntypedFormArray, index: number): void {
    arr.removeAt(index);
    arr.markAsDirty();
  ...
  public valid(model: T, input: string): boolean {
    ...
    return (
      typeof this.identifier(model) != 'undefined' &&
