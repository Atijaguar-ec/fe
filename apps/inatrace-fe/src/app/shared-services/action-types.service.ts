...
import {
  ActionTypeControllerService,
  GetActionTypeList,
} from '../../api/api/actionTypeController.service';
...
export class ActionTypesService extends GeneralSifrantService<ApiActionType> {
  constructor(private codebookService: ActionTypeControllerService) {
    super();
  ...
  public makeQuery(
    key: string,
    params?: any,
  ): Observable<PagedSearchResults<ApiActionType>> {
  ...
  public initializeCodebook() {
    this.sifrant$ =
      ...
      this.codebookService
        .getActionTypeListByMap({ ...this.requestParams })
