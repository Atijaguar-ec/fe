...
export class CountryService extends GeneralCodesService<ApiCountry> {
  constructor(protected commonController: CommonControllerService) {
    super();
  ...
  public formGenerator() {
    return (el: ApiCountry) =>
      generateFormFromMetadata(ApiCountry.formMetadata(), el);
    // return ApiCountry.formGenerator
  }

  public addElement(arr: UntypedFormArray, el: ApiCountry): void {
    arr.push(this.formGenerator()(el) as UntypedFormGroup);
    arr.markAsDirty();
  }
  ...
  public makeQuery(
    key: string,
    params?: any,
  ): Observable<PagedSearchResults<ApiCountry>> {
    let lkey = key ? key.toLocaleLowerCase() : null;
    return this.sifrant$.pipe(
      map((allChoices: PagedSearchResults<ApiCountry>) => {
        return {
          results: allChoices.results.filter(
            (x: ApiCountry) =>
              lkey == null || x.name.toLocaleLowerCase().indexOf(lkey) >= 0,
          ),
  ...
  public initializeCodebook() {
    this.sifrant$ =
      ...
      this.commonController
        .getCountries(null, 'FETCH', 500, null, null, 'ASC')
        .pipe(map((x) => this.pack(x.data.items)));
  }
}
