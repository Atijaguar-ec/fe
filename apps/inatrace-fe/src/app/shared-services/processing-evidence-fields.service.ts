...
export class ProcessingEvidenceFieldsService extends GeneralSifrantService<ApiProcessingEvidenceField> {
  ...
  public makeQuery(
    ...
    params?: any,
    productId?: string,
  ): Observable<PagedSearchResults<ApiProcessingEvidenceField>> {
    ...
    return this.fetchProcessingEvidenceFields(
      reqPars,
      this.valueChainIdList,
    ).pipe(
      map((res: ApiPaginatedResponseApiProcessingEvidenceField) => {
        ...
        return {
          results: results.filter(
            (x: ApiProcessingEvidenceField) =>
              lkey == null || x.label.toLocaleLowerCase().indexOf(lkey) >= 0,
          ),
          offset: 0,
          limit,
  ...
  private fetchProcessingEvidenceFields(
    reqParams,
    valueChainIdList?: Array<number>,
  ): Observable<ApiPaginatedResponseApiProcessingEvidenceField> {
    if (valueChainIdList !== null && valueChainIdList !== undefined) {
      return this.codebookService.listProcessingEvidenceFieldsByValueChainsByMap(
        { valueChainIds: valueChainIdList, ...reqParams },
      );
    } else {
      return this.codebookService.getProcessingEvidenceFieldListByMap(
        reqParams,
      );
    }
  }
