...
export class B2cTabsComponent implements OnInit {
  ...
  get headerBackgroundImage() {
    if (this.b2cSettings.headerBackgroundImage) {
      return (
        this.appBaseUrl +
        '/api/public/document/' +
        this.b2cSettings.headerBackgroundImage.storageKey
      );
    }
    return undefined;
  }
  ...
  generateTabList() {
    ...
    if (this.b2cSettings.tabFairPrices) {
      tempTabList.push({
        number: this.b2cSettings.orderFairPrices,
        tab: B2CTab.FAIR_PRICES,
      });
    }
    if (this.b2cSettings.tabProducers) {
      tempTabList.push({
        number: this.b2cSettings.orderProducers,
        tab: B2CTab.PRODUCERS,
      });
    }
    if (this.b2cSettings.tabQuality) {
      tempTabList.push({
        number: this.b2cSettings.orderQuality,
        tab: B2CTab.QUALITY,
      });
    }
    if (this.b2cSettings.tabFeedback) {
      tempTabList.push({
        number: this.b2cSettings.orderFeedback,
        tab: B2CTab.FEEDBACK,
      });
    }
