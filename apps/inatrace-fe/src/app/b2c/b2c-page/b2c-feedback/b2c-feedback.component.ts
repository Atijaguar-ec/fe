...
export class B2cFeedbackComponent implements OnInit {
  ...
  async submitFeedback() {
    ...
    try {
      ...
      const res = await this.publicController
        .addProductLabelFeedback(this.b2cPage.uuid, data)
        .pipe(take(1))
        .toPromise();
  ...
  get statusList() {
    const obj = {};
    obj['PRAISE'] =
      $localize`:@@frontPage.feedback.statusList.registred:Praise`;
    obj['PROPOSAL'] =
      $localize`:@@frontPage.feedback.statusList.active:Proposal`;
    obj['COMPLAINT'] =
      $localize`:@@frontPage.feedback.statusList.deactivated:Complaint`;
    return obj;
  }
