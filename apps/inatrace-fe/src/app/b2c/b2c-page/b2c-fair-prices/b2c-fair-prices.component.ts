...
export class B2cFairPricesComponent implements OnInit {
  ...
  constructor(@Inject(B2cPageComponent) private b2cPage: B2cPageComponent) {
  ...
  async initLabel() {
    ...
    switch (this.b2cSettings.graphicPriceToProducer) {
      case ApiBusinessToCustomerSettings.GraphicPriceToProducerEnum.DISABLED:
        ...
        break;
      case ApiBusinessToCustomerSettings.GraphicPriceToProducerEnum
        .PERCONTAINER:
  ...
  pricesAndDescription(data) {
    ...
    const producerPrice = this.b2cPage.qrProductLabel.priceToProducer
      ? this.b2cPage.qrProductLabel.priceToProducer
      : this.b2cSettings.manualProducerPrice;
    switch (this.b2cSettings.graphicPriceToProducer) {
      case ApiBusinessToCustomerSettings.GraphicPriceToProducerEnum
        .PERCONTAINER:
        this.worldMarketPrice = Math.ceil(
          (this.b2cSettings.worldMarket / B2cFairPricesComponent.POUND_TO_KG) *
            this.b2cSettings.containerSize,
        );
        this.fairTradePrice = Math.ceil(
          (this.b2cSettings.fairTrade / B2cFairPricesComponent.POUND_TO_KG) *
            this.b2cSettings.containerSize,
        );
        this.productPrice = Math.ceil(
          producerPrice * this.b2cSettings.containerSize,
        );
        break;
      case ApiBusinessToCustomerSettings.GraphicPriceToProducerEnum.PERKG:
        this.worldMarketPrice = Math.ceil(
      ...
      case GraphicPriceToProducerEnum.PERCENTVALUE:
        ...
        this.fairTradePrice = Math.round(
          (this.b2cSettings.fairTrade / this.b2cSettings.worldMarket) * 100,
        );
        this.productPrice = Math.round(
          (producerPrice /
            (this.b2cSettings.worldMarket /
              B2cFairPricesComponent.POUND_TO_KG)) *
            100,
        );
        break;
    }
    ...
    const fairTradePriceHeight =
      (worldMarketPriceHeight * this.fairTradePrice) / this.worldMarketPrice;
    const productPriceHeight =
      (worldMarketPriceHeight * this.productPrice) / this.worldMarketPrice;

    // Normalize the heights to a maximum graphic height
    const maxBarValue = Math.max(
      worldMarketPriceHeight,
      fairTradePriceHeight,
      productPriceHeight,
    );
    const producerGraphicScale =
      B2cFairPricesComponent.PRODUCER_GRAPHIC_HEIGHT / maxBarValue;
    ...
    const farmGatePrice = this.b2cPage.qrProductLabel.priceToFarmer
      ? this.b2cPage.qrProductLabel.priceToFarmer
      : this.b2cSettings.manualFarmGatePrice;
    switch (this.b2cSettings.graphicFarmGatePrice) {
      case ApiBusinessToCustomerSettings.GraphicFarmGatePriceEnum.PERCONTAINER:
        this.farmGateAveragePrice = Math.ceil(
          (this.b2cSettings.averageRegionFarmGatePrice /
            B2cFairPricesComponent.POUND_TO_KG) *
            this.b2cSettings.containerSize,
        );
        this.farmGateProductPrice =
          farmGatePrice * this.b2cSettings.containerSize;
      ...
      case ApiBusinessToCustomerSettings.GraphicFarmGatePriceEnum.PERCENTVALUE:
        ...
        this.farmGateProductPrice =
          (farmGatePrice / this.farmGateAveragePrice) * 100 - 100;
        this.increaseOfCoffee =
          '+' + Math.round(this.farmGateProductPrice) + '%';
        break;
    }
    ...
    if (
      this.b2cSettings.graphicFarmGatePrice ===
      GraphicFarmGatePriceEnum.PERCENTVALUE
    ) {
      increaseOfCoffeeHeight =
        (farmGateAveragePriceHeight * farmGatePrice) /
        this.farmGateAveragePrice;
    } else {
      increaseOfCoffeeHeight =
        (farmGateAveragePriceHeight * this.farmGateProductPrice) /
        this.farmGateAveragePrice;
    }

    const farmGateGraphicScale =
      B2cFairPricesComponent.FARMGATE_GRAPHIC_HEIGHT / increaseOfCoffeeHeight;

    this.farmGateAveragePriceHeight =
      farmGateAveragePriceHeight * farmGateGraphicScale;
    this.increaseOfCoffeeHeight =
      increaseOfCoffeeHeight * farmGateGraphicScale -
      this.farmGateAveragePriceHeight;
  }
}
