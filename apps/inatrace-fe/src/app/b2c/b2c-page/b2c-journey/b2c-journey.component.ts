...
export class B2cJourneyComponent implements OnInit {
  ...
  styleForIconType(iconType: string) {
    switch (iconType) {
      case 'SHIP':
        return 'af-journey-dot-shape--ship';
      case 'LEAF':
        return 'af-journey-dot-shape--leaf';
      case 'WAREHOUSE':
        return 'af-journey-dot-shape--warehouse';
      case 'QRCODE':
        return 'af-journey-dot-shape--qr-code';
      case 'OTHER':
        return 'af-journey-dot-shape--other';
      default:
        return 'af-journey-dot-shape--leaf';
    }
  }
  ...
  async data() {
    ...
    if (this.b2cPage.qrTag !== 'EMPTY') {
      ...
      if (qrData) {
        this.historyItems = qrData.historyTimeline.items.map((item) =>
          this.addIconStyleForIconType(item),
        );
