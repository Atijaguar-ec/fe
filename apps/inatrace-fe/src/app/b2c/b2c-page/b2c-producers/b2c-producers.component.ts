...
export class B2cProducersComponent implements OnInit {
  ...
  init() {
    ...
    if (this.b2cPage.publicProductLabel.videoMeetTheFarmers) {
      this.videoMeetTheFarmers = this.sanitizer.bypassSecurityTrustResourceUrl(
        this.b2cPage.publicProductLabel.videoMeetTheFarmers.link.replace(
          'youtu.be',
          'youtube.com/embed',
        ),
      );
    }
    this.picsMeetTheFarmers =
      this.b2cPage.publicProductLabel.photosMeetTheFarmers;
