...
export class TypeListComponent implements OnInit, OnChanges {
  ...
  searchName = new UntypedFormControl(null);
  ...
  searchParams$ = combineLatest(
    this.searchName.valueChanges.pipe(startWith(null), debounceTime(200)),
    (query: string) => {
      return { query };
    },
  );
    ...
    map((resp) => {
      if (resp) {
        ...
        if (
          resp.data &&
          resp.data.count &&
          this.pageSize - resp.data.count > 0
        ) {
  ...
  ngOnInit(): void {
    ...
    if (this.type === 'facility-types') {
      this.title = $localize`:@@settingsTypes.typeList.title.facility:Facility types`;
    }
    if (this.type === 'measurement-unit-types') {
      this.title = $localize`:@@settingsTypes.typeList.title.measurement:Measurement unit types`;
    }
    if (this.type === 'action-types') {
      this.title = $localize`:@@settingsTypes.typeList.title.actions:Action types`;
    }
    if (this.type === 'processing-evidence-types') {
      this.title = $localize`:@@settingsTypes.typeList.title.processingEvidenceTypes:Processing evidence types`;
    }
    if (this.type === 'processing-evidence-fields') {
      this.title = $localize`:@@settingsTypes.typeList.title.processingEvidenceFields:Processing evidence fields`;
    }
    if (this.type === 'semi-products') {
      this.title = $localize`:@@settingsTypes.typeList.title.semiProducts:Semi-products`;
    }
  ...
  apiLink(params) {
    ...
    if (this.type === 'measurement-unit-types') {
      return this.measureUnitTypeService.getMeasureUnitTypeListByMap({
        ...params,
      });
    }
    if (this.type === 'processing-evidence-types') {
      return this.processingEvidenceTypeService.getProcessingEvidenceTypeListByMap(
        { ...params },
      );
    }
    if (this.type === 'processing-evidence-fields') {
      return this.processingEvidenceFieldService.getProcessingEvidenceFieldListByMap(
        { ...params },
      );
    }
    if (this.type === 'semi-products') {
  ...
  async delete(type) {
    ...
    try {
      ...
      if (this.type === 'facility-types') {
        const res = await this.facilityTypeService
          .deleteFacilityType(type.id)
          .pipe(take(1))
          .toPromise();
      ...
      if (this.type === 'measurement-unit-types') {
        const res = await this.measureUnitTypeService
          .deleteMeasurementUnitType(type.id)
          .pipe(take(1))
          .toPromise();
        if (res && res.status === 'OK') {
          this.reloadPage();
      ...
      if (this.type === 'processing-evidence-types') {
        const res = await this.processingEvidenceTypeService
          .deleteProcessingEvidenceType(type.id)
          .pipe(take(1))
          .toPromise();
        if (res && res.status === 'OK') {
          this.reloadPage();
      ...
      if (this.type === 'processing-evidence-fields') {
        const res = await this.processingEvidenceFieldService
          .deleteProcessingEvidenceField(type.id)
          .pipe(take(1))
          .toPromise();
        if (res && res.status === 'OK') {
          this.reloadPage();
      ...
      if (this.type === 'semi-products') {
        const res = await this.semiProductsService
          .deleteSemiProduct(type.id)
          .pipe(take(1))
          .toPromise();
        if (res && res.status === 'OK') {
          this.reloadPage();
  ...
  showPagination() {
    return (
      (this.showed - this.pageSize === 0 && this.all >= this.pageSize) ||
      this.page > 1
    );
  }
