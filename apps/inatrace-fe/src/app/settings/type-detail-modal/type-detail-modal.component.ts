...
export class TypeDetailModalComponent implements OnInit {
  ...
  codebookProcessingEvidenceTypeType = EnumSifrant.fromObject(
    this.processingEvidenceTypeType,
  );
  ...
  async init() {
    if (this.type === 'facility-types') {
      if (!this.update) {
        this.form = generateFormFromMetadata(
          ApiFacilityType.formMetadata(),
          defaultEmptyObject(ApiFacilityType.formMetadata()) as ApiFacilityType,
          ApiFacilityTypeValidationScheme,
        );
      } else {
        this.form = generateFormFromMetadata(
    ...
    if (this.type === 'measurement-unit-types') {
      if (!this.update) {
        this.form = generateFormFromMetadata(
          ApiMeasureUnitType.formMetadata(),
          defaultEmptyObject(
            ApiMeasureUnitType.formMetadata(),
          ) as ApiMeasureUnitType,
          ApiMeasureUnitTypeValidationScheme,
        );
      } else {
        this.form = generateFormFromMetadata(
    ...
    if (this.type === 'semi-products') {
      if (!this.update) {
        this.form = generateFormFromMetadata(
          ...
          defaultEmptyObject(ApiSemiProduct.formMetadata()) as ApiSemiProduct,
          ApiSemiProductValidationScheme,
        );
      } else {
        this.form = generateFormFromMetadata(
    ...
    if (this.type === 'product-types') {
      if (!this.update) {
        this.form = generateFormFromMetadata(
          ApiProductType.formMetadata(),
          defaultEmptyObject(ApiProductType.formMetadata()) as ApiProductType,
          ApiProductTypeValidationScheme,
        );
      } else {
        this.form = generateFormFromMetadata(
  ...
  async save() {
    if (
      this.type === 'processing-evidence-types' ||
      this.type === 'processing-evidence-fields'
    ) {
      this.form
        .get('label')
    ...
    if (this.type === 'certification-types') {
      this.form.get('name').setValue(this.form.get('translations.0.name').value);
    }
    this.submitted = true;
    ...
    if (
      this.type === 'measurement-unit-types' &&
      (!this.form.value.weight ||
        this.form.value.weight.toString().trim() === '')
    ) {
    ...
    if (this.type === 'facility-types') {
      res = await this.facilityTypeService
        .createOrUpdateFacilityType(data)
        .pipe(take(1))
        .toPromise();
    }

    if (this.type === 'measurement-unit-types') {
      res = await this.measureUnitTypeService
        .createOrUpdateMeasurementUnitType(data)
        .pipe(take(1))
        .toPromise();
    }

    if (this.type === 'processing-evidence-types') {
      res = await this.processingEvidenceTypeService
        .createOrUpdateProcessingEvidenceType(data)
        .pipe(take(1))
        .toPromise();
    }

    if (this.type === 'processing-evidence-fields') {
      res = await this.processingEvidenceFieldService
        .createOrUpdateProcessingEvidenceField(data)
        .pipe(take(1))
        .toPromise();
    }

    if (this.type === 'semi-products') {
      res = await this.semiProductService
        .createOrUpdateSemiProduct(data)
        .pipe(take(1))
        .toPromise();
    }
    ...
    if (this.type === 'product-types') {
      if (data.id) {
        res = await this.productTypeService
  ...
  get processingEvidenceTypeType() {
    const obj = {};
    obj['DOCUMENT'] =
      $localize`:@@processingEvidenceTypeType.document:Document`;
    obj['FIELD'] = $localize`:@@processingEvidenceTypeType.field:Field`;
    obj['CALCULATED'] =
      $localize`:@@processingEvidenceTypeType.calculated:Calculated`;
    return obj;
  }
