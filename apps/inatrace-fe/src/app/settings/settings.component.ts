...
{
  submitted = false;
  labelsHelperLink = new UntypedFormControl(null);
  unpublishedProductLabelText = new UntypedFormControl(null);
  ...
  get tabCommunicationService(): TabCommunicationService {
    return this.authorizedLayout
      ? this.authorizedLayout.tabCommunicationService
      : null;
  }
  ...
  public canDeactivate(): boolean {
    return (
      ...
      (!this.unpublishedProductLabelText ||
        !this.unpublishedProductLabelText.dirty)
    );
  }
  ...
  onShow(event, type) {
    if (type === 'facility-types') {
      this.showedFacilities = event;
    }
    if (type === 'measurement-unit-types') {
      this.showedMeasurements = event;
    }
    if (type === 'action-types') {
      this.showedActions = event;
    }
    if (type === 'processing-evidence-types') {
      this.showedProcEvidTypes = event;
    }
    if (type === 'processing-evidence-fields') {
      this.showedProcEvidFields = event;
    }
    if (type === 'semi-products') {
      this.showedSemiProducts = event;
    }
  ...
  onCountAll(event, type) {
    if (type === 'facility-types') {
      this.allFacilities = event;
    }
    if (type === 'measurement-unit-types') {
      this.allMeasurements = event;
    }
    if (type === 'action-types') {
      this.allActions = event;
    }
    if (type === 'processing-evidence-types') {
      this.allProcEvidTypes = event;
    }
    if (type === 'processing-evidence-fields') {
      this.allProcEvidFields = event;
    }
    if (type === 'semi-products') {
      this.allSemiProducts = event;
    }
    if (type === 'product-types') {
  ...
  async save() {
    try {
      ...
      const res = await this.commonController
        .updateGlobalSettings(
          this.globalEventsManager.globalSettingsKeys(
            'UNPUBLISHED_PRODUCT_LABEL_TEXT',
          ),
  ...
  async initializeLabelsHelperLink() {
    const resp0 = await this.commonController
      .getGlobalSettings(
        this.globalEventsManager.globalSettingsKeys(
          'PRODUCT_LABELS_HELPER_LINK',
        ),
      ...
      .pipe(take(1))
      .toPromise();
    if (resp0 && resp0.data && resp0.data.value) {
      this.labelsHelperLink.setValue(resp0.data.value);
    }

    const resp1 = await this.commonController
      .getGlobalSettings(
        this.globalEventsManager.globalSettingsKeys(
          'UNPUBLISHED_PRODUCT_LABEL_TEXT',
        ),
      ...
      .pipe(take(1))
      .toPromise();
    if (resp1 && resp1.data && resp1.data.value) {
      this.unpublishedProductLabelText.setValue(resp1.data.value);
    }
  }
  ...
  newType(type) {
    let addTitle = '';
    if (type === 'facility-types') {
      addTitle = $localize`:@@settingsPage.newFacilityType.addTitle:Add facility type`;
    }
    if (type === 'measurement-unit-types') {
      addTitle = $localize`:@@settingsPage.newMeasurementUnitType.addTitle:Add measurement unit type`;
    }
    if (type === 'action-types') {
      addTitle = $localize`:@@settingsPage.newActionType.addTitle:Add action type`;
    }
    if (type === 'processing-evidence-types') {
      addTitle = $localize`:@@settingsPage.newProcessingEvidenceType.addTitle:Add processing evidence type`;
    }
    if (type === 'processing-evidence-fields') {
      addTitle = $localize`:@@settingsPage.newProcessingEvidenceField.addTitle:Add processing evidence field`;
    }
    if (type === 'semi-products') {
      addTitle = $localize`:@@settingsPage.newSemiProducts.addTitle:Add new semi-product`;
    }
    if (type === 'product-types') {
    ...
    this.modalService.open(
      ...
      },
      {
        ...
        saveCallback: () => {
          if (type === 'facility-types') {
            this.parentReloadFacility = !this.parentReloadFacility;
          }
          if (type === 'measurement-unit-types') {
            this.parentReloadMeasure = !this.parentReloadMeasure;
          }
          if (type === 'action-types') {
            this.parentReloadAction = !this.parentReloadAction;
          }
          if (type === 'processing-evidence-types') {
            this.parentReloadProcEvidTypes = !this.parentReloadProcEvidTypes;
          }
          if (type === 'processing-evidence-fields') {
            this.parentReloadProcEvidFields = !this.parentReloadProcEvidFields;
          }
          if (type === 'semi-products') {
            this.parentReloadSemiProducts = !this.parentReloadSemiProducts;
          }
          if (type === 'product-types') {
