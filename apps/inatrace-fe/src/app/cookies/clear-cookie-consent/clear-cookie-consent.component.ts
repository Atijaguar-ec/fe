  ...
  ngOnInit() {
    this.sub = this.route.paramMap
      ...
      .subscribe((val) => {
        if (
          [
            ...this.cookieManagementService.activeCookies,
            'ALL',
            'CHAT',
          ].indexOf(val.type) < 0
        )
        ...
        if (type === 'ALL') {
          this.cookieManagementService.activeCookies.forEach((typ) =>
            this.cookieManagementService.consentToCookie(typ, value),
          );
        } else if (type === 'CHAT') {
