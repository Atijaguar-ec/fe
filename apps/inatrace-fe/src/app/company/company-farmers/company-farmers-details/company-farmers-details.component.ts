...
export class CompanyFarmersDetailsComponent implements OnInit, OnDestroy {
  ...
  codebookCoop;
  assocCoop;
  assocForm = new UntypedFormControl(null);
  ...
  productTypesForm = new UntypedFormControl(null);
  productTypes: Array<ApiProductType> = [];
  productTypesCodebook: CompanyProductTypesService;
  selectedProductTypesForm = new UntypedFormControl(null, [
  ...
  static ApiUserCustomerCooperativeCreateEmptyObject(): ApiUserCustomerCooperative {
    const obj: ApiUserCustomerCooperative = defaultEmptyObject(
      ApiUserCustomerCooperative.formMetadata(),
    );
    obj.company = defaultEmptyObject(ApiCompany.formMetadata());
    return obj;
  }

  static ApiUserCustomerCooperativeEmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      return new UntypedFormControl(
        CompanyFarmersDetailsComponent.ApiUserCustomerCooperativeCreateEmptyObject(),
  ...
  static ApiCertificationEmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      return new UntypedFormControl(
        CompanyFarmersDetailsComponent.ApiCertificationCreateEmptyObject(),
        ApiCertificationValidationScheme.validators,
      );
    };
  }

  get certifications(): AbstractControl[] {
    return (this.farmerForm.get('certifications') as UntypedFormArray).controls;
  }

  get farmPlantInfos(): AbstractControl[] {
    return (
      this.farmerForm.get('farm.farmPlantInformationList') as UntypedFormArray
    ).controls;
  }
  ...
  ngOnInit(): void {
    ...
    this.initData().then(() => {
      ...
      this.productTypesCodebook = new CompanyProductTypesService(
        this.companyService,
        this.companyId,
      );
  ...
  async initData() {
    ...
    this.company = await this.selUserCompanyService.selectedCompanyProfile$
      .pipe(take(1))
      .toPromise();
    if (!this.company) {
      return;
    ...
    switch (action) {
      case 'new':
        ...
        const defaultProdTypeCheck = await this.companyService
          .getCompanyProductTypes(this.companyId)
          .pipe(take(1))
          .toPromise();
      ...
      case 'update':
        ...
        this.update = true;
        const uc = await this.companyService
          .getUserCustomer(this.route.snapshot.params.id)
          .pipe(first())
          .toPromise();
        if (uc && uc.status === 'OK') {
          this.farmer = uc.data;
  ...
  private prefillFarmPlantInformation(): void {
    const farmPlantInformationList = this.farmerForm.get(
      'farm.farmPlantInformationList',
    ) as UntypedFormArray;
    ...
    listControls.forEach((control) => {
      ...
      const formGroup = new UntypedFormGroup({
        productType: new UntypedFormControl(farmPlantInformation.productType),
        plantCultivatedArea: new UntypedFormControl(
          farmPlantInformation.plantCultivatedArea,
        ),
        numberOfPlants: new UntypedFormControl(
          farmPlantInformation.numberOfPlants,
        ),
      });

      this.addControlValueChangeListener(
        formGroup.get('plantCultivatedArea') as UntypedFormControl,
      );
    ...
    const listProductTypeControls = (
      this.farmerForm.get('productTypes') as UntypedFormArray
    ).controls;
    listProductTypeControls.forEach((ptControl) => {
      const productType = ptControl.value as ApiProductType;

      const productFound = listControls.find(
        (plantInfo) => plantInfo.value.productType.id === productType.id,
      );
      if (productFound === undefined) {
        const formGroup = new UntypedFormGroup({
          productType: new UntypedFormControl(productType),
          plantCultivatedArea: new UntypedFormControl(null),
          numberOfPlants: new UntypedFormControl(null),
        });
  ...
  private removeFarmPlantInformation(productType: ApiProductType): void {
    const plantInfoListArray = this.farmerForm.get(
      'farm.farmPlantInformationList',
    ) as UntypedFormArray;

    const resultArrayControls = plantInfoListArray.controls.filter(
      (control) => control.get('productType').value.id !== productType.id,
    );
  ...
  initValueChangeListeners() {
    ...
    if (this.totalCultivatedArea != null) {
      this.subscriptions.push(
        this.totalCultivatedArea.valueChanges
          .pipe(startWith(null), debounceTime(100))
          .subscribe((val) => {
            if (val !== null && val !== undefined) {
              this.updateAreaUnitValidators();
    ...
    if (this.areaOrganicCertified != null) {
      this.subscriptions.push(
        this.areaOrganicCertified.valueChanges
          .pipe(startWith(null), debounceTime(100))
          .subscribe((val) => {
            if (val !== null && val !== undefined) {
              this.updateAreaUnitValidators();
  ...
  private addControlValueChangeListener(control: UntypedFormControl) {
    this.subscriptions.push(
  ...
  newFarmer() {
    this.farmerForm = generateFormFromMetadata(
      ApiUserCustomer.formMetadata(),
      this.emptyFarmer(),
      ApiUserCustomerValidationScheme,
    );
    ...
    this.subscriptions.push(
      this.selfOnboardingService.addFarmersCurrentStep$.subscribe(
        (currentStep) => {
          if (currentStep === 3) {
  ...
  async exportGeoData() {
    ...
    this.globalEventsManager.showLoading(true);
    try {
      const res = await this.companyService
        .exportUserCustomerGeoData(this.farmer.id)
        .pipe(take(1))
        .toPromise();
      this.fileSaverService.save(
        res,
        `${this.farmer.name}_${this.farmer.surname}_geo_data.json`,
      );
    } finally {
      this.globalEventsManager.showLoading(false);
    }
  ...
  emptyFarmer() {
    const farmer: ApiUserCustomer = defaultEmptyObject(
      ApiUserCustomer.formMetadata(),
    );

    farmer.bank = defaultEmptyObject(
      ApiBankInformation.formMetadata(),
    ) as ApiBankInformation;
    farmer.farm = defaultEmptyObject(
      ApiFarmInformation.formMetadata(),
    ) as ApiFarmInformation;
    farmer.location = defaultEmptyObject(
      ApiLocation.formMetadata(),
    ) as ApiLocation;
    farmer.location.address = defaultEmptyObject(
  ...
  prepareData() {
    ...
    Object.keys(data.location).forEach(
      (key) => data.location[key] == null && delete data.location[key],
    );
    if (data.farmInfo) {
      Object.keys(data.farmInfo).forEach(
        (key) => data.farmInfo[key] == null && delete data.farmInfo[key],
      );
    }
    if (data.contact) {
      Object.keys(data.contact).forEach(
        (key) => data.contact[key] == null && delete data.contact[key],
      );
    }
    if (data.bankAccountInfo) {
      Object.keys(data.bankAccountInfo).forEach(
        (key) =>
          data.bankAccountInfo[key] == null && delete data.bankAccountInfo[key],
      );
    }
  ...
  async save(stayOnPage: boolean = false) {
    ...
    try {
      ...
      let res;
      if (!this.update) {
        res = await this.companyService
          .addUserCustomer(this.companyId, data)
          .toPromise();
      } else {
      ...
      if (res && res.status === 'OK') {
        const currentAddFarmerStep =
          await this.selfOnboardingService.addFarmersCurrentStep$
            .pipe(take(1))
            .toPromise();
  ...
  initializeListManager() {
    this.producersListManager =
      new ListEditorManager<ApiUserCustomerCooperative>(
        this.farmerForm.get('cooperatives') as UntypedFormArray,
    ...
    this.certificationListManager = new ListEditorManager<ApiCertification>(
      this.farmerForm.get('certifications') as UntypedFormArray,
  ...
  async listOfOrgAssociation() {
    const res = await this.companyService
      .getAssociations(this.companyId)
      .pipe(take(1))
      .toPromise();
  ...
  addAssociation(item, form) {
    ...
    if (
      this.farmerForm.value.associations.some((a) => a.company.id === item.id)
    ) {
      form.setValue(null);
      return;
    }
  ...
  updateAreaUnitValidators() {
    ...
    this.areaUnit.setValidators(
      this.checkAreaFieldsRequired() ? [Validators.required] : [],
    );
  ...
  checkAreaFieldsRequired() {
    return (
      !this.checkNullEmpty(this.totalCultivatedArea) ||
      this.checkPlantsCultivatedAreaFields() ||
      !this.checkNullEmpty(this.areaOrganicCertified)
    );
  }

  checkNullEmpty(control: UntypedFormControl) {
  ...
  checkPlantsCultivatedAreaFields() {
    ...
    if (farmPlantInformationList != null) {
      ...
      return controls.some(
        (control) =>
          !this.checkNullEmpty(
            control.get('plantCultivatedArea') as UntypedFormControl,
          ),
      );
    }
  }

  public get areaUnit(): UntypedFormControl {
    return this.farmerForm.get('farm.areaUnit') as UntypedFormControl;
  }

  public get totalCultivatedArea(): UntypedFormControl {
    return this.farmerForm.get(
      'farm.totalCultivatedArea',
    ) as UntypedFormControl;
  }

  public get areaOrganicCertified(): UntypedFormControl {
    return this.farmerForm.get(
      'farm.areaOrganicCertified',
    ) as UntypedFormControl;
  }
