...
export class CompanyFarmersImportComponent implements OnInit {
  ...
  fileForm = new UntypedFormControl();
      ...
      .subscribe((cp) => {
        if (cp) {
          ...
          if (companyCountryCode === 'RW') {
            this.farmerImportTemplateHref =
              '/assets/farmer-import/Template_list_of_farmers_Rwanda_en.xlsx';
          } else if (companyCountryCode === 'HN') {
            this.farmerImportTemplateHref =
              '/assets/farmer-import/Plantilla_listado_de_agricultores_Honduras_es.xlsx';
          } else if (LanguageCodeHelper.getCurrentLocale() === 'es') {
            this.farmerImportTemplateHref =
              '/assets/farmer-import/Plantilla_listado_de_agricultores_otros_paises_es.xlsx';
          } else {
            this.farmerImportTemplateHref =
              '/assets/farmer-import/Template_list_of_farmers_other_countries_en.xlsx';
          }
        }
      });
  }
  ...
  import() {
    ...
    if (this.fileForm && this.fileForm.value) {
      ...
      this.companyControllerService
        .importFarmersSpreadsheet(this.companyId, this.fileForm.value.id)
        .pipe(
          take(1),
          finalize(() => {
          ...
          if (value) {
            ...
            if (value.validationErrors && value.validationErrors.length > 0) {
              this.toastService.error(
                $localize`:@@companyDetail.farmers.import.validationErrors.message:There are validation errors in the provided spreadsheet of farmers. Please check them below.`,
              );
              ...
              if (
                ...
                this.toastService.error(
                  $localize`:@@companyDetail.farmers.import.noFarmers.error:No farmers imported`,
                );
                return;
              }
              if (
                ...
                this.toastService.success(
                  $localize`:@@companyDetail.farmers.import.success.label:Farmers successfully imported. Please check the data on Company > Farmers tab`,
                );
                this.location.back();
                return;
              }
              if (value.duplicates && value.duplicates.length > 0) {
                this.toastService.warning(
                  $localize`:@@companyDetail.farmers.import.duplicates.error:Some duplicate farmers with same name, surname and village were found. Please accept or reject duplicates`,
                );
  ...
  acceptDuplicate(user: ApiUserCustomer, index: number) {
    this.companyControllerService
      .addUserCustomer(this.companyId, user)
      .pipe(take(1))
      .subscribe(
        () =>
          this.toastService.success(
            $localize`:@@companyDetail.farmers.import.success.single: Farmer successfully imported`,
          ),
        () => {},
