  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  ngAfterViewInit() {
    this.subs = this.selfOnboardingService.addFarmersCurrentStep$.subscribe(
      (currentStep) => {
        if (currentStep === 2) {
        ...
        if (response?.data) {
          ...
          this.showing =
            this.farmerCount >= this.pageSize
              ? Math.min(this.page * this.pageSize, this.farmerCount)
              : this.farmerCount;
  ...
  async exportFarmerData(): Promise<void> {
    ...
    try {
      const res = await this.companyController
        .exportFarmerDataByCompany(this.organizationId)
        .pipe(take(1))
        .toPromise();
