...
export class CompanyUserRoleComponent
  extends GenericEditableItemComponent<ApiCompanyUser>
  implements OnInit
{
  constructor(
  ...
  userForm = new UntypedFormControl(null, Validators.required);
  ...
  get roles() {
    const obj = {};
    obj['ACCOUNTANT'] =
      $localize`:@@companyUserRole.roles.accountant:Accountant`;
    obj['MANAGER'] = $localize`:@@companyUserRole.roles.manager:Manager`;
    obj['COMPANY_USER'] =
      $localize`:@@companyUserRole.roles.companyUser:Company user`;
    obj['COMPANY_ADMIN'] =
      $localize`:@@companyUserRole.roles.companyAdmin:Company admin`;
    return obj;
  }
  ...
  async firstSendToServer() {
    ...
    if (
      ...
      this.form.value &&
      this.form.value.id &&
      this.form.value.companyRole
    ) {
      const id = this.route.snapshot.params.id;
      if (this.contentObject && !this.contentObject.name) {
        // Adding new user to the company
        const resAdd = await this.companyController
          .executeCompanyAction('ADD_USER_TO_COMPANY', {
            companyId: id,
          ...
          .toPromise();
        if (resAdd.status === 'OK') {
          const resSet = await this.companyController
            .executeCompanyAction('SET_USER_COMPANY_ROLE', {
              companyId: id,
              userId: this.form.value.id,
              companyUserRole: this.form.value.companyRole,
            })
            .pipe(take(1))
            .toPromise();
          if (resSet.status === 'OK') {
  ...
  async delete() {
    if (this.listEditorManager && this.listEditorManagerPosition != null) {
      ...
      const res = await this.companyController
        .executeCompanyAction('REMOVE_USER_FROM_COMPANY', removeRequest)
        .pipe(take(1))
        .toPromise();
