...
export class CompanyCollectorsListComponent implements OnInit {
  ...
  searchNameCollectors = new UntypedFormControl('');
  byCategory = 'BY_NAME';
    ...
    this.collector$ = combineLatest([
      ...
      map((response) => {
        if (response) {
          ...
          this.showing =
            this.collectorCount >= this.pageSize
              ? Math.min(this.page * this.pageSize, this.collectorCount)
              : this.collectorCount;
