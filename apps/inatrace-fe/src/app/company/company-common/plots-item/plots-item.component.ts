  ...
  implements OnInit
{
  ...
  plotCoordinatesManager: PlotCoordinatesManagerService =
    new PlotCoordinatesManagerService();
  ...
  get name() {
    ...
    const crop = this.form.get('crop').value?.name
      ? this.form.get('crop').value?.name
      : '/';
  ...
  undoLastPoint() {
    if (this.plotCoordinates.length > 1) {
      this.plotCoordinatesManager.registerEvent(
        PlotCoordinateAction.DELETE_LAST_COORDINATE,
      );
    }
  }
  ...
  updateFormCoordinates(coordinates: Array<ApiPlotCoordinate>) {
    if (coordinates) {
      const coordinatesFormArray = this.form.get(
        'coordinates',
      ) as UntypedFormArray;
      coordinatesFormArray.clear();

      coordinates.forEach((c) => {
        const nextPlotCoordinate = generateFormFromMetadata(
          ApiPlotCoordinate.formMetadata(),
          c,
          null,
        );
      ...
      if (this.mapEditable) {
        ...
        const geoJsonA: Feature<Polygon> = {
          ...
          geometry: {
            type: 'Polygon',
            coordinates: [coordinates2Darray],
          },
          properties: null,
        };
  ...
  async dialogEUDR() {
    return await this.globalEventsManager.openMessageModal({
      ...
      dismissable: false,
      buttonTitles: {
        ok: $localize`:@@map.modal.eudr.warning.ok:Yes, add single pin.`,
        cancel: $localize`:@@map.modal.eudr.warning.cancel:No, add polygon instead.`,
      },
    });
  }
}
