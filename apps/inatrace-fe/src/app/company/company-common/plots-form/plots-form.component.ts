...
import { OpenPlotDetailsExternallyModalComponent } from '../../company-farmers/open-plot-details-externally-modal/open-plot-details-externally-modal.component';
...
export class PlotsFormComponent implements OnInit {
  ...
  @Input()
  form: UntypedFormGroup; // contains the plots control
  ...
  static ApiPlotEmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      return new UntypedFormControl(
        PlotsFormComponent.ApiPlotCreateEmptyObject(),
        ApiPlotValidationScheme.validators,
      );
    };
  }
  ...
  ngOnInit(): void {
    ...
    this.initialLat = this.form
      .get('location')
      ?.get('address')
      ?.get('country')?.value?.latitude;
    this.initialLng = this.form
      .get('location')
      ?.get('address')
      ?.get('country')?.value?.longitude;
  ...
  initializeListManager() {
    this.plotsListManager = new ListEditorManager<ApiPlot>(
      this.form.get('plots') as UntypedFormArray,
