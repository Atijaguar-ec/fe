...
export class ProducersItemComponent
  extends GenericEditableItemComponent<ApiUserCustomerCooperative>
  implements OnInit
{
  @Input()
  disableDelete = false;
  ...
  ngOnInit() {
    ...
    if (
      this.type &&
      this.form.get('userCustomerType') &&
      this.form.get('userCustomerType').value == null
    ) {
  ...
  get name() {
    if (
      ...
      this.form.get('userCustomerType').value
    ) {
      return (
        this.form.get('company.name').value +
        ', ' +
        this.rolesObj[this.form.get('userCustomerType').value]
      );
    }
    if (this.form.get('company').value && this.form.get('company.id').value) {
      return (
        this.assocCoop[this.form.get('company.id').value.toString()] +
        ', ' +
        this.rolesObj[this.form.get('userCustomerType').value]
      );
    }
  ...
  initRoles() {
    if (this.type === UserCustomerTypeEnum.FARMER) {
      this.rolesObj['FARMER'] =
        $localize`:@@collectorDetail.roles.farmer:Farmer`;
    } else {
      this.rolesObj['COLLECTOR'] =
        $localize`:@@collectorDetail.roles.collector:Collector`;
    }
