...
export class CompanyOrdersListComponent implements OnInit {
  ...
  history(item: ApiStockOrder) {
    this.router
      .navigate(['my-stock', 'all-stock', 'stock-order', item.id, 'view'], {
  ...
  payment(order: ApiStockOrder) {
    this.router
      .navigate(['my-stock', 'payments', 'customer-order', order.id, 'new'], {
        queryParams: { returnUrl: this.router.routerState.snapshot.url },
      })
      .then();
  }
  ...
  private loadStockOrders(
    params:
      | GetQuoteOrdersInFacility.PartialParamMap
      | GetStockOrdersInFacilityForCustomer.PartialParamMap,
    ...
    if (this.orderType === 'RECEIVED') {
      ...
      return this.stockOrderController.getStockOrdersInFacilityForCustomerByMap(
        params,
      );
    }
  ...
  canDelete(order: ApiStockOrder) {
    return (
      this.orderType !== 'RECEIVED' &&
      Math.abs(order.fulfilledQuantity - order.availableQuantity) < 0.0000000001
    );
  }
  ...
  async delete(order: ApiStockOrder) {
    ...
    this.stockOrderController
      .getStockOrder(order.id, true)
      .pipe(
        tap(() => this.globalEventsManager.showLoading(true)),
        switchMap((orderResp) => {
          if (orderResp && orderResp.status === 'OK' && orderResp.data) {
            return this.processingOrderController.deleteProcessingOrder(
              orderResp.data.processingOrder.id,
            );
          }
          return of(null);
        }),
