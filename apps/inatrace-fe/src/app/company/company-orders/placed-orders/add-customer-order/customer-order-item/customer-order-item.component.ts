...
export class CustomerOrderItemComponent
  extends GenericEditableItemComponent<ApiStockOrder>
  implements OnInit, OnDestroy
{
  ...
  get name() {
    if (this.contentObject) {
      ...
      const kgFactor = (
        procAction.inputFinalProduct.measurementUnitType as ApiMeasureUnitType
      ).weight;
  ...
  private get processingAction(): ApiProcessingAction {
    if (this.form.get('processingOrder.processingAction')) {
      return this.form.get('processingOrder.processingAction')
        .value as ApiProcessingAction;
    }
    return null;
  }
  ...
  get outputQuantityLabel() {
    if (this.measurementUnit) {
      return (
        $localize`:@@stockOrderItem.textinput.quantityLabelWithUnits.label:Quantity in` +
        ' ' +
  ...
  get pricePerUnitLabel() {
    return (
      $localize`:@@stockOrderItem.textinput.pricePerUnitForEndCustomer.label:Gross price per item at end customer` +
      ` (${this.selectedCurrencyLabel})`
    );
  }

  get pricePerUnitPlaceholder() {
    return (
      $localize`:@@stockOrderItem.textinput.pricePerUnitForEndCustomer.placeholder:Enter price in` +
      ` ${this.selectedCurrencyLabel}`
    );
  }
  ...
  ngOnInit(): void {
    ...
    this.codebookOrderingProcessingActions =
      new CompanyFinalProductQuoteOrderActionsService(
        this.processingActionController,
        this.companyId,
        this.outputFacility,
      );
    ...
    this.internalLotSubs = combineLatest([
      ...
      this.form
        .get('processingOrder.processingAction')
  ...
  public generateForm(value: any): UntypedFormGroup {
    const form = generateFormFromMetadata(
