...
export class AddCustomerOrderComponent implements OnInit {
  ...
  outputFacilityForm = new UntypedFormControl(null, Validators.required);
  ...
  static StockOrderItemEmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      return new UntypedFormControl(
        AddCustomerOrderComponent.StockOrderItemCreateEmptyObject(),
        ApiStockOrderValidationScheme.validators,
      );
    };
  }

  get orderItemsFormArray(): UntypedFormArray {
  ...
  private async getCreatorId() {
    const profile = await this.authService.userProfile$
      .pipe(take(1))
      .toPromise();
    if (profile) {
      return profile.id;
  ...
  private reloadProductOrder() {
    ...
    this.initInitialData().then(() => {
      this.companyCustomerCodebook = new CompanyCustomersService(
        this.companyCustomerController,
        this.companyId,
      );
      this.outputFacilitiesCodebook = new CompanyFacilitiesService(
  ...
  private newOrder() {
    ...
    this.form = generateFormFromMetadata(
      ApiProductOrder.formMetadata(),
      {},
      ApiProductOrderValidationScheme,
    );
  ...
  async saveProductOrder() {
    ...
    productOrder.deliveryDeadline = dateISOString(
      productOrder.deliveryDeadline,
    );
    ...
    productOrder.items.forEach((order) => {
      ...
      order.finalProduct =
        order.processingOrder.processingAction.inputFinalProduct;
