...
export class PlaceQuoteOrderModalComponent implements OnInit, OnDestroy {
  facilityFormControl = new UntypedFormControl(null, Validators.required);

  processingActionFormControl = new UntypedFormControl(
    null,
    Validators.required,
  );
  ...
  ngOnInit(): void {
    this.selUserCompanyService.selectedCompanyProfile$
      ...
      .subscribe((cp) => {
        if (cp) {
          ...
          this.facilitiesCodebookService = new CompanyFacilitiesService(
    ...
    this.facilityChangedSubs = this.facilityFormControl.valueChanges.subscribe(
      (selFacility) => {
        if (selFacility) {
          this.procActionsCodebookService =
            new CompanyQuoteProcActionsForFacilityService(
  ...
  onConfirm() {
    if (
      this.facilityFormControl.valid &&
      this.processingActionFormControl.valid
    ) {
      const result: PlaceQuoteOrderModalResult = {
        facilityId: (this.facilityFormControl.value as ApiFacility).id,
        orderTemplateId: (
          this.processingActionFormControl.value as ApiProcessingAction
        ).id,
      };
