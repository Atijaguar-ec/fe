...
export class PlacedOrdersComponent
  extends OrdersTabComponent
  implements OnInit
{
  rootTab = 1;

  // Controls for company customer dropdown selection component
  companyCustomerForm = new UntypedFormControl(null);
  ...
  async placeQuoteOrder(): Promise<void> {
    ...
    const modalRef = this.modalService.open(PlaceQuoteOrderModalComponent, {
      centered: true,
    });
    ...
    if (modalResult) {
      this.router
        .navigate([
          'my-stock',
          'processing',
          modalResult.orderTemplateId,
          'facility',
          modalResult.facilityId,
          'new',
        ])
        .then();
    }
  ...
  addCustomerOrder() {
    this.router
      .navigate(['customer-order', 'add'], { relativeTo: this.route.parent })
      .then();
  }
