...
export class ApproveRejectTransactionModalComponent implements OnInit {
  ...
  approveRejectForm: UntypedFormArray = new UntypedFormArray([]);
  ...
  async prepare() {
    const resp = await this.transactionController
      .getStockOrderInputTransactions(this.stockOrderId)
      .pipe(take(1))
      .toPromise();
      ...
      for (const item of resp.data.items) {
        if (item.status === 'PENDING') {
          const form = generateFormFromMetadata(
            ApiTransaction.formMetadata(),
            item,
          );
          form.addControl('approved', new UntypedFormControl(true));
    ...
    for (const item of this.approveRejectForm.controls) {
      if (item.value.rejectComment) {
        await this.transactionController
          .rejectTransaction(item.value.id, {
            id: item.value.id,
            rejectComment: item.value.rejectComment,
          })
          .pipe(take(1))
          .toPromise();
      } else {
        await this.transactionController
          .approveTransaction(item.value.id)
          .pipe(take(1))
          .toPromise();
      }
    }
