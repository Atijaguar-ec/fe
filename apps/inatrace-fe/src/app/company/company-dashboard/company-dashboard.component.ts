...
import { ApiProcessingPerformanceRequestEvidenceField } from '../../../api/model/apiProcessingPerformanceRequestEvidenceField';
...
import { CompanyProcessingActionsByStatusService } from '../../shared-services/company-processing-actions-by-status.service';
...
export class CompanyDashboardComponent implements OnInit {
  ...
  deliveriesForm: UntypedFormGroup = this.fb.group({
  ...
  processingPerformanceForm: UntypedFormGroup = this.fb.group({
    from: this.fb.control(
      new Date(
        new Date().getFullYear() - 2,
        new Date().getMonth(),
  ...
  get facilityFormControl(): UntypedFormControl {
    return this.deliveriesForm.get('facility') as UntypedFormControl;
  }

  get farmerFormControl(): UntypedFormControl {
    return this.deliveriesForm.get('farmer') as UntypedFormControl;
  }

  get collectorFormControl(): UntypedFormControl {
    return this.deliveriesForm.get('collector') as UntypedFormControl;
  }

  get organicFormControl(): UntypedFormControl {
    return this.deliveriesForm.get('organic') as UntypedFormControl;
  }

  get womensOnlyFormControl(): UntypedFormControl {
    return this.deliveriesForm.get('womenOnly') as UntypedFormControl;
  }

  get productInDepositFormControl(): UntypedFormControl {
    return this.deliveriesForm.get('productInDeposit') as UntypedFormControl;
  }

  get processingFacilityFormControl(): UntypedFormControl {
    return this.processingPerformanceForm.get('facility') as UntypedFormControl;
  }

  get processingActionFormControl(): UntypedFormControl {
    return this.processingPerformanceForm.get('process') as UntypedFormControl;
  }

  processingEvidenceInputField(name: string) {
    return this.processingPerformanceForm.get(name) as UntypedFormControl;
  }

  ngOnInit(): void {
    this.companyId = Number(localStorage.getItem('selectedUserCompany'));
    this.facilityCodebook = new CompanyCollectingFacilitiesService(
      this.facilityController,
      this.companyId,
    );
    this.companyFacilityCodebook = new CompanyFacilitiesService(
      this.facilityController,
      this.companyId,
    );
    ...
    this.companyControllerService
      ...
      .subscribe((next) => {
        ...
        this.companyValueChains.forEach((valueChain) => {
          this.valueChainController
            .getValueChain(valueChain.id)
            .subscribe((vchain) => {
              if (vchain?.data?.measureUnitTypes) {
                vchain.data.measureUnitTypes.forEach((unitType) => {
                  if (
                    !this.measureUnitTypes.some((ut) => ut?.id === unitType?.id)
                  ) {
    ...
    this.procActionsCodebook = new CompanyProcessingActionsByStatusService(
    ...
    this.procActionController
      .listProcessingActionsByCompanyByMap({
        limit: 1000,
        offset: 0,
        id: this.companyId,
      })
      .subscribe((next) => {
        if (next?.data) {
          this.processingActions = next.data.items.filter((pa) =>
            ['PROCESSING', 'FINAL_PROCESSING'].some((type) => type === pa.type),
          );
          setTimeout(() => {
            this.processingPerformanceForm
              .get('process')
              ?.setValue(this.processingActions[0]);
    ...
    this.facilityController
      .listCollectingFacilitiesByCompanyByMap({
        limit: 1000,
        offset: 0,
        id: this.companyId,
      })
      .subscribe((next) => {
        if (next?.data?.items[0]) {
          ...
          this.semiProductModelChoice =
            next.data.items[0].facilitySemiProductList[0]?.id;
          setTimeout(() => {
            this.semiProductSelected(
              next.data.items[0].facilitySemiProductList[0]?.id?.toString(),
            );
          });
        }
      });
    ...
    let diff = Math.abs(
      this.deliveriesForm.get('from')?.value?.getTime() -
        this.deliveriesForm.get('to')?.value?.getTime(),
    );
    ...
    diff = Math.abs(
      this.processingPerformanceForm.get('from')?.value?.getTime() -
        this.processingPerformanceForm.get('to')?.value?.getTime(),
    );
  ...
  refreshDeliveriesData() {
    ...
    const womenOnlyFilter = this.deliveriesForm.get('womenOnly')?.value
      ? true
      : null;
    const organicFilter = this.deliveriesForm.get('organic')?.value
      ? true
      : null;
    const productInDepositFilter = this.deliveriesForm.get('productInDeposit')
      ?.value
      ? true
      : null;
    ...
    this.dashboardControllerService
      .getDeliveriesAggregatedDataByMap({
        ...
        productionDateStart: dateISOString(
          this.deliveriesForm.get('from')?.value,
        ),
      ...
      .subscribe((next) => {
        if (next?.data) {
          ...
          this.deliveriesGraphTrunked =
            next.data.totals?.length > this.maxShownOnDeliveriesGraphs;

          // Only first n results are shown in the graph
          next.data.totals
            ?.slice(0, this.maxShownOnDeliveriesGraphs)
            .forEach((total) => {
              const timeUnit = this.calculateTimeUnit(
                total.unit,
                total.year,
                this.deliveriesForm.get('timeUnitGraphType').value,
              );
  ...
  refreshProcessingPerformanceData() {
    ...
    this.evidenceFields.forEach((evidenceField) => {
      const value = this.processingPerformanceForm.get(
        evidenceField.fieldName,
      )?.value;
      if (value) {
    ...
    this.dashboardControllerService
      .calculateProcessingPerformanceData({
        companyId: this.companyId,
        facilityId: this.processingPerformanceForm.get('facility')?.value?.id,
        processActionId:
          this.processingPerformanceForm.get('process')?.value?.id,
        dateStart: dateISOString(
          this.processingPerformanceForm.get('from')?.value,
        ),
        dateEnd: dateISOString(this.processingPerformanceForm.get('to')?.value),
        aggregationType:
          this.processingPerformanceForm.get('timeUnitGraphType')?.value,
        evidenceFields: efs,
      })
      .subscribe((next) => {
        if (next?.data) {
          ...
          this.procPerformanceGraphTrunked =
            next.data.totals?.length > this.maxShownOnProcPerformanceGraph;

          // Only first n results are shown in the graph
          next.data.totals
            ?.slice(0, this.maxShownOnProcPerformanceGraph)
            .forEach((total) => {
              const timeUnit = this.calculateTimeUnit(
                total.unit,
                total.year,
  ...
  private translateGraphWeek(weekNum: string, year: number) {
    return (
      $localize`:@@companyDashboardComponent.graph.timeUnit.week:Week` +
      '-' +
      weekNum +
      ' ' +
      year
  ...
  reloadProcessingFields(procAction: ApiProcessingAction) {
    if (procAction) {
      ...
      const foundProcAction = this.processingActions.find(
        (pa) => pa.id === procAction.id,
      );
      if (foundProcAction) {
        foundProcAction.requiredEvidenceFields?.forEach((evidenceField) => {
          if (
            !this.evidenceFields.some(
              (ef) => ef.fieldName === evidenceField.fieldName,
            )
          ) {
            this.evidenceFields.push(evidenceField);
            this.processingPerformanceForm.addControl(
              evidenceField.fieldName,
