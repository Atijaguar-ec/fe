...
export class BeycoOfferListComponent implements OnInit, OnDestroy {
  ...
  coffeeInfoForms: UntypedFormGroup[] = [];
              ...
              (res) => {
                ...
                for (const coffeeOrder of res.data.offerCoffees) {
                  ...
                  this.subscriptions.push(
                    coffeeForm
                      .get('cuppingScore')
                      .valueChanges.subscribe((cuppingScore) =>
                        this.isSpecialityCoffee(cuppingScore, coffeeForm),
                      ),
                    coffeeForm
                      .get('varieties')
                      .valueChanges.subscribe(() =>
                        this.resetValidatorOnCustomVariety(coffeeForm),
                      ),
                    coffeeForm
                      .get('grades')
                      .valueChanges.subscribe(() =>
                        this.resetValidatorOnQualityDescription(coffeeForm),
                      ),
                    coffeeForm
                      .get('incoterms')
                      .valueChanges.subscribe(() =>
                        this.resetValidatorOnCustomIncoterms(coffeeForm),
                      ),
                  );
  ...
  fc(form: UntypedFormGroup, fieldName: string): UntypedFormControl {
    return form.get(fieldName) as UntypedFormControl;
  }
  ...
  submit() {
    ...
    this.beycoOrderService
      .sendBeycoOrder(
        this.beycoTokenService.beycoToken.accessToken,
        this.companyId,
        this.buildApiBeycoOrderFields(),
      )
      .subscribe(
        () => {
          this.globalEventManager.push({
        ...
        (error) => {
          this.globalEventManager.push({
            ...
            message:
              error.error ??
              $localize`:@@beycoOrderList.notification.unsuccessfulOrder.message:There was an error when creating Beyco order! Please, try again!`,
          });
        },
        () => this.globalEventManager.showLoading(false),
      );
  ...
  buildGeneralForm(fieldValues: ApiBeycoOrderFields): UntypedFormGroup {
  ...
  buildCoffeeInfoForm(coffee: ApiBeycoOrderCoffees): UntypedFormGroup {
    return this.fb.group(
      {
        ...
        varieties: [
          coffee?.coffee.varieties?.map((v) => v.type),
          Validators.required,
        ],
        customVariety: [
          coffee?.coffee.varieties?.find(
            (v) => v.type === ApiBeycoCoffeeVariety.TypeEnum.Other,
          )?.customVariety,
        ],
        qualitySegments: [
          coffee?.coffee.qualitySegments?.map((v) => v.type),
          Validators.required,
        ],
        grades: [
          coffee?.coffee.grades?.map((v) => v.type),
          Validators.required,
        ],
        additionalQualityDescriptors: [
          coffee?.coffee.additionalQualityDescriptors,
        ],
  ...
  buildApiBeycoOrderFields(): ApiBeycoOrderFields {
    ...
    for (const form of this.coffeeInfoForms) {
      ...
      apiCoffee.customIncoterms =
        form.get('incoterms').value === 'Other'
          ? form.get('customIncoterms').value
          : null;
      apiCoffee.coffee = {
        certificates: form
          .get('certificates')
          .value?.map((c) => ({ type: c }) as ApiBeycoCoffeeCertificate),
        grades: form
          .get('grades')
          .value?.map((g) => ({ type: g }) as ApiBeycoCoffeeGrade),
        additionalQualityDescriptors: form
          .get('grades')
          .value?.includes('Other')
          ? form.get('additionalQualityDescriptors').value
          : null,
        qualitySegments: form
          .get('qualitySegments')
          .value?.map((q) => ({ type: q }) as ApiBeycoCoffeeQuality),
        varieties: form.get('varieties').value?.map(
          (v) =>
            ({
              type: v,
              customVariety:
                v === ApiBeycoCoffeeGrade.TypeEnum.Other
                  ? form.get('customVariety').value
                  : null,
  ...
  isSpecialityCoffee(value: number, form: UntypedFormGroup) {
    if (value >= 80) {
  ...
  validateScreenSizes: ValidatorFn = (form: UntypedFormGroup) => {
    ...
    if (
      ...
      minScreenSize !== '' &&
      maxScreenSize !== '' &&
      Number(minScreenSize) > Number(maxScreenSize)
    ) {
  ...
  resetValidatorOnCustomVariety(coffeeForm: UntypedFormGroup) {
  ...
  resetValidatorOnQualityDescription(coffeeForm: UntypedFormGroup) {
    if (coffeeForm.get('grades').value?.includes('Other')) {
      coffeeForm
        .get('additionalQualityDescriptors')
  ...
  resetValidatorOnCustomIncoterms(coffeeForm: UntypedFormGroup) {
