...
export class StockAllStockTabComponent
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.subscriptions.push(
      ...
      this.facilityIdPing$.subscribe((facilityId) =>
  ...
  openBeycoOrderFieldList() {
    const stockOrderIds = this.showGroupView
      ? this.selectedGroupOrders.map((o) => o.groupedIds[0])
      : this.selectedOrders.map((o) => o.id);
    this.router
      .navigate(['my-stock', 'beyco', 'list'], {
        queryParams: { id: stockOrderIds },
      })
      .then();
  }
  ...
  private isAuthorizedForBeyco() {
    if (this.companyProfile) {
      this.isAuthRoleToExportToBeyco =
        !!this.companyProfile.allowBeycoIntegration;
    }
  }
