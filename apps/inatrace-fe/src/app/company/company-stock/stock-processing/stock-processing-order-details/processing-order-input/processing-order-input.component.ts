...
import {
  GetAvailableStockForStockUnitInFacility,
...
@Component({
  ...
  styleUrls: [
    './processing-order-input.component.scss',
...
export class ProcessingOrderInputComponent implements OnInit, OnDestroy {
  ...
  dateFromFilterControl = new UntypedFormControl(null);
  dateToFilterControl = new UntypedFormControl(null);
  internalLotNameSearchControl = new UntypedFormControl(null);
  womenOnlyFilterControl = new UntypedFormControl(null);
  organicOnlyFilterControl = new UntypedFormControl(null);

  // Input stock orders properties and controls
  organicOnlyInputStockOrders = false;
  womenOnlyInputStockOrders = false;
  cbSelectAllControl = new UntypedFormControl(false);
  ...
  @Input()
  inputFacilityControl: UntypedFormControl;

  @Input()
  totalInputQuantityControl: UntypedFormControl;

  @Input()
  currentInputStockUnit: ApiSemiProduct | ApiFinalProduct;

  @Input()
  remainingQuantityControl: UntypedFormControl;

  @Input()
  targetStockOrdersArray: UntypedFormArray;

  @Input()
  inputTransactionsArray: UntypedFormArray;
  ...
  get womenOnlyStatusValue() {
    if (this.womenOnlyFilterControl.value != null) {
      if (this.womenOnlyFilterControl.value) {
        return $localize`:@@productLabelStockProcessingOrderDetail.womensOnlyStatus.womenCoffee:Women coffee`;
      } else {
        return $localize`:@@productLabelStockProcessingOrderDetail.womensOnlyStatus.nonWomenCoffee:Non-women coffee`;
      }
    }
    return null;
  }
  ...
  get oneInputStockOrderRequired() {
    ...
    const existingInputTRCount = this.inputTransactions
      ? this.inputTransactions.length
      : 0;
    const selectedInputSOCount = this.selectedInputStockOrders
      ? this.selectedInputStockOrders.length
      : 0;
  ...
  async dateSearch() {
    ...
    this.availableInputStockOrders =
      await this.fetchAvailableStockOrders(requestParams);
  ...
  private registerInputFacilityValueChangeListener() {
    this.subscriptions.push(
      this.inputFacilityControl.valueChanges.subscribe(
        (facility: ApiFacility) => {
          ...
          if (
            ...
            this.targetStockOrdersArray?.length > 0
          ) {
            this.targetStockOrdersArray
              .at(0)
              .get('quoteFacility')
              .setValue(facility);
          }
  ...
  private async fetchAvailableStockOrders(
    params: GetAvailableStockForStockUnitInFacility.PartialParamMap,
  ): Promise<ApiStockOrder[]> {
      ...
      .pipe(
          ...
          if (res && res.status === 'OK' && res.data) {
            ...
            if (this.editing) {
              ...
              this.targetStockOrdersArray.value.forEach((tso) => {
                const soIndex = availableStockOrders.findIndex(
                  (aso) => aso.id === tso.id,
                );
                if (soIndex !== -1) {
        ...
        map((availableStockOrders) => {
          ...
          if (this.selectedProcAction.type === 'GENERATE_QR_CODE') {
            return availableStockOrders.filter(
              (apiStockOrder) => !apiStockOrder.qrCodeTag,
            );
            ...
            return availableStockOrders.filter(
              (apiStockOrder) =>
                !apiStockOrder.qrCodeTag ||
                apiStockOrder.qrCodeTagFinalProduct.id === finalProduct.id,
            );
          } else {
            return availableStockOrders;
          }
  ...
  cbSelectAllClick() {
    ...
    if (this.cbSelectAllControl.value) {
      ...
      this.availableInputStockOrders.map((item) => {
        item.selected = true;
  ...
  cbSelectClick(stockOrder: ApiStockOrderSelectable, index: number) {
    ...
    if (!this.availableInputStockOrders[index].selected) {
      ...
      const currentAvailable =
        this.availableInputStockOrders[index].availableQuantity;
      ...
      if (this.actionType === 'SHIPMENT') {
        ...
        if (toFill > 0 && currentAvailable > 0) {
          ...
          this.availableInputStockOrders[index].selectedQuantity =
            toFill < currentAvailable ? toFill : currentAvailable;
        } else {
  ...
  private setOrganicAndWomenOnly() {
    ...
    this.organicOnlyInputStockOrders =
      countOrganic === allSelected && allSelected > 0;
    this.womenOnlyInputStockOrders =
      countWomenShare === allSelected && allSelected > 0;
  }
  ...
  private clipInputQuantity() {
    ...
    this.selectedInputStockOrders = this.selectedInputStockOrders.filter(
      (so) => so.selected,
    );
  }
      ...
      if (inputQuantity) {
        switch (this.actionType) {
          case 'GENERATE_QR_CODE':
            ...
            this.totalInputQuantityControl.setValue(
              Number(inputQuantity).toFixed(2),
            );
            this.targetStockOrdersArray
              .at(0)
              .get('totalQuantity')
              .setValue(Number(inputQuantity).toFixed(2));
            break;
          default:
            this.totalInputQuantityControl.setValue(
              Number(inputQuantity).toFixed(2),
  ...
  prepInputTransactionsFromStockOrders(): ApiTransaction[] {
    ...
    const isProcessing =
      ...
      this.actionType === 'FINAL_PROCESSING' ||
      this.actionType === 'GENERATE_QR_CODE';
    const status: ApiTransaction.StatusEnum =
      this.actionType === 'SHIPMENT' ? StatusEnum.PENDING : StatusEnum.EXECUTED;
  ...
  prepareTransferTargetStockOrders(
    ...
    for (const [
      index,
      selectedStockOrder,
    ] of this.selectedInputStockOrders.entries()) {
      const newStockOrder: ApiStockOrder = {
        ...
        productionDate: selectedStockOrder.productionDate
          ? selectedStockOrder.productionDate
          : (dateISOString(new Date()) as any),
