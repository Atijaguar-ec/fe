...
import {
  ApiProcessingOrderValidationScheme,
...
@Component({
  ...
  styleUrls: [
    './stock-processing-order-details.component.scss',
    './stock-processing-order-details-common.scss',
  ],
  standalone: false,
})
export class StockProcessingOrderDetailsComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  procActionLotPrefixControl = new UntypedFormControl({
    value: null,
    disabled: true,
  });
  qrCodeForFinalProductControl = new UntypedFormControl({
    value: null,
    disabled: true,
  });

  // Properties and controls used for display and selection of input facility
  inputFacilitiesCodebook:
    | CompanyFacilitiesForStockUnitProductService
    | AvailableSellingFacilitiesForCompany;
  inputFacilityControl = new UntypedFormControl(null, Validators.required);

  // Input stock orders properties and controls
  totalInputQuantityControl = new UntypedFormControl({
    value: null,
    disabled: true,
  });
  remainingQuantityControl = new UntypedFormControl({
    value: null,
    disabled: true,
  });
  ...
  totalOutputQuantityControl = new UntypedFormControl({
    value: null,
    disabled: true,
  });
  ...
  otherProcessingEvidenceArray = new UntypedFormArray([]);
  ...
  get selectedProcAction(): ApiProcessingAction {
    return (
      (this.procOrderGroup?.get('processingAction')
        ?.value as ApiProcessingAction) ?? null
    );
  }
  ...
  get inputTransactions(): ApiTransaction[] {
    return this.inputTransactionsArray.value
      ? this.inputTransactionsArray.value
      : [];
  }

  get totalOutputQuantity() {
    return this.totalOutputQuantityControl.value
      ? parseFloat(this.totalOutputQuantityControl.value)
      : 0;
  }

  get totalInputQuantity() {
    return this.totalInputQuantityControl.value
  ...
  get totalOutputQuantityTooLarge() {
    if (
      this.actionType === 'SHIPMENT' ||
      this.totalInputQuantityControl.value == null
    ) {
      return false;
    }
  ...
  get outputQuantityNotInRange() {
    if (
      this.actionType !== 'PROCESSING' ||
      !this.selectedProcAction?.estimatedOutputQuantityPerUnit ||
      !this.totalOutQuantityRangeLow ||
      !this.totalOutQuantityRangeHigh
    ) {
      return false;
    }
    ...
    if (
      !(
        this.totalOutputQuantity >= this.totalOutQuantityRangeLow &&
        this.totalOutputQuantity <= this.totalOutQuantityRangeHigh
      )
    ) {
      return true;
    }
  ...
  get expectedOutputQuantityHelpText() {
    ...
    if (this.totalInputQuantity) {
      ...
      expectedOutputQuantity =
        this.totalInputQuantity *
        this.selectedProcAction.estimatedOutputQuantityPerUnit;

      this.totalOutQuantityRangeLow = Number(expectedOutputQuantity * 0.8);
      this.totalOutQuantityRangeHigh = Math.min(
        Number(expectedOutputQuantity * 1.2),
        this.totalInputQuantity,
      );
  ...
  get targetStockOrdersArray(): UntypedFormArray {
    return this.procOrderGroup.get('targetStockOrders') as UntypedFormArray;
  }
  ...
  get inputTransactionsArray(): UntypedFormArray {
    return this.procOrderGroup.get('inputTransactions') as UntypedFormArray;
  }
  ...
  private async processingActionUpdated(procAction: ApiProcessingAction) {
    ...
    StockProcessingOrderDetailsHelper.setRequiredProcessingEvidence(
      procAction,
    ...
    if (procAction) {
      ...
      if (
        procAction &&
        procAction.type === 'GENERATE_QR_CODE' &&
        procAction.qrCodeForFinalProduct
      ) {
        this.qrCodeForFinalProductControl.setValue(
          `${procAction.qrCodeForFinalProduct.name} (${procAction.qrCodeForFinalProduct.product.name})`,
        );
      }
  ...
  async saveProcessingOrder() {
    ...
    try {
      ...
      if (this.actionType === 'TRANSFER') {
        ...
        processingOrder.targetStockOrders.forEach((targetStockOrder) => {
          if (
            ...
            processedTargetStockOrders.push(
              ...this.prepareRepackedTargetStockOrders(
                targetStockOrder as RepackedTargetStockOrder,
              ),
            );
          } else {
      ...
      if (!res || res.status !== 'OK') {
        throw Error(
          'Error while creating processing order for order type: ' +
            this.actionType,
        );
      } else {
        this.dismiss();
  ...
  private async initializeData() {
    ...
    this.procActionsCodebook = new CompanyProcessingActionsService(
      this.procActionController,
      this.companyId,
    );
    ...
    if (pageMode === 'create') {
      ...
      if (procActionIdPathParam !== 'NEW') {
        this.facilityIdPathParam = Number(
          this.route.snapshot.params.inputFacilityId,
        );
  ...
  private loadProcessingUser() {
    this.authService.userProfile$
      .pipe(take(1))
      .toPromise()
      .then((profile) => (this.processingUserId = profile.id));
  }

  private async loadCompanyProfile() {
    const cp = await this.selUserCompanyService.selectedCompanyProfile$
      .pipe(take(1))
      .toPromise();
    if (cp) {
  ...
  private async loadProcessingAction(procActionId: number) {
    const respProcAction = await this.procActionController
      .getProcessingAction(procActionId)
      .pipe(take(1))
      .toPromise();
    if (
      respProcAction &&
  ...
  private async loadProcessingOrder() {
    ...
    if (!stockOrderId) {
      throw Error('No Stock order ID in path!');
    }

    // Get the Processing order for the Stock order with the provided ID
    const respProcessingOrder = await this.stockOrderController
      .getStockOrderProcessingOrder(Number(stockOrderId))
      .pipe(take(1))
      .toPromise();
    ...
    setTimeout(async () => {
      ...
      const firstTSO = this.targetStockOrdersArray.at(0) as UntypedFormGroup;
      StockProcessingOrderDetailsHelper.loadExistingOtherEvidenceDocuments(
        firstTSO,
        this.otherProcessingEvidenceArray,
      );
      StockProcessingOrderDetailsHelper.loadExistingRequiredEvidenceDocuments(
        firstTSO,
        this.requiredProcessingEvidenceArray,
      );
      ...
      this.targetStockOrdersArray.controls.forEach((tso) =>
        this.output.setRequiredFieldsAndListenersForTSO(
          tso as UntypedFormGroup,
        ),
      );
    });
  }
  ...
  private async loadFacilities() {
    if (this.editing) {
      ...
      if (this.selectedProcAction.type === TypeEnum.SHIPMENT) {
        const quoteFacility =
          this.targetStockOrdersArray.value[0].quoteFacility;
      ...
      const facilities = await this.inputFacilitiesCodebook
        .getAllCandidates()
        .toPromise();
      ...
      if (this.facilityIdPathParam != null) {
        ...
        const facility = facilities.find(
          (f) => f.id === this.facilityIdPathParam,
        );
        if (facility) {
  ...
  private initializeFacilitiesCodebooks() {
    ...
    if (inputSemiProductId || inputFinalProductId) {
      ...
      if (this.actionType === 'SHIPMENT') {
        this.inputFacilitiesCodebook = new AvailableSellingFacilitiesForCompany(
        ...
        this.inputFacilitiesCodebook =
          new CompanyFacilitiesForStockUnitProductService(
            this.facilityController,
            this.companyId,
            inputSemiProductId,
            inputFinalProductId,
            supportedFacilitiesIds,
          );
      }
    }
    ...
    if (this.selectedProcAction.outputSemiProducts?.length > 0) {
      this.semiProductOutputFacilitiesCodebooks = new Map<
        number,
        CompanyFacilitiesForStockUnitProductService
      >();
      this.selectedProcAction.outputSemiProducts.forEach((osp) => {
        ...
        if (this.actionType === 'SHIPMENT') {
          facilityCodebook = new CompanyFacilitiesForStockUnitProductService(
            this.facilityController,
            this.companyId,
            osp.id,
            undefined,
  ...
  private async defineInputAndOutputStockUnits(
    procAction: ApiProcessingAction,
  ) {
    ...
    switch (procAction.type) {
      ...
      case ApiProcessingAction.TypeEnum.FINALPROCESSING:
        ...
        this.outputFinalProductNameControl.setValue(
          this.currentOutputFinalProduct
            ? `${this.currentOutputFinalProduct.name} (${this.currentOutputFinalProduct.product.name})`
            : null,
        );
        break;
  ...
  private initProcEvidenceListManager() {
    this.processingEvidenceListManager =
      new ListEditorManager<ApiActivityProof>(
        this.otherProcessingEvidenceArray,
  ...
  private prepareNewProcOrderGroup() {
    this.procOrderGroup = generateFormFromMetadata(
      ApiProcessingOrder.formMetadata(),
      {},
      ApiProcessingOrderValidationScheme,
    );
  ...
  private prepareEditingProcOrderGroup(processingOrder: ApiProcessingOrder) {
    ...
    const repackedStockOrdersMap: Map<string, ApiStockOrder[]> = new Map<
      string,
      ApiStockOrder[]
    >();
    ...
    processingOrder.targetStockOrders.forEach((tso, index) => {
      ...
      if (tso.repackedOriginStockOrderId != null) {
        const repackedTSOs = repackedStockOrdersMap.get(
          tso.repackedOriginStockOrderId,
        );
        if (repackedTSOs) {
        ...
        this.targetStockOrdersArray.push(
          generateFormFromMetadata(
    ...
    for (const repackedOriginStockOrderId of repackedStockOrdersMap.keys()) {
      ...
      if (maxOutputWeight == null) {
        processingOrder.processingAction.outputSemiProducts.forEach(
          (apiProcActionOSM) => {
            if (
              apiProcActionOSM.id ===
              repackedTargetStockOrders[0].semiProduct?.id
            ) {
      ...
      const targetStockOrderGroup = generateFormFromMetadata(
        ApiStockOrder.formMetadata(),
        repackedTargetStockOrders[0],
        ApiStockOrderValidationScheme,
      );

      // Add repacked outputs FormArray
      targetStockOrderGroup.addControl(
        'repackedOutputsArray',
      ...
      repackedTargetStockOrders.forEach((tso) => {
        const repackedStockOrderGroup = generateFormFromMetadata(
          ApiStockOrder.formMetadata(),
          tso,
          ApiStockOrderValidationScheme,
        );
  ...
  calcRemainingQuantity() {
    ...
    if (this.actionType === 'SHIPMENT') {
      this.remainingQuantityControl.setValue(
        Number(this.totalOutputQuantity - (inputQuantity ?? 0)).toFixed(2),
      );
      return;
  ...
  calcTotalOutputQuantity() {
    let sumInKGs = 0;
    (this.targetStockOrdersArray.getRawValue() as ApiStockOrder[]).forEach(
      (tso) => {
  ...
  private prepareRepackedTargetStockOrders(
    sourceStockOrder: RepackedTargetStockOrder,
  ): ApiStockOrder[] {
    const repackedTSOId =
      sourceStockOrder.repackedOriginStockOrderId ?? uuidv4();
    ...
    return sourceStockOrder.repackedOutputsArray.map((repackedSacUnit) => {
      ...
      newStockOrder['requiredProcEvidenceFieldGroup'] =
        sourceStockOrder['requiredProcEvidenceFieldGroup'];
  ...
  private enrichTargetStockOrders(targetStockOrders: ApiStockOrder[]) {
    ...
    const requiredEvidenceTypeValues =
      StockProcessingOrderDetailsHelper.prepareRequiredEvidenceTypeValues(
        this.requiredProcessingEvidenceArray,
      );
    const otherEvidenceDocuments =
      StockProcessingOrderDetailsHelper.prepareOtherEvidenceDocuments(
        this.otherProcessingEvidenceArray,
      );
    ...
    targetStockOrders.forEach((tso) => {
      ...
      tso.requiredEvidenceFieldValues =
        StockProcessingOrderDetailsHelper.prepareRequiredEvidenceFieldValues(
          tso['requiredProcEvidenceFieldGroup'],
          this.selectedProcAction,
        );
