...
export class FacilityCardComponent implements OnInit {
  ...
  whereIsIt(facility: ApiFacility) {
    if (
      ...
      facility.facilityLocation.address &&
      facility.facilityLocation.address.country
    ) {
  ...
  facilityTypeColor() {
    return this.FACILITY_COLOR_CLASSES[
      (this.indexInList ?? 0) % this.FACILITY_COLOR_CLASSES.length
    ];
  }
  ...
  private setMenuOptions() {
    for (const action of this.actions) {
      ...
      if (
        action.supportedFacilities &&
        action.supportedFacilities.length > 0 &&
        action.supportedFacilities.findIndex(
