...
export class StockPaymentsListComponent implements OnInit, OnDestroy {
  ...
  @Input()
  representativeOfRecepientUserCustomerIdPing$ = new BehaviorSubject<number>(
    null,
  );
  ...
  ngOnInit(): void {
    this.clearCheckboxesSubscription = this.clickClearCheckboxesPing$.subscribe(
      (val) => {
        if (val) {
          this.clearCBs();
        }
    ...
    this.payments$ = combineLatest([
      ...
      tap((data: ApiPaginatedListApiPayment) => {
        if (data) {
          this.aggregatedTotalPaid = this.calculateAggregatedTotalPaid(
            data.items,
          );
        }
      }),
  ...
  async deletePayment(entity) {
    ...
    const res = await this.paymentControllerService
      .deletePaymentByMap(entity)
      .pipe(take(1))
      .toPromise();
  ...
  showPagination() {
    return (
      (this.showedPayments - this.pageSize === 0 &&
        this.allPayments > this.pageSize) ||
      this.page > 1
    );
  }
  ...
  get paymentPurposeTypes() {
    ...
    obj['FIRST_INSTALLMENT'] =
      $localize`:@@paymentForm.paymentPurposeTypes.firstInstallment:Cherry payment`;
    obj['SECOND_INSTALLMENT'] =
      $localize`:@@paymentForm.paymentPurposeTypes.secondInstallment:Member bonus`;
    obj['WOMEN_PREMIUM'] =
      $localize`:@@paymentForm.paymentPurposeTypes.womenPreminum:AF Women premium`;
    obj['INVOICE_PAYMENT'] =
