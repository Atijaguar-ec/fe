...
export class StockPaymentsDetailComponent implements OnInit {
  ...
  async initInitialData() {
    ...
    if (action === 'new') {
      ...
      const paymentResp = await this.paymentControllerService
  ...
  private async newPayment() {
    ...
    this.paymentForm = generateFormFromMetadata(
      ApiPayment.formMetadata(),
      {},
    ...
    this.paymentForm
      .get('paymentStatus')
      .setValue(PaymentStatusEnum.UNCONFIRMED);
    this.paymentForm.get('stockOrder').setValue(this.stockOrder);

    // Set purchase order (identifier/LOT)
    this.orderReferenceForm.setValue(
      this.mode === ModeEnum.PURCHASE
    ...
    if (this.stockOrder.representativeOfProducerUserCustomer) {
      this.searchCollectorsForm.setValue(
        this.stockOrder.representativeOfProducerUserCustomer,
      );
      this.paymentForm
        .get('representativeOfRecipientUserCustomer')
        .setValue(this.stockOrder.representativeOfProducerUserCustomer);
    }
    ...
    if (this.stockOrder.producerUserCustomer) {
      ...
      this.paymentForm
        .get('recipientUserCustomer')
        .setValue(this.stockOrder.producerUserCustomer);
    }
    ...
    if (this.stockOrder.quoteCompany) {
      ...
      this.paymentForm
        ...
        .setValue(this.stockOrder.quoteCompany);
    }
  ...
  private async editPayment() {
    ...
    this.searchCompaniesForm.setValue(
      this.paymentForm.get('recipientCompany').value,
    );
    this.searchCollectorsForm.setValue(
      this.paymentForm.get('representativeOfRecipientUserCustomer').value,
    );
    this.searchFarmersForm.setValue(
      this.paymentForm.get('recipientUserCustomer').value,
    );
    ...
    if (this.payment.updatedBy) {
      this.lastUpdatedByUser =
        this.payment.updatedBy.name + ' ' + this.payment.updatedBy.surname;
    }
  }
  ...
  async updatePayment() {
    ...
    if (this.paymentForm.get('recipientCompany').value?.id) {
      ...
      this.paymentForm
        .get('recipientType')
        .setValue(RecipientTypeEnum.USERCUSTOMER);
    } else {
    ...
    try {
      const paymentResp = await this.paymentControllerService
        .createOrUpdatePayment(this.paymentForm.getRawValue())
        .pipe(take(1))
        .toPromise();
