...
export class StockPaymentsBulkDetailComponent implements OnInit, OnDestroy {
  ...
  paymentsForm: UntypedFormArray;
  purchaseItems: UntypedFormArray;
  purchaseItemsNoBankAccount: UntypedFormArray;
  bulkPaymentForm: UntypedFormGroup;
  payableToCompany: UntypedFormControl;
  payableToFarmer: UntypedFormControl;
  payableToCollector: UntypedFormControl;
  payableFromForm: UntypedFormControl;

  paymentPurposeTypesCodebook = EnumSifrant.fromObject(
    this.paymentPurposeTypes,
  );
  ...
  static ApiActivityProofItemCreateEmptyObject(): ApiActivityProof {
    return defaultEmptyObject(
      ApiActivityProof.formMetadata(),
    ) as ApiActivityProof;
  }

  static ApiActivityProofItemEmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
  ...
  static ApiPaymentEmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      return new UntypedFormControl(
  ...
  async ngOnInit(): Promise<void> {
    this.payingCompany =
      await this.selUserCompanyService.selectedCompanyProfile$
        .pipe(take(1))
        .toPromise();
  ...
  async initInitialData() {
    ...
    this.paymentsForm = new UntypedFormArray([]);
    this.purchaseItems = new UntypedFormArray([]);
    this.purchaseItemsNoBankAccount = new UntypedFormArray([]);
    this.payableFromForm = new UntypedFormControl(null);
    this.payableToCompany = new UntypedFormControl(null);
    this.payableToFarmer = new UntypedFormControl(
      $localize`:@@productLabelStockBulkPayments.payableTo.multiple:Multiple`,
    );
    this.payableToCollector = new UntypedFormControl(
    ...
    if (action === 'new') {
      ...
      const bulkPaymentResp = await this.paymentControllerService
        .getBulkPayment(bulkPaymentId)
        .pipe(take(1))
        .toPromise();

      if (
        bulkPaymentResp &&
        bulkPaymentResp.status === 'OK' &&
        bulkPaymentResp.data
      ) {
  ...
  newBulkPayment() {
    this.bulkPaymentForm = generateFormFromMetadata(
      ApiBulkPayment.formMetadata(),
      {},
      ApiBulkPaymentValidationScheme,
    );
    ...
    this.bulkPaymentForm
      ...
      .setValue(PaymentPurposeTypeEnum.FIRSTINSTALLMENT);
  }
  ...
  preparePaymentsForEdit() {
    for (const payment of this.bulkPayment.payments) {
      (this.paymentsForm as UntypedFormArray).push(
        new UntypedFormControl(payment),
      );
    }
  }

  conditionalValidators() {
    this.subConditional = this.bulkPaymentForm
      .get('additionalCost')
      .valueChanges.subscribe((val) => {
        if (val) {
          this.bulkPaymentForm.controls[
    ...
    if (this.bulkType === BulkType.BONUS) {
      this.bulkPaymentForm.controls['paymentPerKg'].setValidators([
        Validators.required,
      ]);
  ...
  initializeListManager() {
    this.additionalProofsListManager = new ListEditorManager<ApiActivityProof>(
      this.bulkPaymentForm.get('additionalProofs') as UntypedFormArray,
    ...
    this.paymentsListManager = new ListEditorManager<ApiPayment>(
      this.paymentsForm as UntypedFormArray,
  ...
  async initPayments() {
    ...
    if (this.bulkPayment.createdBy) {
      this.createdBy =
        this.bulkPayment.createdBy.name +
        ' ' +
        this.bulkPayment.createdBy.surname;
    }
  }

  async initPurchaseOrders() {
    const purchaseOrderIds =
      this.route.snapshot.params.purchaseOrderIds.split(',');

    if (this.bulkType === BulkType.PO) {
      for (const purchaseOrderId of purchaseOrderIds) {
        const purchaseOrderResp = await this.stockOrderControllerService
          .getStockOrder(purchaseOrderId)
          .pipe(take(1))
          .toPromise();

        if (
          purchaseOrderResp &&
          purchaseOrderResp.status === 'OK' &&
          purchaseOrderResp.data
        ) {
          const purchaseOrder = purchaseOrderResp.data;

          const purchaseOrderForm = generateFormFromMetadata(
          ...
          if (
            purchaseOrder.preferredWayOfPayment ===
              PreferredWayOfPaymentEnum.CASHVIACOLLECTOR &&
  ...
  async saveBulkPayment() {
    ...
    const formalCreationTime =
      this.bulkPaymentForm.get('formalCreationTime').value;
    if (formalCreationTime) {
      this.bulkPaymentForm
        .get('formalCreationTime')
        .setValue(dateISOString(formalCreationTime));
    }

    (this.bulkPaymentForm.get('payments') as UntypedFormArray).clear();
    ...
    if (!this.update) {
      for (const purchaseOrder of this.purchaseItems.value) {
        const payment = {
          ...
          paymentPurposeType:
            this.bulkPaymentForm.get('paymentPurposeType').value,
          ...
          representativeOfRecipientUserCustomer:
            purchaseOrder.representativeOfRecipientUserCustomer,
    ...
    const bulkPaymentResp = await this.paymentControllerService
      .createBulkPayment(this.bulkPayment)
      .pipe(take(1))
      .toPromise();
  ...
  setTotalAmountToBePaid() {
    if (this.update && this.bulkPayment) {
      this.bulkPaymentForm
        .get('totalAmount')
        .setValue(this.bulkPayment.totalAmount);
      return;
    }
  ...
  get additionalProofsForm(): UntypedFormArray {
    return this.bulkPaymentForm.get('additionalProofs') as UntypedFormArray;
  }
  ...
  get paymentPurposeTypes() {
    const obj = {};
    obj['ADVANCE_PAYMENT'] =
      $localize`:@@paymentForm.paymentPurposeTypes.advancedPayment:Advanced payment`;
    obj['FIRST_INSTALLMENT'] =
      $localize`:@@paymentForm.paymentPurposeTypes.firstInstallment:Base payment`;
    obj['SECOND_INSTALLMENT'] =
      $localize`:@@paymentForm.paymentPurposeTypes.secondInstallment:Member bonus`;
    obj['WOMEN_PREMIUM'] =
      $localize`:@@paymentForm.paymentPurposeTypes.womenPreminum:Women premium`;
    obj['INVOICE_PAYMENT'] =
      $localize`:@@paymentForm.paymentPurposeTypes.invoicePayment:Invoice payment`;
    obj['ORGANIC_BONUS'] =
      $localize`:@@paymentForm.paymentPurposeTypes.organicBonus:Organic bonus`;
    obj['FT_BONUS'] =
      $localize`:@@paymentForm.paymentPurposeTypes.ftBonus:FT bonus`;
    obj['FT_PREMIUM'] =
      $localize`:@@paymentForm.paymentPurposeTypes.ftPremium:FT premium`;
    obj['OTHER_BONUS'] =
      $localize`:@@paymentForm.paymentPurposeTypes.otherBonus:Other bonus`;
    return obj;
  }
