...
export class StockPaymentsFormComponent implements OnInit, OnDestroy {
  ...
  @Input()
  orderReferenceForm = new UntypedFormControl(null);

  @Input()
  payableFromForm = new UntypedFormControl(null);

  @Input()
  searchCompaniesForm = new UntypedFormControl(null);

  @Input()
  searchCollectorsForm = new UntypedFormControl(null);

  @Input()
  searchFarmersForm = new UntypedFormControl(null);
  ...
  searchPreferredWayOfPayment = new UntypedFormControl(null);
  ...
  codebookPreferredWayOfPayment = EnumSifrant.fromObject(
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.initInitialData().then(() => {
      ...
      if (
        ...
        this.paymentForm.value.paymentStatus === PaymentStatusEnum.CONFIRMED
      ) {
        this.viewOnly = true;
      ...
      if (this.mode === ModeEnum.PURCHASE) {
        ...
        if (
          this.paymentForm.get('paymentPurposeType').value ===
          PaymentPurposeTypeEnum.FIRSTINSTALLMENT
        ) {
      ...
      if (this.viewOnly) {
        ...
        if (this.mode === ModeEnum.CUSTOMER) {
          this.paymentForm
            .get('receiptDocumentType')
            .setValue(ReceiptDocumentTypeEnum.RECEIPT);
          this.paymentForm.get('receiptDocumentType').disable();
        }
      } else {
        ...
        if (this.mode === ModeEnum.PURCHASE) {
          if (this.openBalance > 0) {
            if (
              this.searchPreferredWayOfPayment.value ===
              PreferredWayOfPaymentEnum.CASHVIACOLLECTOR
            ) {
              this.paymentForm
                .get('amountPaidToTheCollector')
                .setValue(this.openBalance);
            } else {
    ...
    this.searchCollectorsForm.valueChanges
      .pipe(takeUntil(this.destroy$))
      .subscribe((value) => {
  ...
  private async initInitialData() {
    if (this.stockOrder) {
      this.paymentForm
        .get('productionDate')
        .setValue(this.stockOrder.productionDate);
      this.paymentForm
        .get('preferredWayOfPayment')
        .setValue(this.stockOrder.preferredWayOfPayment);

      if (this.mode === ModeEnum.PURCHASE) {
        this.searchPreferredWayOfPayment.setValue(
          this.stockOrder.preferredWayOfPayment,
        );
        ...
        if (
          ...
          this.paymentForm
            .get('paymentType')
            .setValue(PaymentTypeEnum.BANKTRANSFER);
          this.paymentForm
            .get('amountPaidToTheCollector')
    ...
    this.associatedCompaniesService = new ConnectedCompaniesForCompanyService(
  ...
  private async setConfirmed() {
    this.confirmedAt = formatDateWithDotsAtHour(
      this.paymentForm.get('paymentConfirmedAtTime').value,
    );

    if (this.paymentForm.contains('paymentConfirmedByUser')) {
      const paymentConfirmedByUser = this.paymentForm.get(
        'paymentConfirmedByUser',
      ).value;
      this.confirmedByUser =
        paymentConfirmedByUser.name + ' ' + paymentConfirmedByUser.surname;
    }
  }
  ...
  setCollector(event: ApiUserCustomer) {
    if (event) {
      this.paymentForm
        .get('representativeOfRecipientUserCustomer')
        .setValue(event);
      this.paymentForm.get('receiptDocumentType').setValue(null);
    } else {
      this.paymentForm
  ...
  async setCompany(event) {
    if (event) {
      const companyResp = await this.companyControllerService
        .getCompany(event.company.id)
        .pipe(take(1))
        .toPromise();
  ...
  createPaymentTypes(collector: boolean = false): object {
    ...
    obj['CHEQUE'] =
      $localize`:@@productLabelStockPurchaseOrdersModal.preferredWayOfPayment.cheque:Cheque`;
    obj['OFFSETTING'] =
      $localize`:@@productLabelStockPurchaseOrdersModal.preferredWayOfPayment.offsetting:Offsetting`;
  ...
  get addProofs() {
    const obj = {};
    obj['PURCHASE_SHEET'] =
      $localize`:@@paymentForm.addProofs.purchaseSheet:Purchase sheet`;
  ...
  get preferredWayOfPaymentList() {
    ...
    obj['OFFSETTING'] =
      $localize`:@@productLabelStockPurchaseOrdersModal.preferredWayOfPayment.offsetting:Offsetting`;
    obj['CASH_VIA_COOPERATIVE'] =
      $localize`:@@productLabelStockPurchaseOrdersModal.preferredWayOfPayment.cashViaCooperative:Cash via cooperative`;
    obj['CASH_VIA_COLLECTOR'] =
      $localize`:@@productLabelStockPurchaseOrdersModal.preferredWayOfPayment.cashViaCollector:Cash via collector`;
    obj['BANK_TRANSFER'] =
      $localize`:@@productLabelStockPurchaseOrdersModal.preferredWayOfPayment.bankTransfer:Bank transfer`;
