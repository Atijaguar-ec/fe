...
function onlyOnePayableTo(control: UntypedFormGroup): ValidationErrors | null {
...
function requireReceiptDocument(
  control: UntypedFormGroup,
): ValidationErrors | null {
  const paymentPurposeType =
    control.value && control.value['paymentPurposeType'];
  const receiptDocument = control.value && control.value['receiptDocument'];
  return paymentPurposeType === PaymentPurposeTypeEnum.FIRSTINSTALLMENT &&
    !receiptDocument
    ? { required: true }
    : null;
}
...
export const ApiPaymentValidationScheme = {
  validators: [
    multiFieldValidator(
      ['recipientCompany', 'recipientUserCustomer'],
      atLeastOnePayableTo,
      ['atLeastOnePayableTo'],
    ),
    multiFieldValidator(
    ...
    multiFieldValidator(['receiptDocument'], requireReceiptDocument, [
      'required',
    ]),
  ],
  fields: {
