...
export class GroupStockUnitListComponent implements OnInit, OnDestroy {
  ...
  private loadStockOrders(
    params,
  ): Observable<ApiPaginatedResponseApiGroupStockOrder> {
    ...
    return this.groupStockOrderControllerService.getGroupedStockOrderListByMap({
      ...params,
      facilityId,
    });
  }
  ...
  edit(order: ApiGroupStockOrder) {
    switch (order.orderType as StockOrderType) {
      case 'PURCHASE_ORDER':
        this.router
          .navigate(['my-stock', 'deliveries', 'update', order.groupedIds[0]])
          .then();
        return;
      case 'GENERAL_ORDER':
      ...
      case 'TRANSFER_ORDER':
        this.router
          .navigate(['my-stock', 'processing', 'update', order.groupedIds[0]])
          .then();
        return;
  ...
  history(item: ApiGroupStockOrder) {
    this.router
      .navigate(['all-stock', 'stock-order', item.groupedIds[0], 'view'], {
    ...
    try {
      ...
      if (
        ...
        const stockOrderResp = await this.stockOrderControllerService
          .getStockOrder(order.groupedIds[0], true)
          .pipe(take(1))
          .toPromise();
        if (
          stockOrderResp &&
      ...
      const response = await this.stockOrderControllerService
        .deleteStockOrder(order.groupedIds[0])
        .pipe(take(1))
        .toPromise();
  ...
  private refreshCBs() {
    ...
    for (const item of this.currentData) {
      for (const order of this.selectedOrders) {
        if (
          item.groupedIds.filter((v) => order.groupedIds.includes(v)).length
        ) {
  ...
  aggregateOrderItems(items: ApiGroupStockOrder[]): AggregatedStockItem[] {
    ...
    const aggregatedSemiProductsMap: Map<string, AggregatedStockItem> = items
      ...
      .reduce(
        (acc: Map<string, AggregatedStockItem>, item: ApiGroupStockOrder) => {
