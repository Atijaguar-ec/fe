...
import {
  ...
  ApiPurchaseOrderValidationScheme,
} from './validation';
...
export class StockBulkDeliveryDetailsComponent implements OnInit, OnDestroy {
  ...
  purchaseOrderBulkForm: UntypedFormGroup;
  ...
  searchWomenCoffeeForm: UntypedFormControl[] = [
    new UntypedFormControl(null, Validators.required),
  ];
  netWeightFormArray: UntypedFormControl[] = [new UntypedFormControl(null)];
  finalPriceFormArray: UntypedFormControl[] = [new UntypedFormControl(null)];
  ...
  static AdditionalProofItemEmptyObjectFormFactory(): () => UntypedFormControl {
    return () => {
      return new UntypedFormControl(
        StockBulkDeliveryDetailsComponent.AdditionalProofItemCreateEmptyObject(),
  ...
  finalPriceLabel(idx: number) {
    return (
      ...
      ` (${this.selectedCurrency}/${this.measureUnitArray[idx]})`
    );
  }
  ...
  get additionalProofsForm(): UntypedFormArray {
    return this.purchaseOrderBulkForm.get('activityProofs') as UntypedFormArray;
  }
  ...
  private newPurchaseBulkOrder() {
    this.purchaseOrderBulkForm = generateFormFromMetadata(
      ApiPurchaseOrder.formMetadata(),
      { facility: { id: this.facility.id } },
      ApiPurchaseOrderValidationScheme(),
    );

    this.emptyFarmersFormArray = generateFormFromMetadata(
      ApiPurchaseOrderFarmer.formMetadata(),
      defaultEmptyObject(
        ApiPurchaseOrderFarmer.formMetadata(),
  ...
  async createBulkPurchaseOrder(close: boolean = true) {
    ...
    this.purchaseOrderBulkForm
      .get('creatorId')
      .setValue(this.employeeForm.value);
    ...
    const data: ApiPurchaseOrder = _.cloneDeep(
      this.purchaseOrderBulkForm.value,
    );
  ...
  private cannotUpdatePO() {
    return (
      this.purchaseOrderBulkForm.invalid ||
      this.employeeForm.invalid ||
      this.checkWomenOnlyForm() ||
      this.checkAllTareInvalid() ||
      this.checkAllDamagedPriceDeductionInvalid()
    );
  }

  private checkWomenOnlyForm(): boolean {
    const invalidExist = this.searchWomenCoffeeForm.find(
  ...
  private checkAllTareInvalid(): boolean {
    const invalidExist = this.farmersFormArray.controls.find((control, index) =>
      this.tareInvalidCheck(index),
    );
    return invalidExist !== undefined;
  }
  ...
  tareInvalidCheck(idx: number) {
    const tare: number = Number(
      this.farmersFormArray.at(idx).get('tare').value,
    ).valueOf();
    const totalGrossQuantity: number = Number(
      this.farmersFormArray.at(idx).get('totalGrossQuantity').value,
    ).valueOf();
  ...
  private checkAllDamagedPriceDeductionInvalid(): boolean {
    const invalidExist = this.farmersFormArray.controls.find((control, index) =>
      this.damagedPriceDeductionInvalidCheck(index),
    );
    return invalidExist !== undefined;
  }
  ...
  damagedPriceDeductionInvalidCheck(idx: number) {
    const damagedPriceDeduction: number = Number(
      this.farmersFormArray.at(idx).get('damagedPriceDeduction').value,
    ).valueOf();
    const pricePerUnit: number = Number(
      this.farmersFormArray.at(idx).get('pricePerUnit').value,
    ).valueOf();
    return (
  ...
  damagedWeightDeductionInvalidCheck(idx: number) {
    const damagedWeightDeduction = Number(
      this.farmersFormArray.at(idx).get('damagedWeightDeduction').value,
    ).valueOf();
    const totalQuantity = Number(
      this.farmersFormArray.at(idx).get('totalQuantity').value,
    ).valueOf();
    return (
  ...
  setCollector(event: ApiUserCustomer) {
    if (event) {
      ...
      if (
        this.purchaseOrderBulkForm.get('preferredWayOfPayment') &&
        this.purchaseOrderBulkForm.get('preferredWayOfPayment').value ===
          'UNKNOWN'
      ) {
  ...
  async setMeasureUnit(semiProdId: number, idx: number) {
    if (
      this.measureUnitArray[idx] === null ||
      this.measureUnitArray[idx] === undefined
    ) {
      // init value
  ...
  setToBePaid(idx: number) {
    if (
      this.farmersFormArray.at(idx) &&
      this.farmersFormArray.at(idx).get('totalGrossQuantity').value &&
      this.farmersFormArray.at(idx).get('pricePerUnit').value
    ) {
      let netWeight = this.farmersFormArray
        .at(idx)
        .get('totalGrossQuantity').value;
      ...
      if (this.farmersFormArray.at(idx).get('damagedWeightDeduction').value) {
        netWeight -= this.farmersFormArray
          .at(idx)
          .get('damagedWeightDeduction').value;
      }
      ...
      if (this.farmersFormArray.at(idx).get('damagedPriceDeduction').value) {
        finalPrice -= this.farmersFormArray
          .at(idx)
          .get('damagedPriceDeduction').value;
      }
  ...
  setBalance(idx: number) {
    if (
      this.farmersFormArray.at(idx) &&
      this.farmersFormArray.at(idx).get('cost').value !== null &&
      this.farmersFormArray.at(idx).get('cost').value !== undefined
    ) {
      this.farmersFormArray
        .at(idx)
        .get('balance')
        .setValue(this.farmersFormArray.at(idx).get('cost').value);
    } else {
  ...
  showWomensOnly(idx: number) {
    return (
      ...
      this.farmersFormArray.at(idx).get('womenShare').value
    );
  }
    ...
    this.farmersFormArray.controls.forEach((nextFormGroup) => {
      if (nextFormGroup.get('totalGrossQuantity').valid) {
        let quantity = parseFloat(
          nextFormGroup.get('totalGrossQuantity').value,
        );
  ...
  addFarmerInfo() {
    ...
    if (
      this.measureUnitArray[farmerListSize] === null ||
      this.measureUnitArray[farmerListSize] === undefined
    ) {
      // init value
      this.measureUnitArray.push('-');
    }

    this.netWeightFormArray.push(new UntypedFormControl(null));
    this.finalPriceFormArray.push(new UntypedFormControl(null));
    this.searchWomenCoffeeForm.push(
      new UntypedFormControl(null, Validators.required),
    );

    // add new fields to form, and init semiProduct if only one available
    const emptyFarmersFormArrayNewItem = _.cloneDeep(
      this.emptyFarmersFormArray,
    );
    (emptyFarmersFormArrayNewItem as UntypedFormGroup)
      .get('semiProduct')
      .setValue(semiProductControlValue);
    this.farmersFormArray.push(emptyFarmersFormArrayNewItem);
  ...
  get farmersFormArray() {
    return this.purchaseOrderBulkForm.get('farmers') as UntypedFormArray;
  }
