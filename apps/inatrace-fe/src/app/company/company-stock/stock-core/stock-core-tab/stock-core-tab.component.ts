...
export type StockOrderListingPageMode =
  | 'PURCHASE_ORDERS'
  | 'COMPANY_ADMIN'
  | 'SYSTEM_ADMIN';
...
export class StockCoreTabComponent implements OnInit, AfterViewInit {
  ...
  facilityForStockOrderForm = new UntypedFormControl(null);
  ...
  filterWayOfPayment = new UntypedFormControl('');
  wayOfPaymentPing$ = new BehaviorSubject<string>(
    this.filterWayOfPayment.value,
  );
  ...
  fromFilterDate = new UntypedFormControl(null);
  toFilterDate = new UntypedFormControl(null);
  ...
  tabNames = ['deliveries', 'processing', 'payments', 'all-stock'];
  ...
  private initFacilityCodebook(): void {
    switch (this.pageMode) {
      case 'SYSTEM_ADMIN':
      case 'COMPANY_ADMIN':
        this.facilityCodebook = new CompanyFacilitiesService(
  ...
  facilityForStockOrderChanged(event: ApiFacility) {
    ...
    setNavigationParameter(
      ...
      this.route,
      'facilityId',
      String(this.selectedFacilityId),
    );
  }
  ...
  selectedIdsChanged(event, type?) {
    if (type === 'PURCHASE') {
      ...
      this.selectedGroupOrders = event;
    } else {
  ...
  private async isAuthorisedCompanyRole() {
    const userProfile = await this.authService.userProfile$
      .pipe(take(1))
      .toPromise();

    const companyUsersRes = await this.companyController
