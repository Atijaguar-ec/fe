...
export class BatchHistoryComponent implements OnInit {
  ...
  history$: Observable<ApiStockOrderHistory> = combineLatest([
    ...
    switchMap((val) => {
      return this.stockOrderService.getStockOrderAggregatedHistory(
        val[1].stockOrderId,
      );
    }),
    map((val) => {
          ...
          if (timelineItem.processingOrder) {
            ...
            for (const tso of timelineItem.processingOrder.targetStockOrders) {
              if (tso.sacNumber != null) {
                ...
                if (lastSlashIndex !== -1) {
                  const internalLOTNumber = tso.internalLotNumber.substring(
                    0,
                    lastSlashIndex,
                  );
            ...
            groups.forEach((stockOrders, internalLOTNumber) => {
              const stockOrdersGroup: GroupedStockOrders = {
                ...
                summedUpQuantity: stockOrders
                  .map((so) => so.totalQuantity)
              ...
              stockOrders.forEach((so) => {
                const removeIndex =
                  timelineItem.processingOrder.targetStockOrders.findIndex(
                    (tso) => tso.id === so.id,
                  );
                timelineItem.processingOrder.targetStockOrders.splice(
                  removeIndex,
                  1,
                );
              });
  ...
  getTargetStockOrders(timelineItem: ApiStockOrderHistoryTimelineItem) {
    return timelineItem.purchaseOrders?.length > 0
      ? timelineItem.purchaseOrders
      : timelineItem.processingOrder.targetStockOrders;
  }

  getStockOrderGroups(
    timelineItem: ApiStockOrderHistoryTimelineItem,
  ): GroupedStockOrders[] {
    return timelineItem.processingOrder
      ? timelineItem.processingOrder['stockOrderGroups']
      : [];
  }
  ...
  goToInput(tx: ApiTransaction) {
    this.router
      .navigate(['stock-order', tx.sourceStockOrder.id, 'view'], {
  ...
  async goToOutput(tx: ApiTransaction) {
    this.router
      .navigate(['stock-order', tx.targetStockOrder.id, 'view'], {
        relativeTo: this.route.parent,
      })
      .then();
  }
  ...
  goToSibling(order: ApiStockOrder) {
    this.router
      .navigate(['stock-order', order.id, 'view'], {
        relativeTo: this.route.parent,
      })
      .then();
  }
