...
import {
  DeliveryDates,
  StockOrderListingPageMode,
} from '../stock-core-tab/stock-core-tab.component';
...
export class StockUnitListComponent
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  @Input()
  representativeOfProducerUserCustomerIdPing$ = new BehaviorSubject<number>(
    null,
  );
  ...
  @Input()
  deliveryDatesPing$ = new BehaviorSubject<DeliveryDates>({
    from: null,
    to: null,
  });
  ...
  cbCheckedAll = new UntypedFormControl(false);
    ...
    this.sortOptions = [
      {
        ...
        name: '',
        selectAllCheckbox:
          ['PURCHASE_ORDERS'].indexOf(this.pageListingMode) >= 0,
      ...
      },
      {
        ...
        inactive: true,
        hide:
          ['COMPANY_ADMIN', 'SYSTEM_ADMIN'].indexOf(this.pageListingMode) >= 0,
      },
      {
        key: 'semiProduct',
  ...
  private loadStockOrders(
    ...
    if (this.mode === 'PURCHASE') {
      if (!facilityId) {
        return this.stockOrderControllerService.getStockOrderListByCompanyIdByMap(
          { ...params, companyId: this.companyId },
        );
      }
      return this.stockOrderControllerService.getStockOrderListByFacilityIdByMap(
        { ...params, facilityId },
      );
    }
  ...
  edit(order: ApiStockOrder) {
    if (this.pageListingMode === 'PURCHASE_ORDERS') {
      this.router
        .navigate(['my-stock', 'deliveries', 'update', order.id])
        .then();
    } else {
      switch (order.orderType as StockOrderType) {
        ...
        case 'TRANSFER_ORDER':
          this.router
            .navigate(['my-stock', 'processing', 'update', order.id])
            .then();
          return;
  ...
  history(item: ApiStockOrder) {
    this.router
      .navigate(['all-stock', 'stock-order', item.id, 'view'], {
  ...
  payment(order: ApiStockOrder) {
    ...
    this.router
      .navigate(['my-stock', 'payments', 'delivery', order.id, 'new'])
      .then();
  }

  farmerProfile(id) {
    this.router
      .navigate(['my-farmers', 'edit', id], {
        queryParams: { returnUrl: this.router.routerState.snapshot.url },
      })
      .then();
  }
  ...
  canDelete(order: ApiStockOrder) {
    if (order.orderType === 'PROCESSING_ORDER') {
      return true;
    }
    if (order.orderType === 'TRANSFER_ORDER') {
      return true;
    }
    if (order.orderType === 'GENERAL_ORDER') {
      return (
        Math.abs(order.fulfilledQuantity - order.availableQuantity) <
        0.0000000001
      );
    }
    return true;
  ...
  async delete(order: ApiStockOrder) {
    let confirmResult;
    if (
      order.orderType === 'PROCESSING_ORDER' ||
      order.orderType === 'TRANSFER_ORDER'
    ) {
    ...
    try {
      ...
      if (
        order.orderType === 'PROCESSING_ORDER' ||
        order.orderType === 'TRANSFER_ORDER' ||
        order.orderType === 'GENERAL_ORDER'
      ) {
        const stockOrderResp = await this.stockOrderControllerService
          .getStockOrder(order.id, true)
          .pipe(take(1))
          .toPromise();
        if (
          stockOrderResp &&
          stockOrderResp.status === 'OK' &&
          stockOrderResp.data
        ) {
          const procOrderResp = await this.processingOrderController
            .deleteProcessingOrder(stockOrderResp.data.processingOrder.id)
            .pipe(take(1))
            .toPromise();
      ...
      const response = await this.stockOrderControllerService
  ...
  showPagination() {
    return (
      (this.showedOrders - this.pageSize === 0 &&
        this.allOrders >= this.pageSize) ||
      this.page > 1
    );
  }
  ...
  cbSelected(order: ApiStockOrder, index: number) {
    (this.currentData[index] as any).selected = !(
      this.currentData[index] as any
    ).selected;
  ...
  aggregateOrderItems(items: ApiStockOrder[]): AggregatedStockItem[] {
    ...
    const aggregatedSemiProductsMap: Map<number, AggregatedStockItem> = items
      .filter(
        (stockOrder) => stockOrder.semiProduct && stockOrder.semiProduct.id,
      )
    ...
    const aggregatedFinalProducts: Map<number, AggregatedStockItem> = items
      .filter(
        (stockOrder) => stockOrder.finalProduct && stockOrder.finalProduct.id,
      )
      .reduce((acc: Map<number, AggregatedStockItem>, item: ApiStockOrder) => {
        const nextTotalQuantity = item.totalQuantity ? item.totalQuantity : 0;
    ...
    return [
      ...Array.from(aggregatedSemiProductsMap.values()),
      ...Array.from(aggregatedFinalProducts.values()),
    ];
  }
  ...
  farmerName(farmer: ApiUserCustomer) {
    if (farmer) {
      if (farmer.location?.address?.country?.code === 'RW') {
        const cell = farmer.location.address.cell
          ? farmer.location.address.cell.substring(0, 2).toLocaleUpperCase()
          : '--';
        const village = farmer.location.address.village
          ? farmer.location.address.village.substring(0, 2).toLocaleUpperCase()
          : '--';
        return (
          farmer.name +
          ' ' +
        ...
        const municipality = farmer.location.address.hondurasMunicipality
          ? farmer.location.address.hondurasMunicipality
          : '--';
        const village = farmer.location.address.hondurasVillage
  ...
  async exportGeoData(order: ApiStockOrder) {
    ...
    if (res.size > 0) {
      ...
      this.toasterService.info(
        $localize`:@@orderList.export.geojson.noDataAvailable:There is no Geo data available for this order`,
      );
      return;
    }
  }
