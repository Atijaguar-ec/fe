...
export class StockDeliveriesTabComponent
  extends StockCoreTabComponent
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.facilityIdChangeSub = this.facilityIdPing$.subscribe((facilityId) =>
      this.setFacilitySemiProducts(facilityId),
    );
  }
  ...
  async exportDeliveriesExcel(): Promise<void> {
    ...
    try {
      const res = await this.stockOrderControllerService
        .exportDeliveriesByCompany(this.companyId)
        .pipe(take(1))
        .toPromise();
  ...
  private setFacilitySemiProducts(facilityId: number) {
    if (facilityId !== null && facilityId !== undefined) {
      this.facilitySemiProducts = new FacilitySemiProductsCodebookService(
