...
export class CompanyCustomersDetailsComponent implements OnInit {
  ...
  submitted = false;
  userProfile;
  semiProductWithPrices = new UntypedFormArray([]);
  ...
  async initialize() {
    ...
    if (action === 'new') {
      ...
      const res = await this.companyService
        .getCompanyCustomer(this.customerId)
        .pipe(take(1))
        .toPromise();
  ...
  newCustomer() {
    this.customerForm = generateFormFromMetadata(
      ApiCompanyCustomer.formMetadata(),
      this.emptyCustomer(),
      ApiCompanyCustomerValidationScheme,
    );
  }
  ...
  emptyCustomer() {
    const obj: ApiCompanyCustomer = defaultEmptyObject(
      ApiCompanyCustomer.formMetadata(),
    );
    obj.location = defaultEmptyObject(
      ApiGeoAddress.formMetadata(),
    ) as ApiGeoAddress;
