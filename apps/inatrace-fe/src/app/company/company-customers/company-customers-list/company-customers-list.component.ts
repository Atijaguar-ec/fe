...
export class CompanyCustomersListComponent implements OnInit {
  ...
  searchNameCustomers = new UntypedFormControl('');
  byCategory = 'BY_NAME';
  ...
  private async loadCustomers() {
    this.customer$ = combineLatest([
      ...
      switchMap((params) =>
        this.companyController.getCompanyCustomersListByMap(params),
      ),
      map((response) => {
        if (response) {
          this.customerCount = response.data.count;
          this.showing =
            this.customerCount >= this.pageSize
              ? Math.min(this.page * this.pageSize, this.customerCount)
              : this.customerCount;
