...
export class CompanyDetailTabManagerComponent
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  tabNames = ['company', 'users', 'facilities', 'processingActions'];
  ...
  ngOnInit(): void {
      ...
      (userProfile) => {
        if (userProfile) {
          ...
          this.isAdmin =
            ...
            userProfile.role === ApiUserGet.RoleEnum.REGIONALADMIN;
          ...
          if (this.cId != null) {
            ...
            this.isCompanyAdmin =
              userProfile.companyIdsAdmin.findIndex(
                (id) => String(id) === this.cId,
              ) !== -1;

            // If not company enrolled, lock the facilities and processing actions tabs
            if (!this.isCompanyAdmin) {
              this.lockedTabs = ['facilities', 'processingActions'];
            }
          } else {
            // If creating a new company, these tabs should be locked
            this.lockedTabs = ['users', 'facilities', 'processingActions'];
          }
        }
      },
    );
    ...
    if (this.rootTab !== 0) {
      this.companyController
        .getCompanyName(Number(this.cId))
        .subscribe((resp) => {
