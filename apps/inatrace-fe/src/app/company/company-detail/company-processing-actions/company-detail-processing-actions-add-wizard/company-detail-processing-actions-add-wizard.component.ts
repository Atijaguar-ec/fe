...
import {
  ApiEnumWithLabelString,
  EnumSifrant,
} from '../../../../shared-services/enum-sifrant';
...
@Component({
  ...
  styleUrls: [
    './company-detail-processing-actions-add-wizard.component.scss',
    '../../company-detail.component.scss',
  ],
  standalone: false,
})
export class CompanyDetailProcessingActionsAddWizardComponent
  implements OnInit
{
  protected readonly faTimes = faTimes;

  // Step 1
  processingActionTypesCodebookService = EnumSifrant.fromObject(
  ...
  // Step 2
  procActionNameControl = new UntypedFormControl(null, Validators.required);

  // Step 3
  procActionLOTNameControl = new UntypedFormControl(null, Validators.required);
  ...
  // Step 5
  selectedInputSemiProductControl = new UntypedFormControl(
    null,
    Validators.required,
  );
  ...
  selectedOutputFinalProductControl = new UntypedFormControl(
    null,
    Validators.required,
  );
  ...
  outputSemiProductControl = new UntypedFormControl(null);
  selectedOutputSemiProductsControl = new UntypedFormControl(null, [
    ListNotEmptyValidator(),
  ]);
  ...
  ngOnInit(): void {
    this.processingActionTypesCodebookService
      .getAllCandidates()
      .pipe(take(1))
      .subscribe((value) => {
  ...
  setSelectedProcActionType(procActionType: string) {
    this.selectedProcActionType =
      procActionType as ApiProcessingAction.TypeEnum;
    this.currentStep++;
  }
  ...
  setProcActionValueChains() {
    ...
    if (this.selectedCompanyValueChainsControl.valid) {
      const valueChainIds = (
        this.selectedCompanyValueChainsControl.value as ApiValueChain[]
  ...
  selectProcActionFacility(facility: ApiFacility) {
    const index = this.selectedProcActionFacilities.findIndex(
      (f) => f.id === facility.id,
    );
    if (index > -1) {
  ...
  createNewProcessingAction() {
    let processingAction: ApiProcessingAction = {
      ...
      translations: [
        {
          name: this.procActionNameControl.value,
          description: '',
    ...
    switch (this.selectedProcActionType) {
      case TypeEnum.PROCESSING:
        processingAction.outputSemiProducts =
          this.selectedOutputSemiProductsControl.value;
        break;
      case TypeEnum.FINALPROCESSING:
        processingAction.outputFinalProduct =
          this.selectedOutputFinalProductControl.value;
        break;
      case TypeEnum.GENERATEQRCODE:
        processingAction.outputSemiProducts = [
          this.selectedInputSemiProductControl.value,
        ];
        processingAction.qrCodeForFinalProduct =
          this.qrCodeForFinalProductControl.value;
        break;
      default:
        processingAction.outputSemiProducts = [
          this.selectedInputSemiProductControl.value,
    ...
    this.processingActionControllerService
      .createOrUpdateProcessingAction(processingAction)
      .subscribe(() => {
