...
export class CompanyDetailProcessingActionsDetailComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  codebookProcessingTransaction = EnumSifrant.fromObject(
    this.processingActionType,
  );

  outputSemiProductInputForm: UntypedFormControl;
  ...
  evidenceDocInputForm: UntypedFormControl;
  evidenceFieldInputForm: UntypedFormControl;

  supportedFacilitiesInputForm: UntypedFormControl;
  ...
  get processingActionType() {
    const obj = {};
    obj['SHIPMENT'] =
      $localize`:@@companyDetailProcessingActions.singleChoice.type.quote:Quote`;
    obj['PROCESSING'] =
      $localize`:@@companyDetailProcessingActions.singleChoice.type.processing:Processing`;
    obj['FINAL_PROCESSING'] =
      $localize`:@@companyDetailProcessingActions.singleChoice.type.finalProcessing:Final processing`;
    obj['TRANSFER'] =
      $localize`:@@companyDetailProcessingActions.singleChoice.type.transfer:Transfer`;
    obj['GENERATE_QR_CODE'] =
      $localize`:@@companyDetailProcessingActions.singleChoice.type.generateQrCode:Generate QR code`;
    return obj;
  }

  get maxOutputFinalProductQuantityLabel() {
    const outputMeasurementUnit: string =
      (this.outputFinalProductControl.value as ApiFinalProduct)
        ?.measurementUnitType?.label ?? '';
  ...
  get estimatedOutputQuantityLabel() {
    ...
    return (
      $localize`:@@companyDetailProcessingActions.textInput.estimatedOutputQuantity.label:Estimated output quantity per` +
  ...
  private get inputMeasurementUnit(): ApiMeasureUnitType {
    ...
    return (this.inputSemiProductControl.value as ApiSemiProduct)
      .measurementUnitType;
  }

  public get outputSemiProductsArray(): UntypedFormArray {
    return this.form.get('outputSemiProducts') as UntypedFormArray;
  }

  private get outputFinalProductControl(): UntypedFormControl {
    return this.form.get('outputFinalProduct') as UntypedFormControl;
  }

  private get repackedOutputFinalProductsControl(): UntypedFormControl {
    return this.form.get('repackedOutputFinalProducts') as UntypedFormControl;
  }

  private get maxOutputFinalProductsWeightControl(): UntypedFormControl {
    return this.form.get('maxOutputWeight') as UntypedFormControl;
  }

  private get inputSemiProductControl(): UntypedFormControl {
    return this.form.get('inputSemiProduct') as UntypedFormControl;
  }
  ...
  ngOnInit(): void {
    ...
    this.evidenceDocInputForm = new UntypedFormControl(null);
    this.evidenceFieldInputForm = new UntypedFormControl(null);
    this.supportedFacilitiesInputForm = new UntypedFormControl(null);
    this.outputSemiProductInputForm = new UntypedFormControl(null);
    ...
    this.initInitialData().then(() => {
      ...
      this.supportedFacilitiesService = new CompanyFacilitiesService(
        this.facilitiesController,
        this.companyId,
      );
      ...
      this.valueChainSubs = this.form
        ...
        .valueChanges.subscribe((valueChains: ApiValueChain[]) => {
          if (valueChains && valueChains.length > 0) {
            const valueChainIds = valueChains?.map(
              (valueChain) => valueChain.id,
            );

            // Initialize codebook services for proc. evidence types and proc. evidence fields
            this.processingEvidenceTypeService =
              new ProcessingEvidenceTypeService(
                this.processingEvidenceTypeControllerService,
                this.codebookTranslations,
                'DOCUMENT',
                valueChainIds,
              );
            this.processingEvidenceFieldService =
              new ProcessingEvidenceFieldsService(
                this.processingEvidenceFieldControllerService,
                this.codebookTranslations,
                valueChainIds,
              );
  ...
  emptyObject() {
    const obj = defaultEmptyObject(
      ApiProcessingAction.formMetadata(),
    ) as ApiProcessingAction;
    obj.inputSemiProduct = defaultEmptyObject(
      ApiSemiProduct.formMetadata(),
    ) as ApiSemiProduct;
  ...
  finalizeForm() {
    if (!this.form.contains('outputSemiProducts')) {
      this.form.addControl('outputSemiProducts', new UntypedFormArray([]));
    }

    if (!this.form.contains('requiredEvidenceFields')) {
      this.form.addControl('requiredEvidenceFields', new UntypedFormArray([]));
    }

    if (!this.form.contains('requiredDocumentTypes')) {
      this.form.addControl('requiredDocumentTypes', new UntypedFormArray([]));
    }

    if (!this.form.contains('supportedFacilities')) {
      this.form.addControl('supportedFacilities', new UntypedFormArray([]));
    }
  ...
  newAction() {
    this.form = generateFormFromMetadata(
      ApiProcessingAction.formMetadata(),
      this.emptyObject(),
      ApiProcessingActionValidationScheme,
    );
  ...
  editAction() {
    ...
    if (this.action.outputSemiProducts?.length > 0) {
      ...
      this.action.outputSemiProducts.forEach((apiPaOSM) => {
        const apiPaOSMFormGroup = generateFormFromMetadata(
          ApiProcessingActionOutputSemiProduct.formMetadata(),
          apiPaOSM,
        );
        apiPaOSMFormGroup
          .get('maxOutputWeight')
  ...
  async initInitialData() {
    ...
    if (action === 'new') {
      ...
      const defaultValChainCheck = await this.companyController
        .getCompanyValueChains(this.companyId)
        .pipe(take(1))
        .toPromise();
      ...
      if (resp && resp.status === 'OK') {
        ...
        this.valueChains = this.action?.valueChains
          ? this.action?.valueChains
          : [];
  ...
  async removeEvidenceDoc(doc: ApiProcessingEvidenceType) {
    ...
    const index = (formArray.value as ApiProcessingEvidenceType[]).findIndex(
      (x) => x.id === doc.id,
    );
    if (index >= 0) {
  ...
  async removeEvidenceField(field: ApiProcessingEvidenceField) {
    ...
    const index = (formArray.value as ApiProcessingEvidenceField[]).findIndex(
      (x) => x.id === field.id,
    );
    if (index >= 0) {
      formArray.removeAt(index);
  ...
  async addSelectedSupportedFacility(facility: ApiFacility) {
    ...
    const formArray = this.form.get('supportedFacilities') as UntypedFormArray;
    ...
    // Add selected element to array
    formArray.push(new UntypedFormControl({ ...facility }));
    formArray.markAsDirty();
  ...
  async removeSupportedFacility(facility: ApiFacility) {
    ...
    const formArray = this.form.get('supportedFacilities') as UntypedFormArray;
    const index = (formArray.value as ApiFacility[]).findIndex(
      (x) => x.id === facility.id,
    );
    if (index >= 0) {
      formArray.removeAt(index);
  ...
  async addSelectedOutputSemiProduct(semiProduct: ApiSemiProduct) {
    ...
    const semiProductForm = generateFormFromMetadata(
      ApiProcessingActionOutputSemiProduct.formMetadata(),
      semiProduct,
    );
  ...
  repackOutputSemiProductChecked(index: number) {
    const repackChecked: boolean = this.outputSemiProductsArray
      .at(index)
      .get('repackedOutput').value;

    const maxOutputWeightControl = this.outputSemiProductsArray
      .at(index)
      .get('maxOutputWeight');
  ...
  groupDecided(doc: ApiProcessingEvidenceType) {
    ...
    let group = 1;
    const lst: ApiProcessingEvidenceType[] =
      this.form.get('requiredDocumentTypes').value || [];
    const res = lst
      .map((x) => x.requiredOneOfGroupIdForQuote)
  ...
  goBack() {
    this.router
      .navigate(['companies', this.companyId, 'processingActions'])
      .then();
  }
