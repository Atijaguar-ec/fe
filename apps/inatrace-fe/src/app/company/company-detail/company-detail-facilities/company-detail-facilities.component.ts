...
import { CompanyDetailFacilityAddWizardComponent } from '../company-detail-facility-add-wizard/company-detail-facility-add-wizard.component';
...
export class CompanyDetailFacilitiesComponent
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  ngAfterViewInit() {
    ...
    this.subscriptions =
      this.selfOnboardingService.addFacilityCurrentStep$.subscribe((step) => {
        if (step === 3) {
  ...
  loadEntityList() {
    return this.facilityControllerService.listAllFacilitiesByCompanyByMap({
      id: this.companyId,
    });
  }
  ...
  locationName(location: ApiFacilityLocation) {
    if (location.address.cell != null && location.address.country) {
      return location.address.cell + ', ' + location.address.country.name;
    }
    if (location.address.city != null && location.address.country) {
      return location.address.city + ', ' + location.address.country.name;
    }
    if (location.address.country != null) {
      return location.address.country.name;
    }
    return '';
  }
  ...
  showPagination() {
    return (
      (this.displayedFacilities - this.pageSize === 0 &&
        this.allFacilities >= this.pageSize) ||
      this.page > 1
    );
  }
      ...
      tap((resp: ApiPaginatedResponseApiFacility) => {
        if (resp) {
          ...
          if (
            ...
            this.displayedFacilities = resp.data.count;
          } else if (
            resp.data &&
            resp.data.count &&
            this.pageSize - resp.data.count <= 0
            ...
            this.displayedFacilities =
              temp >= this.pageSize ? this.pageSize : temp;
          }
  ...
  async activateFacility(facilityId) {
    const resActivate = await this.facilityControllerService
  ...
  async deactivateFacility(facilityId) {
    ...
    const resDeactivate = await this.facilityControllerService
