...
@Component({
  ...
  styleUrls: [
    './company-detail-facility-add-wizard.component.scss',
...
export class CompanyDetailFacilityAddWizardComponent implements OnInit {
  ...
  // Step 2
  facilityNameControl = new UntypedFormControl(null, Validators.required);

  // Step 3
  collectionFacility = false;

  // Step 4
  mayInvolveCollectors = new UntypedFormControl(false);
  mayContainOrganic = new UntypedFormControl(false);
  mayContainWomenOnly = new UntypedFormControl(false);
  ableToDocumentTare = new UntypedFormControl(false);
  priceDeductionDamage = new UntypedFormControl(false);
  weightDeductionDamage = new UntypedFormControl(false);
  priceDeterminedLater = new UntypedFormControl(false);
  ...
  // Step 8
  facilityLocationParentForm = generateFormFromMetadata(
  ...
  ngOnInit(): void {
    ...
    this.finalProductsForCompanyService = new FinalProductsForCompanyService(
      this.finalProductController,
      Number(this.companyId),
    );
    this.finalProductsForCompanyService
      .getAllCandidates()
      .subscribe((value) => {
  ...
  selectSemiProduct(semiProduct: ApiSemiProduct) {
    const index = this.selectedSemiProducts.findIndex(
      (sp) => sp.id === semiProduct.id,
    );
    if (index > -1) {
  ...
  selectFinalProduct(finalProduct: ApiFinalProduct) {
    const index = this.selectedFinalProducts.findIndex(
      (fp) => fp.id === finalProduct.id,
    );
    if (index > -1) {
  ...
  setFacilityLocation() {
    ...
    const facilityLocationForm = this.facilityLocationParentForm.get(
      'facilityLocation',
    ) as UntypedFormGroup;
