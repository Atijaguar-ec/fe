...
export class CompanyListComponent implements OnInit, OnDestroy {
  ...
  myCompanies = new UntypedFormControl(true);
  ...
  searchParams$ = combineLatest(
    ...
    this.searchStatus.valueChanges.pipe(startWith(null)),
    this.myCompanies.valueChanges.pipe(startWith(true)),
    ...
    map((resp: ApiPaginatedResponseApiCompanyListResponse) => {
      if (resp) {
        if (
          ...
          this.showedCompanies = resp.data.count;
        } else {
  ...
  async setAllCompanies() {
    if (this.isSystemAdmin) {
      const res = await this.companyController
        .listCompaniesAdmin(null, null, null, 'COUNT')
        .pipe(take(1))
        .toPromise();
      ...
      const res = await this.companyController
        .listCompanies(null, null, null, 'COUNT')
        .pipe(take(1))
        .toPromise();
      if (res && res.status === 'OK' && res.data && res.data.count >= 0) {
        this.allCompanies = res.data.count;
  ...
  get statusList() {
    ...
    obj['REGISTERED'] =
      $localize`:@@companyList.statusList.registred:Registred`;
    obj['ACTIVE'] = $localize`:@@companyList.statusList.active:Active`;
    obj['DEACTIVATED'] =
      $localize`:@@companyList.statusList.deactivated:Deactivated`;
    return obj;
  }
  ...
  async activate(id) {
    try {
      ...
      await this.companyController
        .executeCompanyAction('ACTIVATE_COMPANY', { companyId: id })
        .pipe(take(1))
        .toPromise();
      this.reloadPage();
  ...
  async deactivate(id) {
    try {
      this.globalEventsManager.showLoading(true);
      await this.companyController
        .executeCompanyAction('DEACTIVATE_COMPANY', { companyId: id })
        .pipe(take(1))
        .toPromise();
      this.reloadPage();
      this.globalEventsManager.push({
  ...
  async mergeTo(company) {
    ...
    if (otherCompany) {
      const res = await this.companyController
        .executeCompanyAction('MERGE_TO_COMPANY', {
  ...
  clearValue(form: UntypedFormControl, myCompanies: boolean = false) {
    if (myCompanies) {
      form.setValue(false);
    } else {
      form.setValue('');
  ...
  showPagination() {
    return (
      (this.showedCompanies - this.pageSize === 0 &&
        this.allCompanies >= this.pageSize) ||
      this.page > 1
    );
  }
}
