 ...
 */
export function objectSlice<T extends object>(
  obj: T,
  keys: Array<keyof T>,
): Partial<T> {
    ...
    for (let key of obj) {
      options.push({
        ...
        value: forceLowercase
          ? (obj[key] as string).toLocaleLowerCase()
          : obj[key],
      });
    }
    return options;
...
export function applyValidationScheme(
  inForm: AbstractControl,
  ...
  if (valScheme.fields) {
    ...
    for (let key in valScheme.fields) {
      if (!(form as UntypedFormGroup).controls[key]) {
        throw Error('Non matching key for validation scheme: ' + key);
      }
      applyValidationScheme(
        (form as UntypedFormGroup).controls[key],
        valScheme.fields[key],
      );
      (form as UntypedFormGroup).controls[key].updateValueAndValidity();
    }
  } else if (valScheme.arrayElementValidators) {
    (form as UntypedFormArray).controls.forEach((a: AbstractControl) => {
...
export function duplicateValidationScheme(
  valScheme: SimpleValidationScheme<any>,
): SimpleValidationScheme<any> {
  let dup: any = {
  ...
  if (valScheme.arrayElementValidators) {
    dup.arrayElementValidators = duplicateValidationScheme(
      valScheme.arrayElementValidators,
    );
  }
  if (valScheme.fields) {
    dup.fields = {};
...
export function setForceFormControl(
  valScheme: SimpleValidationScheme<any>,
  value: boolean = true,
) {
...
export function setForceExpand(
  valScheme: SimpleValidationScheme<any>,
  value: boolean = true,
) {
  let vs = duplicateValidationScheme(valScheme);
...
export function generateFormFromMetadata(
  ...
  for (let varInfo of metadata.vars) {
    ...
    let validators =
      valScheme &&
      valScheme.fields &&
      valScheme.fields[varInfo.name] &&
      valScheme.fields[varInfo.name].validators;
    let forceFormControl =
      valScheme &&
      valScheme.fields &&
      valScheme.fields[varInfo.name] &&
      valScheme.fields[varInfo.name].forceFormControl;
    let forceExpand =
      valScheme &&
      valScheme.fields &&
      valScheme.fields[varInfo.name] &&
      valScheme.fields[varInfo.name].forceExpand;
    let fieldSchema =
      valScheme && valScheme.fields && valScheme.fields[varInfo.name];
    let arrayElementsSchema =
      valScheme &&
      valScheme.fields &&
      valScheme.fields[varInfo.name] &&
      valScheme.fields[varInfo.name].arrayElementValidators;
    if (forceFormControl && forceExpand) {
      throw Error(
        'forceFormControl in forceExpand must not be true at the same time.',
      );
    }
    if (
      value == null &&
      varInfo.required &&
      !(varInfo.isListContainer || varInfo.isPrimitiveType || varInfo.isEnum)
    ) {
      throw Error(
        'Required complex variable missing: ' + name + `(${varInfo.classname})`,
      );
    }
    if (varInfo.isPrimitiveType) {
      if (varInfo.isListContainer) {
            ...
            value
              ? value.map((x) => {
                  if (arrayElementsSchema && arrayElementsSchema.forceExpand) {
                    ...
                    return generateFormFromMetadata(
                      varInfo.metadata(),
                      x,
                      arrayElementsSchema,
                    );
                    ...
                    return new UntypedFormControl(
                      x,
                      arrayElementsSchema && arrayElementsSchema.validators,
                    );
                  }
                })
              : [],
        ...
        result.setControl(name, new UntypedFormControl(value, validators));
      }
      ...
      if (varInfo.isEnum) {
        ...
        if (forceExpand) {
          if (value != null) {
            result.setControl(
              name,
              generateFormFromMetadata(varInfo.metadata(), value, fieldSchema),
            );
          } else {
            ...
            result.setControl(name, new UntypedFormControl(null));
          }
        } else {
...
export function formAssign(
  ...
  for (let varInfo of metadata.vars) {
    ...
    if (forceFormControl || varInfo.isPrimitiveType) {
      ...
      let fa = form.get(name) as UntypedFormArray;
      fa.clear();
      for (let el of value) {
        fa.push(
          generateFormFromMetadata(varInfo.metadata(), el, arrayElementsSchema),
        );
      }
    } else {
      if (varInfo.isEnum) {
        ...
        if (form.get(name).value == null && value != null) {
          // legacy
          form.setControl(
            name,
            generateFormFromMetadata(varInfo.metadata(), value, fieldSchema),
          ...
          form.setControl(
            name,
            new UntypedFormControl(null, fieldSchema.validators),
          );
        } else {
          formAssign(
            form.get(name) as UntypedFormGroup,
            varInfo.metadata(),
            value,
            fieldSchema,
          );
...
export function getValidatorParameter(
...
export function getPath(route: ActivatedRouteSnapshot): string {
  let pathFromParent = route.pathFromRoot
    .map((x) => {
      if (x.url.length > 0) {
...
export function formatDateWithDotsAtHour(dateTimeToBeFormatted: string) {
  ...
  const time =
    wholeDate.getHours() +
    ':' +
    wholeDate.getMinutes().toString().padStart(2, '0');
...
export function setNavigationParameter(
  router: Router,
  route: ActivatedRoute,
  paramName: string,
  paramValue: string,
...
export async function setSelectedIdFieldFromQueryParams(
  component: any,
  route: ActivatedRoute,
  field: string,
  form: UntypedFormControl,
  codebook: GeneralSifrantService<any>,
